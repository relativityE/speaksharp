...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      severity: 'ignore', // Suppressed for monorepo transitive dependency noise
      comment:
    ...
    // 🚀 SPEAKSHARP ARCHITECTURAL BOUNDARIES
    {
      name: 'no-direct-service-access-from-hooks',
      comment: 'Hooks must use SpeechRuntimeController, never TranscriptionService directly. (Rule 1)',
      severity: 'error',
      from: {
      ...
      to: {
        path: '^frontend/src/services/transcription/TranscriptionService\\.ts$'
      }
    },
    {
      name: 'no-ui-imports-in-services',
      comment: 'Services must be pure logic and never import UI components or pages. (Rule 1)',
      severity: 'error',
      from: {
      ...
      to: {
        path: '^frontend/src/(components|pages|hooks)',
        pathNot: '(__tests__|\\.test\\.|\\.spec\\.)'
      }
    },
    {
      name: 'transcription-domain-isolation',
      comment: 'Transcription internals should only be accessed via the Controller or Public API.',
      severity: 'error',
      from: {
        pathNot: '^frontend/src/services/(transcription|SpeechRuntimeController\\.ts)'
      },
      to: {
        path: '^frontend/src/services/transcription/modes'
      }
    }
  ],
  options: {
...
// generated: dependency-cruiser@17.3.8 on 2026-03-12T15:57:13.771Z
