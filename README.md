...
7.  **Validate Your Local Installation:**
    If you're a new developer setting up the project, it's highly recommended to run the **Core System Probe** to ensure your local environment is sound:
    ```bash
    pnpm test:core
    ```
...
*   **Run the core environmental probe (Recommended):**
    ```bash
    pnpm test:core
    ```
...
*   When run locally (e.g., `pnpm test:full` or `pnpm test:core`), a summary is printed to your console.
...
pnpm test:core
```
- Runs preflight, build, and the **Core System Probe** Journey
