...
/**
 * #1042 PR2 — "How Freestyle Practice works" Session help overlay.
 *
...
const DIR = 'test-results/freestyle-help';
...
async function openSession(page: Page) {
    ...
    await expect(page.getByTestId('freestyle-help-button')).toBeVisible({ timeout: 15_000 });
}

test.describe('#1042 PR2 Freestyle help overlay', () => {
    test('desktop: opens above Mic ready, closes on Escape with focus return, disabled while recording', async ({ page }) => {
        ...
        const order = await page.evaluate(() => {
            const btn = document.querySelector('[data-testid="freestyle-help-button"]');
        ...
        await help.click();
        const overlay = page.getByTestId('freestyle-help-overlay');
        ...
        await expect(page.getByTestId('freestyle-help-steps').getByRole('listitem')).toHaveCount(6);
        await expect(page.getByTestId('freestyle-help-feedback')).toContainText(FEEDBACK);
        ...
        await expect(page.getByTestId('freestyle-help-overlay')).toHaveCount(0);
    ...
    test('mobile: opens as a bottom sheet', async ({ page }) => {
        ...
        await page.getByTestId('freestyle-help-button').click();
        const overlay = page.getByTestId('freestyle-help-overlay');
        await expect(overlay).toBeVisible();
        // The panel must be FULLY OPAQUE (no page bleed-through) — fail if it settles translucent.
        await expect
