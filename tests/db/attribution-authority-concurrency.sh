...
BEGIN;
INSERT INTO proof_result(who,val) VALUES ('A', public.attest_session_engine_v1('$SESS','$EVID'::jsonb));
...
( PGAPPNAME=connB psql -qAtX -v ON_ERROR_STOP=1 -c "SET ROLE service_role" \
    -c "INSERT INTO proof_result(who,val) VALUES ('B', public.attest_session_engine_v1('$SESS','$EVID'::jsonb))" >/dev/null 2>&1 ) & BPID=$!
...
R=$(qs "SELECT public.attest_session_engine_v1('$SESS','$EVID'::jsonb)")
...
exec 8>"$CO3_FIFO"
printf 'SELECT pg_advisory_lock(666);\n' >&8
for _ in $(seq 1 100); do [ "$(q "SELECT count(*) FROM pg_locks WHERE locktype='advisory' AND objid=666 AND granted")" = "1" ] && break; sleep 0.05; done

( PGAPPNAME=connCompleteA psql -qAtX -v ON_ERROR_STOP=1 >/dev/null 2>&1 <<SQL
BEGIN;
UPDATE public.sessions SET status='completed' WHERE id='$SESS3';
SELECT pg_advisory_lock(555);
SELECT pg_advisory_lock(666);
COMMIT;
SQL
) & A3=$!
