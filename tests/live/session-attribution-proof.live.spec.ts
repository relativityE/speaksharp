...
test.describe.serial('#1033 live production attribution proof @live', () => {
  ...
  test('a successful Pro recording persists attribution_status=verified with a coherent engine tuple, no duplicate', async ({ page }) => {
    ...
    // controller creates the placeholder DB row and publishes its id to the store as soon as recording
    // starts, so WAIT for that id (this inherently waits through token issuance + recording start), then
    // assert it is present. Doing this first guarantees afterAll cleanup always has an owner-scoped target
    // even if the token / recording-state / transcript / save assertions below fail. If the token never
    // succeeds, no row is created and this poll times out with a clear message (nothing to leak).
    const readStoreSessionId = () => page.evaluate(() => {
