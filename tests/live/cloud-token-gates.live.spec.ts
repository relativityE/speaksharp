...
test.describe.serial('Live Cloud token abuse gates @live', () => {
  ...
  test('allows subscribed Pro and denies Free, Private-sample, and over-quota Pro before minting AssemblyAI token', async () => {
    ...
    expect(
      freeResult.body?.error,
      JSON.stringify(evidence),
    ).toMatch(/Cloud STT is (?:a paid Early Access feature|available (?:with Pro|as a Pro feature)|a Pro feature)|pro trial or subscription required/i);
