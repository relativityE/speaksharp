...
**Content-free contract (privacy).** Redaction is layered: (1) emitter allowlists (`practiceTelemetry.ts`, `privateSampleTelemetry.ts`) project every payload down to enumerated non-PII primitives — transcript, audio, email, free-form title/agenda, and raw identifiers are dropped before send; (2) at the `AnalyticsBuffer` send boundary, general events have any `transcript`/`audio`/`wav`/`blob`/`base64`-shaped value redacted, and **Private (`private_*`) events are re-projected through the Private allowlist a SECOND time** (`sanitizePrivateSampleProps`), so a Private event can never carry a non-allowlisted field even if a caller bypassed the emitter.
