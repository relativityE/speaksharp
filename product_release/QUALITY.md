...
**Private engineering signals** (`privateTelemetry.ts`, allowlisted): `private_setup_started` → `private_setup_succeeded` / `private_setup_failed`, plus bounded `private_error`. These stay strictly content-free (mechanism/enum props only); the allowlist drops any title/description/transcript/audio/PII by construction. Recording completion remains represented by the content-free Practice/session funnel rather than sample-specific lifecycle events.

**Report Issue — the one explicit user-submitted support-content exception** (`reportIssueTelemetry.ts`, #1294): when a user opens "Report an issue" and submits, `report_issue_submitted` is emitted through a **dedicated, narrowly-allowlisted** emitter (NOT the passive Private allowlist). It carries the user's typed **title** and **description**, and — **only when the user explicitly ticks the transcript option and supplies a nonblank snippet** — a bounded transcript excerpt. Bounds are re-enforced at the service boundary (title 160 / description 5000 / snippet 4000, trimmed). It **never** carries audio, audio notes, name, email, password/credential/token material, raw user id, the full transcript, a raw URL/query/fragment, or unrelated session content. It is emitted **only after** the canonical `user_issue_reports` insert succeeds, and a telemetry error can never mask a persisted report. This is a deliberate, user-initiated disclosure of support content — distinct from, and never a widening of, passive content-free telemetry.
