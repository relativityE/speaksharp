...
- **Gate 4 (SCA / dependency review):** `pnpm audit --audit-level critical` + Actions/runtime warning review. A critical runtime exploit with a safe fix blocks release; non-critical advisories/deprecations are P2 unless they break CI/deploy or expose secrets. (Current SCA exception + the pnpm-audit-endpoint breakage are documented in `OPERATIONS_AND_SECURITY.md`.)
