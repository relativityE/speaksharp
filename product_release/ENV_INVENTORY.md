...
| `CLOUD_STT_ENABLED` | Supabase Edge secret | **#1120 S1 backend Cloud kill-switch (fail-closed). Default OFF.** `assemblyai-token` returns `503` (before any JWT auth, Supabase access, or AssemblyAI token/provider call) unless this === `"true"`. Mirrored by the canonical client gate `VITE_CLOUD_STT_ENABLED` (exact-`"true"`/fail-closed); there is NO PostHog `cloud_stt_enabled` flag — Cloud is build/release-gated, not cohort-gated. A stale/direct client cannot mint a paid provider token while Cloud is globally off. Launch invariant: Cloud stays default-disabled unless separately authorized. | product-ops |
...
| `stt_private_primary_v1` (PostHog flag) | PostHog runtime flag (project 207400) — **not** an env var | `frontend/src/config/sttHierarchyFlags.ts` (`STT_HIERARCHY_FLAG_KEY`, `isPrivatePrimaryEnabled` → `readFlag`) | #1120 S1 primary control for the Private-primary / Browser-fallback ORDERING **only**. Parser: `posthog.isFeatureEnabled(key) === true` (exact boolean `true`; missing/unresolved/SSR/throw → OFF, fail-closed). HARD-disabled by build env `VITE_STT_PRIVATE_PRIMARY_DISABLED === 'true'` (overrides the flag). Default **OFF** (Browser-default ordering = kill switch). Does **NOT** gate Cloud: Cloud is the independent `VITE_CLOUD_STT_ENABLED` (client) / `CLOUD_STT_ENABLED` (Edge) gate, and this flag never restores Cloud in either state. |
