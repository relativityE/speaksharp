...
**Private-only entitlement gate.** A live trial and a paid account both resolve to effective `pro`, but that must NOT inherit the old Browser/Cloud/Native allowances. The enforcement migration restricts `tier_configs.pro.allowed_engines` to the **Private** engine and its on-device variants only; `update_user_usage` gates recording on that list. The customer path negotiates the `private` token — proven to succeed while Browser, Cloud, and caller-asserted Native are rejected for both trial and paid accounts (`tests/db/trial-journey.integration.test.ts`). `native` remains solely an internal deterministic E2E hook, never a customer entitlement.
