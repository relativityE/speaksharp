...
**Migration/Edge sequencing safety (DATABASE-FIRST split).** Because the merge redeploys `stripe-webhook` but NOT the migration, the new Edge webhook (which calls the 7-arg `process_stripe_webhook_event` with `p_event_created` and the `lapse_pro`/`renew_pro` actions) must never reach production before its database capability exists — otherwise it would fail (`PGRST202` / "Unknown action") and break live Stripe webhooks. The webhook lifecycle RPC is therefore delivered as a **separate DB-only prerequisite PR (#1287, under #1266)** that is **merged, applied, and verified FIRST**. That migration keeps a **6-arg shim** so the pre-#1282 Edge stays compatible with the new DB (old Edge + new DB), and its 7-arg form takes **no argument defaults** so a 6-arg call is never ambiguous. Only after the database capability is verified live is #1282's new Edge deployed (new Edge + new DB). There is **no runtime fallback** — the earlier compat-fallback was removed because, before the migration, it silently downgraded a recoverable lapse and no-op'd renewal (200-acked), which could leave a paying customer locked out. Both compatibility directions are proven by `tests/db/webhook-lifecycle.integration.test.ts` (in #1287).
