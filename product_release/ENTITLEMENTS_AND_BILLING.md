...
**Migration/Edge sequencing safety (deployment-order defect fix):** because the merge redeploys the Edge webhook but NOT the migration, the deployed `stripe-webhook` could otherwise call the new `process_stripe_webhook_event` contract (7-arg `p_event_created`, and the `lapse_pro`/`renew_pro` actions) against a production DB that still has the pre-#1282 6-arg function — failing with `PGRST202` / "Unknown action" and breaking live Stripe webhooks. `stripe-webhook` is therefore **backward-compatible**: `processWebhookEventCompat` tries the full new contract first and, ONLY on those exact backward-incompatibility errors, falls back to the pre-#1282 6-arg contract with a legacy action mapping (`lapse_pro→downgrade_to_free`, `renew_pro→none`). This reproduces current production behavior until the migration lands (no regression), after which the primary call succeeds and full #1282 behavior applies. **The Edge deploy is thus safe regardless of migration timing** (proven by `stripe-webhook/index.test.ts`); applying the migration first is still the preferred order, but no longer a correctness prerequisite for the deploy.
