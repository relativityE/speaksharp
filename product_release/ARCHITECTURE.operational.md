...
- The protected admin page is implemented as the in-app route `/admin/ops-status`. Its sole production gate is HTTP Basic auth at the Vercel edge (`middleware.js`, env `OPS_STATUS_PASSWORD`); it reads the published summary JSON, not vendor checks from the browser. It is intentionally **not** behind the app's `InternalRoute` guard (PR #962) — routing it through `InternalRoute` would require `VITE_ENABLE_INTERNAL_ROUTES` in production, which would also expose `/design`. `/design` stays behind `InternalRoute`.
