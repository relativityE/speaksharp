...
- **Built-in allowlist:** the active production host (`https://speaksharp-public.vercel.app`) and exact local-dev origins (`http://localhost:5173/5174`, `http://127.0.0.1:5173/5174`). No arbitrary localhost ports, no localhost subdomains, no `*.vercel.app`. Any genuinely-owned additional production origin is configured via the `ALLOWED_ORIGIN` env (exact value) — the built-in set never carries an unaffiliated domain (#1260).
