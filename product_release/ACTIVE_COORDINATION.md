...
|---|---|---|---|---|---|---|
| 2026-06-07T00:36Z | CI-MAIN | P0 | test-release-agent | **PASS / latest main** | GitHub `CI - Test Audit` run `27077989733` passed on `main@c25e87d0` (prepare, build, unit coverage, unit shards 1-4, edge, health, Lighthouse advisory, E2E shards 1-4, report all success). Production Canary `27077989725` and Deploy Supabase `27077989736` also passed on the same SHA. | `=> test` |
...
| 2026-06-07T00:35Z | SPINE-GUARDS | P1 | dev-agent -> test-release-agent | **LANDED on main — verify CI then remove** | Release-spine invariants locked in tests (test-only, no runtime change): `frontend/src/services/transcription/__tests__/releaseSpineGuards.test.ts` (9 tests) — tiny default; base opt-in; base consent ~80 MB; v4 engine key never default/candidate; CANDIDATES has no onnx-community (v3/v4) ids; v4/unknown Private-flag requests reject (no silent fallback). FF-landed to `main` (guard commit `3bb775e8`, now an ancestor of current `main`). Verified locally 9/9 + transcription 121/121 + tsc/eslint clean. **Ask:** confirm CI green on current main, then remove this row. | `=> test` |
| 2026-06-07T00:35Z | STT-EVIDENCE-SCHEMA | P1 | dev-agent -> test-release-agent | **scoped read-only instrumentation on branch — needs harness-alignment review** | Pure module `dev/stt-evidence-schema@af997dc2` (local; NO behavior/default/transcript/UX change; not imported by runtime). `frontend/src/services/transcription/sttEvidence.ts`: `SttEvidence` schema (tier; verdict; engine: provider/modelId/modelSource/dtype/requestedDevice/resolvedDevice/fallbackOccurred/runtimeVersion; audioValidity: fixtureId/audioDurationSec/audioDelivered/processAudioReadyCount/speechExpected/speechDetected/rmsMax/peakMax; **separate** timing: modelLoadMs/firstTextMs/decodeMs/finalizeMs/stopFinalizationMs/saveMs/detailHydrationMs/rtf; output: transcriptLength/nonEmptyTranscript/wer/repetitionRisk; error: errorClass/rawErrorDevOnly). `classifySttRun()` precedence NOT_TESTED < BLOCKED(env) < INVALID(audio/engine) < FAIL(empty/error) < PASS — so a bad INPUT is INVALID/BLOCKED, never a model FAIL. `buildSttEvidence()` fills `NOT_AVAILABLE`, derives rtf + nonEmptyTranscript, gates wer on referenceText. invalidReason codes MIRROR `scripts/private-corpus-acceptance.mjs` and extend it (blocked_env_invalid, invalid_worker_not_started, invalid_model_not_loaded, fail_no_transcript, fail_error). 17/17 tests, tsc + eslint clean. **Ask (harness alignment):** confirm field names + reason codes match how the harness collects per-tier signals, and tell dev which existing globals a read-only `window.__STT_EVIDENCE__()` collector should aggregate (step 2). **Land rule:** dev merges to main on your ack (additive, no runtime), or renames fields first. | `=> test` |
...
| 2026-06-07T00:06Z | PROD-CONFIG-1 | P0 | product-ops -> test-release-agent | **live proof found production Stripe key is TEST** | Live browser proof against `https://speaksharp-public.vercel.app/` and `/pricing`: `window.__APP_RUNTIME_CONFIG__` is present and reports real Supabase (`mockAuth=false`, `releaseProofEligible=true`), `release="adfe1d5fc4a0a63db5e5c726285c827fd0240135"`, and `stripeKeyClass="test"`. Public home/pricing pages did not expose checkout/Stripe/subscribe buttons; `/pricing` showed Pro plan copy plus only a `Start Free` button. **Required product-ops action before launch/payment exposure:** set live Stripe publishable key or keep payment surfaces hidden. After deploy, test rechecks `stripeKeyClass === "live"` for payment launch, or confirms non-live key still hides checkout. | `=> product-ops/test` |
