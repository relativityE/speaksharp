...
| 2026-06-06T23:30Z | UX-NAV-1 | P1 | dev-agent -> test-release-agent | **branch stale; rebase before any proof or merge** | **Root cause:** in-app nav already saves because it `await`s `stopRecording()` before navigating (App.tsx click guard); hard nav (URL bar/refresh/tab close) can't — `beforeunload` cannot await the async Private stop→decode→save, and the existing `sessionRecoveryDraft` was only written inside the stop/save flow (never during active recording), so the partial was lost. **Implemented branch:** `dev/ux-nav-1-draft-on-unload@b0663e1a` adds `SpeechRuntimeController.persistActiveRecoveryDraft()` and flushes it on `pagehide`/`beforeunload`. **New code-review finding:** branch is stale relative to latest `main@1475e52e` and must NOT merge as-is. `git diff main..dev/ux-nav-1-draft-on-unload` shows it would revert the current proof-gate hardening in `scripts/manual-stt-corpus-proof.mjs`, `scripts/manual-native-chrome-proof.mjs`, `scripts/private-corpus-acceptance.mjs`, and `tests/release/private-corpus-acceptance.test.ts` by removing `missing_supabase_*` invalid reasons and loosening the current `audio chunks + process_audio_ready` validity gate. **Required next step:** rebase or recreate UX-NAV branch on latest main, preserve all STT-HARNESS-INVALID-AUDIO changes, then rerun the acceptance battery once in real-auth 5174 Private: (1) URL-bar/refresh mid-recording with partial present → return to /session shows "Recovered unsaved session draft"; (2) tab close + reopen → same; (3) in-app nav with confirm → still stops+saves; (4) normal stop+save → no leftover draft / no false recovery banner; (5) no React max-depth during hard nav; (6) UX judgment: recovered text is labeled a draft to review, not a silently-saved session. **Land rule:** only merge after the branch is rebased cleanly and PASS of (1)(2)(3)(4)(5). | `=> dev` |
