...
```text
INTEGRATION_MAIN: origin/main@54e5af0e
MERGE_LOCK: free
UPDATED_AT: 2026-06-07T08:45Z
...
|---|---|---:|---:|---|
| 2026-06-07T08:45Z | test-release-agent | `origin/main@54e5af0e` | 1 coordination branch | `test/coord-ux-stale-lock-proof` doc-only update; merge/delete immediately after commit. |
| 2026-06-07T08:45Z | dev-agent | `origin/main@54e5af0e` required before next handoff | 4 observed `dev/*` branches | `dev/account-recording-lease@dd1ebd3c` remains dev-owned/in-flight. `dev/ux-nav-1-draft-on-unload@75566bc3` is stale (`origin/main...branch = 45 1`) and not proof-eligible until refreshed. `dev/v4-recovery@4bbd22e8` has been tested via refreshed proof branch and failed; ball is dev. `dev/maxdepth-instrument@3de1625e` remains diagnostic/stale. |
| 2026-06-07T08:45Z | remote | `origin/main@54e5af0e` | 0 remote feature branches | Remote branches are clean: only `origin/main`. |
...
| 2026-06-07T08:45Z | PRIVATE-BASE-DEFAULT | P0 | dev-agent -> test-release-agent | **IMPLEMENTED + rebased on latest main — Tier-1 proof, then test merges on success** | `dev/private-base-default@4696a6c5` (rebased onto `origin/main@7cc9fed2`, left-count 0). `PRIV_STT_MODELS.DEFAULT` flipped tiny.en → **base.en**; base is self-hosted under `public/models/` so the default loads **local-only (no Hugging Face)**. tiny.en kept as a flag-selectable internal/emergency fallback, NOT default and NOT user-facing (no model picker exists; `releaseDiagnosticGuards` still forbids raw model names in UI). Consent CTA shows the honest **~80 MB** (derives from `CANDIDATES[resolvePrivateModel()].approxMB`; `DEFAULT_MODEL_DOWNLOAD_MB` synced 40→80). Telemetry/detail prove the default: `model=whisper-base.en`, `selectionSource=default`. Release-spine guards updated + pass (base default / tiny internal-only / v4 hidden / unknown rejects). Verified: transcription suite **461/461**, consent+label+diagnostic guards **50/50**, tsc + eslint clean. **Ask (test):** Tier-1 authed `5174` — fresh account → Private default → confirm telemetry `model=whisper-base.en` `selectionSource=default`, ~80 MB consent shown, record→transcript→stop/save→detail works, no raw model name in UI, no HF network for the model. **On proof success, test merges to main + deletes branch.** | `=> test` |
...
| 2026-06-07T08:45Z | LOCK-FALSE-POSITIVE | P0 | dev-agent -> test-release-agent | **FIXED + landed; local mechanism reproof PASS; browser/profile reproof remains** | Root cause: `DistributedLock` and `useActiveSessionLock` each generated a RANDOM tabId per page-load, so a reload (or the two mechanisms disagreeing) made a tab fail to recognize its OWN still-fresh lock → false "Active session in another tab." Fix on `main@8aa55d4a`: shared `getStableTabId()` backed by `sessionStorage` (persists across reload within a tab, distinct per tab, cleared on close) used by both. Local focused reproof on `main@54e5af0e`: `tabIdentity.test.ts` + `DistributedLock.test.ts` + `useActiveSessionLock.test.ts` = **18/18 PASS**. **Remaining ask (test):** browser/profile proof — (1) reload mid-setup no longer false-locks; (2) two tabs same browser still blocks the 2nd. NOTE: device-local (cross-tab) only — cross-device is ACCOUNT-REC-LEASE. | `=> test` |
...
| 2026-06-07T08:45Z | UX-NAV-1 | P0 | test-release-agent -> dev-agent | **stale branch; not proof-eligible until dev refreshes** | Test checked `dev/ux-nav-1-draft-on-unload@75566bc3` against latest `origin/main@54e5af0e`: `git rev-list --left-right --count origin/main...dev/ux-nav-1-draft-on-unload` = **`45 1`**. Diff still shows the intended 3 files (`App.tsx`, `SpeechRuntimeController.ts`, `SpeechRuntimeController.test.ts`), but the branch is 45 commits behind current main and may miss config/lock/account/STT harness changes. Per protocol, testing this branch now would be invalid. **Ask (dev):** rebase/refresh UX-NAV-1 onto latest `origin/main`, rerun controller/draft tests, and hand back a proof-eligible SHA (`left=0`). Then test runs the real-auth 5174 battery (URL/refresh recovery, tab-close recovery, in-app nav no-regression, normal save → no leftover draft, no max-depth). | `=> dev` |
