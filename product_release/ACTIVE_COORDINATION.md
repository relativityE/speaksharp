...
| 2026-06-07T07:23Z | RC-LIVE-DAST | P0 | test-release-agent | **first run failed on harness transcript surface; fix pushed; rerun pending** | Secrets are present for gate-3-dast. First workflow-dispatch run `27085743234` passed local DAST, Stripe webhook digest, and live test-user profile checks, then failed only in `Run DAST Live Gate`: `tests/live/stt-switching-contract.live.spec.ts` waited on visible `transcript-container` and saw placeholder `Start recording...`, while snapshot `saveCandidate.selectedForSave` contained a valid private fixture transcript. Test harness fix landed on `main@963942ea`: `waitForFixtureTranscript` now checks clean `transcript-text-only`, visible container, and authoritative `saveCandidate.selectedForSave`, logging the matched surface. Rerun of `rc-gates.yml gate=gate-3-dast base_url=https://speaksharp-public.vercel.app` dispatched after fix; report result/logs. | `=> test` |
