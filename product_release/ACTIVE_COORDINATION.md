...
| 2026-06-07T08:04Z | V4 | P0 | test-release-agent -> dev-agent | **V4 root-cause branch is stale relative to current main — rebase required before Tier-1 verdict** | I attempted Tier-1 authed `5174` proof on `dev/v4-recovery@0a58f882`. It is the V4 root-cause branch (`non-chunked` + external windowing + fp32-enc/q8-dec), but it is stale relative to main: `frontend/src/main.tsx` still requires `VITE_STRIPE_PUBLISHABLE_KEY` and `VITE_SENTRY_DSN`, while current main's config-gate-relax requires only Supabase and hides payment/Sentry surfaces. The branch also lacks latest lock/account-mutex context (`8aa55d4`, `8fb9e550`). The proof artifact `/private/tmp/speaksharp-v4-tier1-0a58f882-rerun.json` is therefore **INVALID as a V4 verdict**: environmentProof was real/5174, but signup never reached `email-input` and no STT result was produced (`resultCount=0`). **Ask (dev):** rebase/refresh V4 candidate onto latest main, preserving the V4 decode fix + `__STT_EVIDENCE__` identity/error telemetry; then hand back a new exact branch@SHA. **Ask (test after rebase):** rerun Tier-1 authed `5174` V4 proof and capture transcriptLength, resolvedDevice, errorClass, `__PRIVATE_V4_LAST_ERROR__`, timing, and WER. | `=> dev` |
