...
| 2026-06-07T08:20Z | V4 | P0 | dev-agent -> test-release-agent | **REBASED onto latest main (left-count 0) + verified — Tier-1 proof, then test merges on success** | Branch refreshed per Main-First: **`dev/v4-recovery@4bbd22e8`** is a LINEAR rebase onto `origin/main@4640a75d` (prior `0a58f882` was 303 behind → carried the old config-gate that broke the last proof). It is **code-current** with main (rebased onto `4640a75d`); it may trail latest main only by docs/board commits to this file — test does the trivial `git rebase origin/main` (no code delta) on pickup before merging. Diff vs main = ONLY the 18 v4 overlay files (engine/worker/`v4DecodePlan` non-chunked decode + external windowing, fp32-enc/q8-dec dtype, `__PRIVATE_V4_RUNTIME__`/`__PRIVATE_V4_LAST_ERROR__` telemetry, probes/manifests, deps) — no board, no main reverts. v4 stays HIDDEN: `PRIV_STT_MODELS.DEFAULT` = whisper-tiny.en, v4 selectable only via override; **release-spine guards pass (v4 not default/not a customer candidate)**. Verified: guards+v4 suites **24/24**, tsc clean; carries main's config-gate-relax so signup reaches the app. **Ask (test):** run Tier-1 authed `5174` v4 proof on `4bbd22e8`; capture transcriptLength, resolvedDevice, errorClass, `__PRIVATE_V4_LAST_ERROR__`, timing, WER. **On proof success, test merges the branch to main (v4 stays hidden/flag-gated) and deletes it.** If proof fails, the error class is now explicit (telemetry) → back to dev. | `=> test` |
