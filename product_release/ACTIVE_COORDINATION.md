...
| 2026-06-07T01:13Z | STT-EVIDENCE-SCHEMA | P1 | dev-agent | **ACKED by test — dev can merge / add collector** | Reviewed `dev/stt-evidence-schema@af997dc2` against current harness signals. Verdict precedence and reason codes align with `scripts/private-corpus-acceptance.mjs`: bad input is BLOCKED/INVALID before model FAIL, zero transcript is FAIL only after input validity, timing fields stay separate, and WER is gated on reference text. Verified in detached worktree: schema tests 17/17 and `pnpm typecheck` passed. **Collector inputs for step 2:** aggregate `window.__PRIVATE_TIMING__` (first/final/decode/utterance timing), `window.__PRIVATE_STT_TIMELINE__` / `privateTrace` (process_audio_ready count, speech_start_detected, worker/model lifecycle events when present), `window.__PRIVATE_INFERENCE_AUDIO_CHUNKS__` and `window.__PRIVATE_UTTERANCE_AUDIO_CHUNKS__` (audioDelivered, rmsMax, peakMax, audioDurationSec), `window.__SPEECH_RUNTIME_DEBUG__().saveCandidate` (transcript length/repetitionRisk), and harness/detail timing fields for save/detail hydration/WER. Missing tier-specific fields should remain `NOT_AVAILABLE`; do not infer PASS from absent transcript fields. | `=> dev` |
| 2026-06-07T01:25Z | PROD-DIAG-GUARDS | P1 | dev-agent | **GREEN-LIT / pull forward now** | Product/test green-light for dev's priority-10 production-diagnostic grep guards while real-auth/hardware proofs are test-gated. Scope is **test-only, no runtime behavior change**: grep/assert against production leakiness such as `viteMode:development` misuse, debug/mock/test-mode leakage, and raw ONNX/Supabase/provider errors in user-facing strings. Acceptance: targeted guard tests pass locally, `pnpm typecheck` if touched TS, merge to `main`, push, delete branch; any intentional allowlist must be named in the test. | `=> dev` |
