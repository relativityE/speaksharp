...
- **Private-First Hierarchy**: Private is the recommended transcription mode and the main beta experience. Browser is a free "Quick preview" convenience path, NOT an equivalent to Private. Cloud is a Pro feature and is unavailable during the no-billing beta.
- **STT Mode Consent**: Private STT MUST NOT silently switch to Cloud STT. Cloud is a first-class Pro choice, but it requires explicit user selection and Pro entitlement; during the no-billing beta the Cloud option is not available.
