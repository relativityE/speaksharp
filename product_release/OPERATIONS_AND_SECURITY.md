...
| `ASSEMBLYAI_API_KEY` | C — Ops-managed in Supabase; NOT synced from GitHub | assemblyai-token (Cloud STT) |
...
|---|---|---|---|
| `VITE_NATIVE_PUNCTUATION_RESTORE` | A/B | `.../modes/nativePunctuationRestore.ts` | Default **true**; only `'false'` disables word-preserving punctuation restore for saved Native transcripts. |
...
**Recommended product-ops sequence:** (1) preserve old key **fingerprints, not full secrets**; (2) rotate Supabase first if the committed service-role value was real (regenerate JWT secret → update all GitHub/Vercel/Supabase secrets using anon/service-role → redeploy/re-run auth, canary, live DAST, Edge checks); (3) rotate Stripe (new live secret key → rotate webhook endpoint secret → update secrets → one controlled checkout/webhook entitlement proof only when payments are intended live); (4) rotate AssemblyAI (new key → update secrets → Cloud STT token/provider smoke); (5) reset any real test accounts/passwords found in committed env files; (6) decide history handling (fake → record proof, no purge; any real → secret-scanning incident record + history purge).

**Verification after rotation:** run `pnpm rc:sast:secrets`, `pnpm test:edge`, `pnpm run rc:dast:live`; verify `git ls-files` excludes `.env.test`/`frontend/.env.test`; GitHub secret update timestamps are newer than the cleanup commit; Vercel prod reports real Supabase auth and no mock/test mode; payment surfaces remain hidden unless the publishable key is live; Cloud STT token smoke succeeds only with the rotated AssemblyAI key. **Ownership:** product-ops owns provider credential rotation; dev/test own repo-side scans, app verification, and proof artifacts; agents must not paste provider secret values into coordination files, logs, or commits.

---

## 4. Live-value verification protocol

The launch checklist verifies each variable against the **live production environment** (runtime truth is the primary release gate). Verify each key in the Supabase/Stripe/Vercel dashboards; invoke `check-usage-limit` via `curl` against the production URL for connectivity; review Supabase Logs for env-load errors after deployment. Key live checks: `window.__APP_RUNTIME_CONFIG__.stripeKeyClass === "live"` (any other value blocks launch — `"test"`=test key in prod, `"missing"`=Vercel override absent, `"unknown"`=malformed); `window.__APP_RUNTIME_CONFIG__.release` equals the deployed git SHA (`"unknown"`/absent = build received no SHA, breaking bug-report/log correlation); production build has source maps disabled, no auth bypass, `VITE_ENABLE_INTERNAL_ROUTES` absent/false, and no dev/test `VITE_*` present; AssemblyAI token denial returns 401 (unauth), grant returns a short-lived token (in-limit Pro), over-limit returns 403; live entitlement functions fall back to `free` for null/unknown statuses and a fresh Free profile gets one server-backed Private sample without Cloud. Preview deployments must not be shared as tester/public URLs unless explicitly reviewed.
...
- **Rate limiting & transport.** `rate-limiter` config set to production values (e.g. 100/min per IP); production domain has a valid active TLS certificate.
...
## 8. SCA / dependency-audit exceptions

Documented, justified suppressions for `pnpm audit` (the `rc:gate:4:sca` gate runs `pnpm audit --audit-level critical`). Each entry states the advisory, why it is not reachable, the compensating control, and the real remediation path. Suppressions live in `package.json` → `pnpm.auditConfig.ignoreGhsas`.
...
**Durable finding — the SCA gate command is currently broken (route to `ROADMAP.md`).** `pnpm audit` (pinned pnpm 10.29.1) calls the retired legacy npm audit endpoint (`registry.npmjs.org/-/npm/v1/security/audits`) which now returns HTTP 410, so `pnpm audit --audit-level critical` errors/exits 1 in CI and cannot produce a valid pass/fail. An endpoint-independent `osv-scanner` cross-check over the root `pnpm-lock.yaml` proves the **only** critical is `vitest@3.2.4` → GHSA-5xrq (the already-ignored advisory; the historical "2 critical" count is a single-advisory duplicate-path artifact of `vitest` resolving into two importers) — i.e. **zero unignored distinct criticals**. Required remediation (two items, both durable, routed to `ROADMAP.md`): (1) un-break the gate — move `rc:gate:4:sca` off the retired endpoint (a pnpm that uses the bulk advisory endpoint, or `osv-scanner` / GitHub Dependency Review); (2) retire the suppression by upgrading vitest to `>= 4.1.0`. Dated audit-run evidence is indexed by `EVIDENCE_INDEX.md`.
