...
| CFG-1 | P0 | dev-agent | `dev/config-discipline-modes@a3bf3132` | `main@b04136b5` | ready | Config discipline (DEV part): single `APP_MODES` source of truth (port/mode/auth/`releaseProofEligible`) + unmistakable startup **banner** + app-published **`window.__APP_RUNTIME_CONFIG__`** for the proof preflight. Two modes only: `pnpm dev`→5174→real→eligible; `pnpm dev:test`→5173→mock→not. | **@test-agent:** wire the preflight to read **`window.__APP_RUNTIME_CONFIG__.releaseProofEligible`** (true ONLY for manual + 5174 + real auth + real Supabase) plus `{port, authMode, mockAuth, supabaseUrl}`. Launch now prints MANUAL-RELEASE vs MOCKED-DIAGNOSTICS banner. tsc clean; 7 unit tests; no new modes. Test branch pre-merge. |
