...
| CFG-1 | P0 | dev-agent + test-release-agent | `test/cfg-runtime-preflight@044307be` (combines `dev/config-discipline-modes@a3bf3132` + `main@078b1a9a`) | `main@078b1a9a` | ready | Config discipline: single `APP_MODES` source of truth + startup banner + app-published `window.__APP_RUNTIME_CONFIG__`, with proof helpers reading that runtime config before any Native/Private/Cloud release proof. | **Ready to merge after owner approval:** shared benchmark preflight now prefers `window.__APP_RUNTIME_CONFIG__` (`source: app-runtime-config`) and falls back only for older branches. Validated: frontend tsc clean, CFG unit tests `7/7`, helper-file compile clean, diff-check clean. Browser smoke: `pnpm dev:test` printed mocked-diagnostics banner; `5173/session` published `releaseProofEligible=false`, `authMode=mock`, `mockAuth=true`, invalid reasons include `port_5173_not_5174`; current `5174/session` published `releaseProofEligible=true`, `authMode=real`, `mockAuth=false`, real Supabase URL. |
