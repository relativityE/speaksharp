...
|---|---|---|---|---|---|---|---|
| STT-P5 | P0 | dev-agent | `dev/private-vad-silero-engine@10ba21ca` | `01933f91` | static-ready / browser asset blocked | VAD engine: Silero VAD via the **existing onnxruntime-web** (NO @ricky0123/vad-web → no duplicate ORT) injected at the `PrivateWhisper` **onset** gate behind `?privateVad=1` / `window.__PRIVATE_VAD_PROTOTYPE__`. Off by default. | Test-agent prep rerun: focused P5 tests pass (`PrivateWhisper` + `privateVadFlag` = 49/49) and frontend typecheck passes. Browser A/B is still blocked by missing `frontend/public/models/silero_vad.onnx`; current `frontend/public/models/` only has Whisper assets. Once the Silero asset is staged, run RMS-vs-VAD A/B from PRIVATE_STT report. Do not merge until browser A/B + product approval. |
| STT-P6 | P0 | dev-agent | `dev/private-model-eval@25cde3b4` | `d910a07d` | static-ready / needs current-main browser A/B | Model-eval: flag-selected Private model (`whisper-tiny.en` default / `whisper-base.en` / `distil-small.en`) threaded through worker `init`, behind `?privateModel=` / `window.__PRIVATE_MODEL__`. Off by default. Directly attacks the CI **51.7% WER** decode/model-quality gap. | Test-agent prep rerun: focused P6 tests pass (`privateModelFlag`, `PrivateSTT`, `PrivateWhisper` = 63/63) and frontend typecheck passes. Branch is behind current main; use a current-main test branch/rebase for browser A/B. Capture `__PRIVATE_MODEL_TELEMETRY__`, WER/accuracy, download MB, load time, decode latency/RTF, app-vs-drop-in, and app-vs-Native-baseline. Do not merge until A/B + product approval. |
