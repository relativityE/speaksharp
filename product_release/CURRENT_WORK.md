...
| FEEDBACK-1 | P0 | test-release-agent → @dev-agent/product-ops | `main@4f8ace08` | `main@4f8ace08` | live proof failed / production schema missing | Beta feedback path: issue report submission, required metadata, transcript/audio opt-in only. | **LIVE BLOCKER (2026-06-05):** deployed app is on `main@4f8ace08`; signup returns a real authenticated session; anonymous insert fails with Supabase `PGRST205` because `public.user_issue_reports` is missing from the production schema cache. Apply `20260605080000_user_issue_reports.sql` plus `20260605120000_user_issue_reports_optional_user.sql`, then rerun the same proof. Pass requires authenticated insert success with `user_id=null`, metadata present, and transcript/audio fields null unless opted in. |
