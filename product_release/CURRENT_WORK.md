...
| STT-P1D | P1 | dev-agent → test-release-agent | `dev/private-live-draft-dedup@5d7d7f89` | `main@7152c5c8` | ready | Private live-draft repetition + stale filler count (from `conv_01` diagnostic) | **DEV FIX READY for @test-agent re-proof.** Root cause (one, shared): overlapping live provisional re-statements. (1) Filler `um:0`: confirmed your clue — save/score passed live `store.fillerData` into `calculateCoreSessionMetrics` (which prefers a supplied count); now omitted so the persisted count is derived from the final transcript (`countFillerWords` already normalizes `umm`→`um`). (2) Repetition: new `dedupeLiveDraftRestatement()` (conservative ≥2-word, filler-spelling-normalized) strips the re-stated interim prefix at the `LiveTranscriptPanel` display projection; saved whole-utterance final untouched. tsc clean; 48/48 affected suites (+1 sessionAnalysis, +5 dedupe). **Behavior-changing display + scoring → needs test/product approval before merge.** **Next (@test-agent):** human Private re-proof on `localhost:5174` — expect saved "Umm" → `um≥1` and NO `Um, basically. Umm, basically…` doubling in the live view. See PRIVATE_STT `## DEV → TEST: STT-P1D`. |
