...
|---|---|---|---|---|---|---|---|
| STT-P5 | P0 | dev-agent → test-release-agent | `dev/private-vad-silero-main@52cc2034` (rebased onto current main) | `main@f9ddf497` | ready / asset + dep staged | VAD engine: Silero VAD via the **existing onnxruntime-web** (NO @ricky0123/vad-web → no duplicate ORT) injected at the `PrivateWhisper` **onset** gate behind `?privateVad=1` / `window.__PRIVATE_VAD_PROTOTYPE__`. Off by default. | **DEV DONE — ready for @test-agent RMS-vs-VAD browser A/B.** Rebased onto current main; staged `frontend/public/models/silero_vad.onnx` (Silero v5, 2,327,524 B, sha256 `1a153a22…`, from official snakers4/silero-vad; I/O verified to match `sileroVad.ts`: input/state/sr → output/stateN, state `[2,1,128]`, sr int64). **`onnxruntime-web@1.14.0` now declared as an explicit direct dependency** in the ROOT `package.json` (the real install source — `frontend/package.json` is the minimal/non-installed one) — pinned to the exact version `@xenova/transformers@2.17.2` already resolves, so **no duplicate ORT** (lockfile-only sync; 3-line lockfile diff, version unchanged). tsc clean; 49/49 VAD tests. Run the RMS-vs-VAD A/B from PRIVATE_STT `## DEV → TEST: STT-P5`. Do not merge until browser A/B + product approval. |
