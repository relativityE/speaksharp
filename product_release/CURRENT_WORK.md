...
| STT-P6-HUMAN | P0 | test-release-agent → dev-agent/product | `dev/private-model-eval-rc@d4558159`; tested as `test/stt-p6-human-rc` | `main@6458c362` | partial / selector honored but detail label wrong | Human Private model proof requested to back bigger-model accuracy claims. | **LATEST TEST RESULT (2026-06-05):** focused selector tests pass `53/53`. Browser proof with `?privateModel=whisper-base.en` now honors the selector in runtime telemetry: `privateModelTelemetry.model=whisper-base.en`, `overridden=true`, `selectionSource=window`, and h1_6 transcript is `100%` accurate. **But detail still displays `Recorded with Private (whisper-tiny.en, transformers-js, browser)`**, so persisted/displayed model identity is wrong. Evidence `product_release/evidence/stt_p6_human_rc_base_selector_2026-06-05.json`; raw `/private/tmp/stt-p6-human-rc-base-h1_6.json`. **Next @dev-agent:** persist/display selected private model identity across detail/history/export/analytics; do not green human/base proof until detail and telemetry agree. |
...
| RC-LH-1 | P1 | test-release-agent → product/dev | `dev/rc-gates-fix@2f58eacb`; tested as `test/rc-lh-2f58eacb` | `main@4aa6f213` | invalid local proof / needs normal-terminal or CI rerun | Product gate Lighthouse / product CI proof. | **LATEST TEST RESULT (2026-06-05):** `pnpm run rc:gate:1:product` reached the fixed path: banned-directive scanner passed, code quality passed, Playwright E2E passed `33/33`, Lighthouse scored `98/100` performance and `100/100` accessibility/best-practices/SEO, quality score `97/100`. **But the generated audit is `INVALID / NOT EVIDENCE`** because the sandbox blocked preview bind (`listen EPERM: operation not permitted 127.0.0.1:4173`). Evidence `product_release/evidence/rc_lh_reproof_2026-06-05.json`. **Next:** re-run the same gate in a normal terminal or GitHub Actions. Do not count this sandbox artifact as RC evidence. |
