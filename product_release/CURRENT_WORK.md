...
| STT-P7 | P0 | dev-agent | `dev/stt-p7-soft-frozen-trace@30eb264a` (extends `main@d8a3b7d2`) | `main@6eaa4ba6` | ready / needs 5174 branch slot | Private mic-start reliability: human proof hit `RECORDING_LIFECYCLE_FAIL` ~857 ms after `RECORDING` + `Heartbeat Failure Escalated`; user saw mic-click errors/**toasts** before recording worked. | `d8a3b7d2` fixed the **controller** watchdog (hard fail). **DEV extension** fixes the remaining gap: the **`TranscriptionService` soft-"frozen" watchdog** ALSO false-fired on fresh start → the **"Engine Frozen" toast** the user saw. Same bounded-grace logic (now a shared, **tested** `evaluateHeartbeatWatchdog`); real death still trips. **Req 6 trace** added on both watchdogs (`window.__PRIVATE_HEARTBEAT_TRACE__`). Test-agent proof so far: 10/10 watchdog/trace tests + frontend typecheck pass. Browser smoke is **not closed**: first attempt was invalid because it launched direct Vite on 5178 and got blank 404. Current `localhost:5174` is occupied by a different worktree (`dev/config-discipline-modes@a3bf3132`), so it cannot validate this branch. **Next (@test-agent):** get a deliberate 5174 slot serving `dev/stt-p7-soft-frozen-trace`, then run injected Private-start smoke + human re-proof. Expect no `RECORDING_LIFECYCLE_FAIL`, no "Engine Frozen" toast, and `__PRIVATE_HEARTBEAT_TRACE__` source `watchdog-grace` during warmup then `inner-heartbeat`. |
