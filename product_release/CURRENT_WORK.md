...
| STT-P7 | P0 | dev-agent | `dev/stt-p7-soft-frozen-trace@30eb264a` (extends `main@d8a3b7d2`) | `main@6eaa4ba6` | ready | Private mic-start reliability: human proof hit `RECORDING_LIFECYCLE_FAIL` ~857 ms after `RECORDING` + `Heartbeat Failure Escalated`; user saw mic-click errors/**toasts** before recording worked. | `d8a3b7d2` fixed the **controller** watchdog (hard fail). **DEV extension** fixes the remaining gap: the **`TranscriptionService` soft-"frozen" watchdog** ALSO false-fired on fresh start → the **"Engine Frozen" toast** the user saw. Same bounded-grace logic (now a shared, **tested** `evaluateHeartbeatWatchdog`); real death still trips. **Req 6 trace** added on both watchdogs (`window.__PRIVATE_HEARTBEAT_TRACE__`). tsc clean; 6 new + controller `watchdog.test.ts` 4/4. Test-agent re-ran focused unit/type proof on the branch: 10/10 watchdog/trace tests + frontend typecheck pass. Browser smoke is **not closed**: first attempt was invalid because it launched direct Vite on 5178 and got blank 404; valid proof needs the branch served through canonical `pnpm dev` / `localhost:5174`. **Next (@test-agent):** injected Private-start smoke + human re-proof — expect **no `RECORDING_LIFECYCLE_FAIL` and no "Engine Frozen" toast**; trace shows `source: watchdog-grace` during warmup. See PRIVATE_STT `## DEV → TEST: STT-P7 extension`. |
