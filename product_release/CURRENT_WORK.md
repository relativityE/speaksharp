...
| STT-P7 | P0 | dev-agent | `dev/stt-p7-soft-frozen-trace@30eb264a` (extends `main@d8a3b7d2`) | `main@6eaa4ba6` | ready | Private mic-start reliability: human proof hit `RECORDING_LIFECYCLE_FAIL` ~857 ms after `RECORDING` + `Heartbeat Failure Escalated`; user saw mic-click errors/**toasts** before recording worked. | `d8a3b7d2` fixed the **controller** watchdog (hard fail). **DEV extension** fixes the remaining gap: the **`TranscriptionService` soft-"frozen" watchdog** ALSO false-fired on fresh start → the **"Engine Frozen" toast** the user saw. Same bounded-grace logic (now a shared, **tested** `evaluateHeartbeatWatchdog`); real death still trips. **Req 6 trace** added on both watchdogs (`window.__PRIVATE_HEARTBEAT_TRACE__`). tsc clean; 6 new + controller `watchdog.test.ts` 4/4. **Next (@test-agent):** injected Private-start smoke + human re-proof — expect **no `RECORDING_LIFECYCLE_FAIL` and no "Engine Frozen" toast**; trace shows `source: watchdog-grace` during warmup. See PRIVATE_STT `## DEV → TEST: STT-P7 extension`. |
...
| STT-P1D | P1 | test-release-agent / dev-agent | `/private/tmp/private-corpus-conv01-f9b21005.json` | `main@f9b21005` + localhost app | done / actioned | Private injected-mic diagnostic on `conv_01` | **Diagnostic only, not human release proof:** env was valid (`5174`, real auth), save/history/detail passed, first text `2826ms`, final accuracy `85.71%`. New follow-up: @dev-agent owns live-draft repetition and filler normalization (`Umm` should count as user-perceived `um` or lower confidence); @test-agent still owns human re-proof. See PRIVATE_STT latest diagnostic section. |
...
| CFG-1 | P0 | dev-agent + test-release-agent | `test/cfg-runtime-preflight` (combines `dev/config-discipline-modes@a3bf3132` + `main@078b1a9a`) | `main@078b1a9a` | testing | Config discipline: single `APP_MODES` source of truth + startup banner + app-published `window.__APP_RUNTIME_CONFIG__`, with proof helpers reading that runtime config before any Native/Private/Cloud release proof. | **Test wiring complete on branch:** shared benchmark preflight now prefers `window.__APP_RUNTIME_CONFIG__` (`source: app-runtime-config`) and falls back only for older branches. Validated: frontend tsc clean, CFG unit tests `7/7`, helper-file compile clean, diff-check clean. Browser smoke: `pnpm dev:test` printed mocked-diagnostics banner; `http://localhost:5173/session` published `releaseProofEligible=false`, `authMode=mock`, `mockAuth=true`, `source=app-runtime-config`, invalid reasons include `port_5173_not_5174`. Manual `5174` smoke is pending a free port because another app was already running there. |
