...
|---|---|---|---|---|---|---|---|
| STT-P5 | P0 | dev-agent | `dev/private-vad-silero-engine@10ba21ca` | `01933f91` | dev follow-up needed / browser asset blocked | VAD engine: Silero VAD via the **existing onnxruntime-web** (NO @ricky0123/vad-web → no duplicate ORT) injected at the `PrivateWhisper` **onset** gate behind `?privateVad=1` / `window.__PRIVATE_VAD_PROTOTYPE__`. Off by default. | **Next @dev-agent:** stage/provide `frontend/public/models/silero_vad.onnx` and, if the branch still needs it, declare the direct `onnxruntime-web@1.14.0` dependency without introducing a duplicate ORT runtime. Test-agent prep already passed focused P5 tests (`PrivateWhisper` + `privateVadFlag` = 49/49) and frontend typecheck. **After dev stages the asset**, @test-release-agent runs RMS-vs-VAD browser A/B from PRIVATE_STT. Do not merge VAD until browser A/B + product approval. |
| STT-P6 | P0 | dev-agent | `dev/private-model-eval@25cde3b4` | `d910a07d` | dev follow-up needed / needs current-main branch | Model-eval: flag-selected Private model (`whisper-tiny.en` default / `whisper-base.en` / `distil-small.en`) threaded through worker `init`, behind `?privateModel=` / `window.__PRIVATE_MODEL__`. Off by default. Directly attacks the CI **51.7% WER** decode/model-quality gap. | **Next @dev-agent:** rebase/provide a current-main test branch for the model-eval commits, preserving the off-by-default flag and avoiding unrelated behavior changes. Test-agent prep already passed focused P6 tests (`privateModelFlag`, `PrivateSTT`, `PrivateWhisper` = 63/63) and frontend typecheck on the stale branch. **After dev provides the current-main branch**, @test-release-agent captures `__PRIVATE_MODEL_TELEMETRY__`, WER/accuracy, download MB, load time, decode latency/RTF, app-vs-drop-in, and app-vs-Native-baseline. Do not merge model-eval until A/B + product approval. |
...
| STT-P1D | P1 | dev-agent | `dev/private-live-draft-dedup@5d7d7f89`; tested as `test/private-p1d-current-main@112bb51e` | `main@cc630d3e` | partial pass / dev follow-up needed | Private live-draft repetition + stale filler count (from `conv_01` diagnostic) | **TEST RESULT:** current-main injected proof on canonical `localhost:5174` passed environment proof and the filler-count half (`um=2`, basically=3, like=2, literally=2), but failed the repetition/quality half. Visible live text still repeated (`Uhm, basically... Uhm, basically...`) and the authoritative save candidate was still repeated/corrupted: `Basically, we should literally like, "Wait, um, basically, we should literally like, wait, um, basically."` vs truth `Um. Basically, we should literally like, wait.` `saveCandidateReason=service_result`; service_result/committed_final/store_visible_snapshot all length 106. **Next @dev-agent:** keep the filler-count caller fix, but do not merge P1D as release-ready until the service-result/final candidate repetition is fixed or separately classified. See PRIVATE_STT `## TEST → DEV: STT-P1D current-main injected proof`. **DEV RESPONSE (2026-06-04):** confirmed — the repetition is in the whole-utterance **final decode** (`service_result`), not the live display, so the P1D display-dedup is correct-but-insufficient (keep it). Reclassified as a final-candidate **duplication/repetition** bug (same surface as `private-duplication-probe`). Two candidate causes under investigation: (1) **audio duplication** in the onset/preroll/speechStart buffer assembly → decode sees the utterance ~2× (DEV investigating next); (2) **Whisper repetition-hallucination on degraded box-averaged capture audio** → see new **STT-P8** below (linear-resampler A/B tests this directly). |
| STT-P8 | P1 | dev-agent → test-release-agent | `dev/private-resampler-parity@58d9ba7d` | `main@7656d68d` | ready | Private capture **resampler parity** — close app(63.22%) < drop-in accuracy gap (Open Blocker P0 / F2-#37) | **DEV CANDIDATE READY for @test-agent A/B.** Root cause (from code, deterministic): the Private final/saved decode runs on audio from the real-time capture worklet, which downsamples to 16 kHz with **box-averaging**, while the drop-in reference AND the app's own `AudioProcessor.downsampleAudio` use **linear interpolation**. Same engine/model/decode-params/contiguous buffer → the resampler is the **only** app-vs-drop-in divergence on the decode input, which also explains why the decode-param A/B couldn't fix it. Flagged candidate (`?privateResampler=linear` / `window.__PRIVATE_RESAMPLER__='linear'`); **default `box` = byte-identical**. tsc clean; pcmResamplers 4/4 (streaming-linear == one-shot linear; box≠linear), privateResamplerFlag 6/6, utils 72/72. **Next (@test-agent):** box-vs-linear A/B on the SAME injected audio (conv_01 + h1_6 + human script) — capture `__PRIVATE_RESAMPLER_TELEMETRY__`, WER/accuracy, app-vs-drop-in delta, and **whether linear removes the STT-P1D save-candidate repetition** and the substitutions. Behavior-changing audio → no merge before A/B + product approval. See PRIVATE_STT `## DEV → TEST: STT-P8`. |
