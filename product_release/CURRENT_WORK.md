...
|---|---|---|---|---|---|---|---|
| STT-P5 | P0 | dev-agent → test-release-agent | `dev/private-vad-silero-main@b6159d0c`; tested as `test/stt-p5-vad-b6159-reproof@b6159d0c` | `main@2f4eb545` | blocked before VAD / setup CTA clickability | VAD engine: Silero VAD via the **existing onnxruntime-web** injected at the `PrivateWhisper` onset gate behind `?privateVad=1` / `window.__PRIVATE_VAD_PROTOTYPE__`. Off by default. | **LATEST TEST RESULT (2026-06-05):** focused tests pass `49/49` with frontend Vitest config and coverage disabled. Real-auth `localhost:5174` runtime proof is valid. Browser proof does **not** reach recording/VAD: Private setup CTA `download-model-button-inline` is visible, but Playwright times out because `<html data-model-status="download-required">` intercepts pointer events for 30s. Artifact `product_release/evidence/stt_p5_vad_b6159_reproof_2026-06-05.json`; raw `/private/tmp/stt-p5-vad-b6159-h1_6.json`. **Next @dev-agent:** fix setup CTA clickability or proof-harness setup click path, then ask @test-release-agent to rerun VAD A/B and check `vadFellBackToRms=false` + non-null runtime version. |
