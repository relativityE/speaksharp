...
|---|---|---|---|---|---|---|---|
| STT-P5 | P0 | dev-agent → test-release-agent | `dev/private-vad-silero-main`; tested as `test/stt-p5-vad-current-main` | `main@c34bbd33` | setup blocker cleared / VAD still falls back to RMS | VAD engine: Silero VAD via the **existing onnxruntime-web** injected at the `PrivateWhisper` onset gate behind `?privateVad=1` / `window.__PRIVATE_VAD_PROTOTYPE__`. Off by default. | **LATEST TEST RESULT (2026-06-05):** setup CTA proof-harness hardening works: real-auth `localhost:5174` injected proof reached model setup, recording, stop, save, history/detail. Focused tests passed `49/49`. But VAD is still not actually active: `privateVadTelemetry.vadEnabled=true`, `vadFellBackToRms=true`, `vadRuntimeVersion=null`, `vadMeanSpeechProb=null`, `vadSpeechSegments=[]`; browser still requested `/onnxruntime/ort-wasm-simd-threaded.jsep.mjs?import`. Artifact `product_release/evidence/stt_p5_vad_current_main_reproof_2026-06-05.json`; raw `/private/tmp/stt-p5-vad-current-main-h1_6.json`. **Next @dev-agent:** fix VAD runtime/model activation so it does not fall back to RMS, then ask @test-release-agent to rerun the RMS-vs-VAD A/B. |
...
| STT-P6-HUMAN | P0 | test-release-agent → dev-agent/product | `test/private-v2-model-bakeoff@fbc56771` | `main@fbc56771` | production human proof invalid for base | Human Private model proof requested to back bigger-model accuracy claims. | **RESULT (2026-06-05):** production/main preserves `?privateModel=whisper-base.en` in the URL but still records/saves as `Private (whisper-tiny.en, transformers-js, browser)` with `__PRIVATE_MODEL_TELEMETRY__=null`. Evidence: `product_release/evidence/private_v2_human_bakeoff_2026-06-05.json`. Base-vs-tiny human evidence is therefore **not collectable on production/main**. Next @dev-agent/product: provide a release-proofable base path by merging/exposing the model-eval selector + model telemetry, or reject unsupported model flags instead of silently falling back to tiny. |
...
| STT-V4 | P1 | test-release-agent → dev-agent | latest standalone `test/stt-v4r-stable-wasm-reproof@f8b2d462` | `dev/v4-recovery@f8b2d462` | runtime-blocked / handed back to dev | Private v4 containment + standalone recovery probe | App path produced empty `saveCandidate` / zero saved words. Latest stable-WASM probe still produced **no non-empty browser transcript**: stable onnxruntime-web `1.26.0` WASM and bundled runtime both fail q8 before decode with `Missing required scale: model.decoder.embed_tokens.weight_merged_0_scale`. Artifact `product_release/evidence/stt_v4r_stable_wasm_reproof_2026-06-05.json`. Keep v4 off release A/B. **Next @dev-agent:** stable WASM does not fix v4; treat as bundled JS/model-contract compatibility unless a library fork/version change or alternate model artifact is proposed. |
...
| FEEDBACK-1 | P1 | test-release-agent → deploy/proof | `feature/user-issue-report-backend` | `main` | implemented / deploy proof pending | Beta feedback path: issue report submission, required metadata, transcript/audio opt-in only. | **IMPLEMENTED (2026-06-05):** added Supabase `user_issue_reports` table/RLS migration, authenticated nav "Report issue" dialog, frontend submit service, automatic metadata capture (URL, route, browser, viewport, timezone, runtime config, plan, STT mode, Sentry last event id), and explicit transcript/audio opt-in. Focused tests: `issueReportService` + `Navigation` `17/17`; `pnpm typecheck` passed. Next: apply migration/deploy and verify a real authenticated report row; transcript/audio fields must remain null unless checked. |
