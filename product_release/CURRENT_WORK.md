...
| STT-V4 | P0 | dev-agent → test-release-agent | `dev/v4-recovery@9d4c95bc`; latest main ping `def8d6fa` | `main@def8d6fa` | active / bakeoff probe hardening needed | Private v4 base.en WebGPU candidate. v4 is active again, but must beat v2 base on accuracy and materially improve latency before release use. | Current evidence: worker+WebGPU tiny probe was BOTH-OK; v4 base risk buy-down landed (WebGPU-only/no silent WASM fallback, `modelSource` telemetry, q8/q8 size parity manifest, hidden-guard). Test-agent no-auth bakeoff attempt loaded `/h1_6.wav` (`49,575` samples) but hung at the first `v4-base q8/q8` cell for `120,915ms` with ORT WebGPU node-assignment warnings and no `window.__V4_BAKEOFF__`. **Next @dev-agent:** harden `frontend/v4-bakeoff-probe.html` with per-cell timeout/progress/error or a single-cell selector, then test reruns q8/q8, fp16, fp32, v2 base, and v2 tiny. Authed app lifecycle proof comes after the no-auth bakeoff is measurable. |
