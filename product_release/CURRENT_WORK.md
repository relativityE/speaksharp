...
| STT-P6 | **P0 — TOP PRIORITY (Private accuracy lever)** | dev-agent → test-release-agent | `dev/private-model-eval-main@7c04064a` | `main@ef13a20b` | dev fix ready — candidate load FIXED, re-run A/B | Model-eval: flag-selected Private model (`whisper-tiny.en` default / `whisper-base.en` / `distil-small.en`) threaded through worker `init`, behind `?privateModel=` / `window.__PRIVATE_MODEL__`. Off by default. | **DEV FIXED the candidate-load blocker — ready for @test-agent re-run.** Root cause of your `Unexpected token <` / `index.html instead of model file`: only the default tiny is bundled in `public/models/`, but the worker tried EVERY model **local-first**, and the SPA dev server answers `/models/<candidate>` with `index.html` (HTTP 200, not 404) so it never fell through to HF. Fixes on the branch: (1) **candidate models now load REMOTE-ONLY** (`allowLocalModels=false`); default tiny unchanged/byte-identical; (2) **candidate ids corrected to the verified `onnx-community/` family** — the old `Xenova/distil-small.en` returned **HTTP 401 (does not exist)**; `onnx-community/{whisper-tiny,whisper-base,distil-small}.en` all confirmed present with quantized ONNX; (3) **fail-fast** `MODEL_LOAD_FAILED [key -> remoteId]` (no 180s mic hang). tsc clean; privateModelFlag 5/5, PrivateWhisper 43/43. **Framing (per product): this A/B is the test of whether `whisper-tiny` is the dominant accuracy limiter — do NOT call it a "model ceiling" until this proves it.** Full run spec (3 models × the required fixtures/metrics/3-bar pass-fail) in PRIVATE_STT `## DEV → TEST: STT-P6 model A/B run spec`. Do not merge until A/B + product approval. |
