...
| STT-P6 | P0 | dev-agent → test-release-agent | `dev/private-model-eval-main@1c50587c`; tested as `test/stt-p6-current-main@2c903463` | `main@6a24984c` | blocked / larger candidates fail setup | Model-eval: flag-selected Private model (`whisper-tiny.en` default / `whisper-base.en` / `distil-small.en`) threaded through worker `init`, behind `?privateModel=` / `STT_PRIVATE_MODEL=...`. Off by default. | **PARTIAL BROWSER PROOF:** default `whisper-tiny.en` works on h1_6 (`87.5%`, loadTime `663 ms`, artifact `/private/tmp/stt-p6-tiny-h1_6.json`). Both candidate models fail before recording: `whisper-base.en` and `distil-small.en` time out in setup and console shows `Model load failed with "Unexpected token <"` / `server returned index.html instead of the model file`. **Next @dev-agent:** fix candidate model IDs or model asset resolution and add a fail-fast named setup error (`MODEL_NOT_FOUND` / `MODEL_MANIFEST_INVALID`) so the mic is not disabled for 180s. See PRIVATE_STT `## TEST → DEV: STT-P6 browser model-eval setup proof`. |
