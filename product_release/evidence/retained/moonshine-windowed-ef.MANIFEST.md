...
E2 and E3 fail solely because they assert exact whole-string equality (`sessionA.final ===
sessionB.final`), so a single unstable token fails them. The assertions are correct as written and
**should not be relaxed to obtain a pass**; the variance belongs recorded, which is what this manifest
does.
...
Disposition: **P2, routed to #1399.** One unstable word at end-of-session is below the noise floor of a
multi-minute spoken take, so it does not disqualify moonshine from the human comparison. It does mean
exact reproducibility cannot be claimed for this candidate.
