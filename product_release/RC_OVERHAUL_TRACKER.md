...
| RC-14 | Private engine decision table | Gate 1 | In progress | v2 default, v4 experimental, unavailable/bad dtype/fallback behavior explicitly tested. | Added contract coverage for registry fallback reporting the actual instantiated engine. Found and fixed bug where fallback `whisper-turbo`/`mock` engines could be reported as `transformers-js`. Remaining gap: v4 experimental isolation and bad dtype/failure behavior. |
