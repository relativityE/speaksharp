...
| WI-22 | Existing test contract audit | All | In progress | Existing unit, integration, e2e, live, and workflow checks must either name their independent contract source or be marked advisory/diagnostic. | Live scan found 157 test/spec files. Inventory is current at bucket/gate level; per-file contract-source ledger remains incomplete for mocked E2E/live/canary and broad frontend unit/component tests. Prevents tests from defending current implementation behavior instead of product requirements. |
