...
| WI-22 | Existing test contract audit | All | In progress | Existing unit, integration, e2e, live, and workflow checks must either name their independent contract source or be marked advisory/diagnostic. | Live scan found 157 test/spec files. RC-counted ledgers are explicit for browser/live/canary and unit/component files. Remaining work is deeper review of advisory tests for deletion, promotion, or bug-defending assertions. Prevents tests from defending current implementation behavior instead of product requirements. |
