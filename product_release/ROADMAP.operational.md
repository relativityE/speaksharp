...
3. **Stripe Verification**: Not required for the controlled **no-billing beta** — paid checkout is intentionally closed by the payment switches (`VITE_PAYMENTS_ENABLED` / `PAYMENTS_ENABLED` both OFF), NOT by the key class; `stripeKeyClass="test"` + the Beta-50 billing freeze are co-facts. The billing journey is proven in Stripe **test** mode (accepted proof). A live-money transaction belongs to the separate future paid cutover (`PAID_OPS_HARDENING_RUNBOOK.md`), gated on written owner approval — do **not** run a live charge during this beta.
