...
| **WPM Accuracy** | ✅ CODE FIX PRESENT / LIVE MONITORING | New recording start clears transcript, filler data, chunks, pause metrics, elapsed time, and saved-state in `SpeechRuntimeController.resetAnalysisStateForNewRecording()`. Continue watching live tester reports for metric drift. | **P2 if regressed**: Minor metric glitch | Yes |
