...
- [x] Unit tests and mocked E2E are green in the latest GitHub `CI - Test Audit`: `744fea8` run `25607518542` passed on 2026-05-09.
- [x] Production deploy smoke is green: `canary.yml` passed on `744fea8` (run `25607518535`), `deploy-edge-functions.yml` passed on the same push (run `25607518527`), and `deploy-supabase-migrations.yml` passed on `7da01c8` (run `25606499810`).
...
| AI suggestion parsing | Code fix present in `9dff649`; latest local hardening also returns degraded fallback suggestions when Gemini is unavailable, not only when JSON is malformed. | `CI - Test Audit` run `25607518542` passed on the previous push; local `deno test --allow-env --allow-net backend/supabase/functions/get-ai-suggestions` passed 7 steps. | Push/rerun Edge deploy, then live AI suggestion smoke. |
...
| Local release test gate | Latest GitHub release gate is green on `744fea8`; current audio/promo UX analytics fixes are local and need the next push/rerun. | `CI - Test Audit` run `25607518542` passed; production canary run `25607518535` passed; Edge Function deploy run `25607518527` passed; migration deploy run `25606499810` passed. Local pause/Cloud audio pump and expired-promo dialog fixes have targeted regression coverage and frontend typecheck green. | Push/rerun GitHub CI for the newest local fixes, then keep live feature matrix pending until deployed browser validation proves Native/Private/Cloud transcript, save, analytics, and export paths. |
