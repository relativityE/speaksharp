...
| **Accuracy Benchmarks** | Accuracy claims are backed by `.wav` plus ground-truth WER runs before they appear as user-facing comparisons. | Benchmark harnesses exist, but the current manifest is not release-grade: Cloud has no history, Native has no numeric WER, Private CPU history conflicts with `expectedAccuracy`, and the browser fixture/reference mapping must be fixed before trustworthy Native/Private WER can be recorded. The user-facing comparison component is being changed to show "not benchmarked" instead of fallback ceilings when current WER evidence is missing. | 🔴 BLOCKED / TRUTHFUL UI PATCH LOCAL | Fix browser benchmark fixture/reference mismatch, rerun Cloud/Native/Private CPU WER, optionally run WebGPU as acceleration evidence, and record current WER before advertising benchmark ceilings. |
...
| Local release test gate | GitHub release gate is green on `9dff649`; local patch evidence has moved ahead for PDF and analytics. | `CI - Test Audit` run `25602605844` passed; production canary run `25602605839` passed; Edge Function deploy run `25602605834` passed. Local-only follow-up work improves PDF proof and analytics correctness but is not yet pushed/run. | Keep live feature matrix pending until deployed browser validation proves Native/Private/Cloud transcript, save, analytics, and export paths. |
