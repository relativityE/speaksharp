...
- [ ] Unit tests and mocked E2E are green in the latest GitHub `CI - Test Audit`: latest pushed run `25621064008` failed in unit truth because pricing-page tests expected stale copy/grid, then report also failed because E2E was skipped and no Playwright results existed. Local fix is applied; `pnpm ci:unit` passes. GitHub rerun pending.
...
| **Custom/User Words** | User words persist to Supabase, are available next session, are saved into per-session analysis snapshots, and Cloud receives boost words when explicitly selected. | Mocked add/remove/detection E2E passed locally on 2026-05-10 (`tests/e2e/user-filler-words.e2e.spec.ts`, 7/7 including infra probe). Cloud unit coverage confirms `keyterms_prompt` is included only when Cloud starts with user words and omitted without user words. Per-session custom word snapshot persists on stop. | 🟡 MOCKED PASS / LIVE VALIDATION PENDING | Live Pro test: add word, refresh/login, record Native/Private/Cloud session, verify persistence and keyterms behavior with real credentials. |
...
| **Observability** | Sentry/PostHog capture launch-relevant frontend/backend events. | Instrumentation exists; live project ingest is pending. Local mixed-environment Vitest command is not evidence because frontend Sentry tests require jsdom/Vite aliases and Edge Function tests run under Deno. | 🟡 PENDING | Send frontend error, Edge Function error, and key product analytics events to the live projects; verify in Sentry/PostHog dashboards. |
...
| Local release test gate | Latest deploy/canary gates are green; latest GitHub CI audit is not green. | `deploy-supabase-migrations.yml` run `25620857952` passed; production canary run `25621064004` passed; Edge Function deploy run `25621064007` passed; Vercel served `1ea2b099` before this local patch. Latest `CI - Test Audit` run `25621064008` failed on stale pricing tests plus report artifact handling. Local `pnpm ci:unit`, `pnpm test:edge`, typecheck, and focused regression tests pass after fixes. | Push/rerun GitHub CI, then keep live feature matrix pending until deployed browser validation proves Native/Private/Cloud transcript, save, analytics, and export paths. |
