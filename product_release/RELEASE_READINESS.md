...
| **All STT Artifact Path** | Private, Cloud, and Native each record a fixture transcript, save, open analytics detail, get AI feedback, and export a transcript-bearing PDF. | Private already passed this path in run `25634578516`. A new serialized Pro STT artifact matrix has been added to require the identical path for `private`, `cloud`, and `native` against the deployed app. | 🔴 LIVE MATRIX PENDING | Run the `Pro STT Artifact Matrix` workflow after push. Green requires non-placeholder transcript text, `Session saved`, analytics detail, live AI suggestions response, and parsed PDF transcript evidence for all three modes. |
...
| **Cloud STT** | Pro users may explicitly choose Cloud as a first-class option. | Live Cloud token gates have passed, and GitHub `STT Ceiling Benchmarks` run `25643746457`, job `75268546634`, proved AssemblyAI at `0.00%` WER across all 10 Harvard WAV fixtures. Product artifact path is still not green: after fixing AssemblyAI tiny-chunk rejection, the latest app-path run failed because the Cloud selector raced Pro policy sync. The selector helper now waits through delayed Pro option availability. | 🔴 PRODUCT ARTIFACT PATH PENDING | Run the serialized Pro STT artifact matrix. Cloud is green only when it records fixture transcript, stops after the product save policy, saves, opens analytics, gets AI feedback, and exports parsed PDF evidence. |
