...
| **Cloud STT** | Pro users may explicitly choose Cloud as a first-class option. | Live Cloud token gates have passed, and GitHub `STT Ceiling Benchmarks` run `25643746457`, job `75268546634`, proved AssemblyAI at `0.00%` WER across all 10 Harvard WAV fixtures. The Cloud blocker was decomposed into gates. Commit `cafcad8f` fixed mode authority, and commit `1b9d1667` patched/deployed stale active-session concurrency handling (`Deploy Supabase Migrations` run `25687248801` succeeded). Latest focused Cloud run `25687338083` proved WebSocket open, mic frames, audio chunks, and AssemblyAI `Turn` transcript messages up to `textLength:71`; it then failed stop/save because `CLOUD_SAVE_DECISION` saw `empty_transcript`. The confirmed row is now finalization-source mismatch: provider partial text is visible, but save used final transcript only. Current local patch makes `TranscriptionService` preserve latest partial text when no provider final exists; focused unit tests pass 9/9. | 🟡 FIX APPLIED / LIVE ARTIFACT RERUN PENDING | Push current patch and rerun `Pro STT Artifact Matrix` with `mode=cloud`. Cloud is green only when it proves transcript -> Stop -> `Session saved` -> analytics detail -> AI feedback -> parsed PDF transcript evidence. |
