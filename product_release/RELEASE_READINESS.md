...
- [ ] Unit tests and mocked E2E must be green in the latest GitHub `CI - Test Audit`. Prior green evidence exists on `9dff649` (`25602605844`), but the newer `7da01c8` CI run exposed an Edge Function test import issue; local fix is applied and pending push/GitHub rerun.
- [x] Production deploy smoke is green: `canary.yml` passed on `7da01c8` (run `25606491521`), `deploy-edge-functions.yml` passed on the same push (run `25606491505`), and `deploy-supabase-migrations.yml` passed on `7da01c8` (run `25606499810`).
...
| Local release test gate | Prior GitHub release gate is green on `9dff649`; latest GitHub CI is pending rerun after a local Edge Function import fix. | `CI - Test Audit` run `25602605844` passed; production canary run `25606491521` passed; Edge Function deploy run `25606491505` passed; migration deploy run `25606499810` passed. The newer `7da01c8` CI run failed `edge-tests` because the webhook test graph resolved `npm:stripe@16` at import time; local fix lazy-loads Stripe/Supabase only in production runtime. Local `pnpm test:edge` now passes 8 files / 36 steps, focused session UI Vitest passes 19 tests, and frontend typecheck passes. | Push/rerun GitHub CI, then keep live feature matrix pending until deployed browser validation proves Native/Private/Cloud transcript, save, analytics, and export paths. |
