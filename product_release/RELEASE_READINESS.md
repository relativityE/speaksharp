...
| **Baseline Basic Tier** | The unpaid baseline tier is internally `basic` everywhere and displays as Basic to testers. | Strict pre-launch cutover is implemented across frontend, Edge Functions, migrations, workflows, test fixtures, and docs. Existing pre-launch `free` rows are converted by `20260521000000_basic_tier_cutover.sql`; fresh databases start with `basic`. | 🟡 LOCAL FIX GREEN / DEPLOY VALIDATION PENDING | Deploy the migration and functions, then verify Basic signup, quota, Native-only gating, promo upgrade, expired-promo downgrade, and session save/read. |
