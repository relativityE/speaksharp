...
| **Promo Pro Access** | Promo code grants select tester Pro access for the intended duration. Every promo code is one-time use; a redeemed code must not succeed again, even for the same user during the active promo window. | Mocked promo E2E passes valid promo, invalid inline error, and invalid-with-valid-credentials fallback; DB-backed attempt throttling exists; one-time DB constraint/RPC migration added. | 🟡 MOCKED GREEN / LIVE VALIDATION PENDING | Deploy migration/function, generate a live code, redeem once successfully, verify second redemption fails, then run live promo signup smoke. |
...
| Promo tester flow | Mocked E2E green locally. | `promo-admin-journey.e2e.spec.ts` passed 8/8 including infra probes; `PROMO_GEN_ADMIN_SECRET` is now present in GitHub secrets; manual generator workflow added. | Push, review CI/canary, generate a live one-time promo, then run live promo smoke with production secrets. |
