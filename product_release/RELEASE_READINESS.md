...
- [x] CI/Test Audit is green on `main`: run `25944598514`.
- [x] Production canary is green on `main`: run `25944598537`.
- [x] Edge Function deploy is green on `main`: run `25944598524`.
...
- [ ] Physical real-mic Pro Cloud proof is completed if Cloud remains marketed as a primary public Pro feature.
...
| **Transcript Propagation** | Live transcript updates and `TRANSCRIPT_PULSE` telemetry come from the same successful path. | Current CI/Test Audit run `25944598514` is green on `main`. Public-launch ledger evidence now records Basic first-use PASS, Private/Cloud provider-level artifact evidence, and PDF/AI proof; physical real-mic Cloud remains a separate public-launch caveat. | 🟢 CONTROLLED TESTER GREEN / PUBLIC REAL-MIC CAVEAT | Keep physical real-mic Cloud proof separate from CI and provider-level artifact evidence. |
...
| **GitHub Canary Deploy Smoke** | Main-branch GitHub canary proves deployed auth/session/analytics path against real infrastructure. | GitHub production canary passed on `main` in run `25944598537`; Edge Function deploy passed in run `25944598524`. | ✅ PASSING | Keep as required post-deploy smoke; investigate immediately if it regresses. |
...
| **CI Audit Runtime** | Required CI gives fast enough feedback to unblock next-day release fixes without hiding correctness failures. | Latest `CI - Test Audit` run `25944598514` passed on `main`; correctness is green. Runtime optimization remains release-velocity work, not a product blocker. | 🟡 PROMOTED RELEASE-VELOCITY WORK | After launch blockers are cleared, shard/split unit coverage by domain or package and preserve a single aggregate required result. |
...
| Local release test gate | Deploy/canary/CI gates are green on the latest pushed commit. | CI/Test Audit run `25944598514`, production canary run `25944598537`, and Edge Function deploy run `25944598524` passed on `main` after commit `1066ba6d`. | Keep public launch NO-GO until live Stripe and physical real-mic/real-device caveats are resolved or explicitly scoped out. |
