...
| **Cloud STT** | Pro users may explicitly choose Cloud as a first-class option. | Live Cloud token gates have passed, and GitHub `STT Ceiling Benchmarks` run `25643746457`, job `75268546634`, proved AssemblyAI at `0.00%` WER across all 10 Harvard WAV fixtures. Product artifact path is still not green. After fixing AssemblyAI tiny-chunk rejection and the Pro selector race, the full artifact matrix `25644204999` did not reach Cloud because Private failed first. Focused Cloud-only run `25644479281` is currently running via `mode=cloud`. | 🔴 PRODUCT ARTIFACT PATH PENDING | Cloud is green only when the focused Cloud row records fixture transcript, stops after the product save policy, saves, opens analytics, gets AI feedback, and exports parsed PDF evidence. |
