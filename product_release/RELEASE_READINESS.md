...
| **Promo Pro Access** | Promo code grants select tester Pro access for the intended duration. Every promo code is one-time use; a redeemed code must not succeed again, even for the same user during the active promo window. | Mocked promo E2E passes valid promo, invalid inline error, and invalid-with-valid-credentials fallback. DB-backed attempt throttling and one-time redemption are present in the release code. Prior production promo canary passed new-user redemption, reuse rejection, and no-promo baseline; the current post-push canary/live QA run is still pending. | 🟡 PRIOR LIVE SMOKE PASSED / CURRENT DEPLOY VALIDATION PENDING | Continue live tester QA with fresh one-time codes; keep code duration short for external testers. |
...
|---|---|---|---|
| Quota fail-closed | Code fix present in current pushed commit. | `check-usage-limit` Deno tests pass locally; Edge Function deploy workflow for the current push is green. | GitHub/live Edge Function fail-closed validation. |
| Cloud token usage gate | Code fix present in current pushed commit. | Targeted ESLint passed before push. | Live Pro/over-limit token smoke. |
| Negative duration guards | Forward migration present in current pushed commit. | SQL review complete. | Apply migration and verify negative increments reject. |
| Promo brute-force | Code + migration present in current pushed commit. | `deno check` and targeted ESLint passed before push. | Verify live throttling/reuse on the current deployment. |
| Canary harness | Code fix present for login route and production origin guard. | Targeted ESLint passed before push; prior canary run passed. | Current GitHub canary rerun must pass. |
| AI suggestion parsing | Code fix present in current pushed commit. | `deno test --no-lock --allow-env --allow-net backend/supabase/functions/get-ai-suggestions` passed. | Deploy evidence and live AI suggestion smoke. |
| Pro warning UI | Code fix present in current pushed commit. | `vitest ... --coverage.enabled=false frontend/src/hooks/__tests__/useSessionLifecycle.test.tsx` passed; targeted ESLint passed. | Review current GitHub CI. |
| Request-aware CORS | Code fix present in current pushed commit. | `check-usage-limit` now uses shared request-aware `corsHeaders(req)`; Deno tests pass locally. | Verify deployed Edge Function CORS headers. |
| Private model cache/progress | Hotfix present in current pushed commit. | `ModelManager` probes Whisper Turbo IndexedDB stores; store progress normalization clamps `0..1` and `0..100` inputs to UI percent. 2026-05-08 browser testing found stale error state on first-use cache miss and a later stuck `Initializing engine...` state after failed Private setup; targeted local tests for FSM/TranscriptionService/TransformersJS pass after retry-state fix. CPU/Transformers.js same-origin model assets are now locally load-proven, Private now routes through `PrivateWhisper`, and launch policy is CPU-first with explicit WebGPU validation. | After Vercel/current canary are green, perform headed browser cache/download/transcript validation. |
| Session analysis persistence | Code fix present in current pushed commit. | `storage.test.ts` and `SttSafeguards.test.ts` pass; selectors reload transcript, WER, pause, AI, custom-word, and engine metadata fields. | Review current GitHub CI and live-test returning-user comparison. |
...
| Local release test gate | Local green after readiness and mock-mic fixes. | `pnpm ci:unit` passed (`106` files, `627 passed | 1 todo`); `pnpm test:e2e` passed (`40 passed`, `0 flaky`); targeted promo expired component tests passed (`15/15`); frontend typecheck is clean. | Review current GitHub CI and production canary after deploy. |
