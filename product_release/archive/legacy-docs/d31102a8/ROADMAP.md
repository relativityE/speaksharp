<!-- HISTORICAL / NON-AUTHORITATIVE — pinned copy of docs/ROADMAP.md at commit d31102a8 (2026-05-17). Read-only migration source; not current product truth. Current authority: product_release/README.md. Do not edit. -->

> ⚠️ **HISTORICAL / NON-AUTHORITATIVE.** Pinned copy of `docs/ROADMAP.md` at commit `d31102a8` (2026-05-17). Read-only migration source for the doc-canonicalization effort — **not** current product truth. Current authority: `product_release/README.md`. Do not edit.
...
- [ ] Refactor Download Model tier gating: display disabled button for Free users as marketing funnel ([PRD §3.5](https://github.com/relativityE/speaksharp/blob/d31102a8/docs/PRD.md#L157-L159))
...
- 🔴 **1-Click Practice Session Summaries (Funnel Feature):** Allow users to generate PDF practice summaries. Basic and Pro exports must include prominent SpeakSharp watermarking/branding. Basic PDF export is not count-limited because client-side generation has no variable provider cost.
...
| 29 | **Vitest Parallel Worker Evaluation** | `frontend/vitest.config.mjs`, unit test setup | P3 | ⏸️ Deferred | Current release gate keeps `maxForks: 1` for deterministic singleton isolation across Zustand stores, SpeechRuntimeController, SessionManager, Supabase mocks, browser globals, and analytics buffers. Do not switch CI to 4 workers before production validation. Later work: add an opt-in `test:unit:fast` script with 3-4 forks, run repeated stability trials, and fix any shared-state leaks before promotion. |
...
| **L2** | **Test Category Consolidation** | **HIGH** | ✅ Complete | Superseded legacy per-phase test trackers. Current execution ladder: `pnpm test:infra`, `pnpm test:unit`, `pnpm test:e2e`, `pnpm test:full`, `pnpm ci:local`, `pnpm ci:github`, and `pnpm ci:full`. Latest validation: 621 unit passing + 1 todo; CI mimic reports infra probe 5/5 and full app suite 35/35, replacing the stale 39/40 goal-setting status. |
