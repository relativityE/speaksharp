...
| Work Item | Area | Gate | Status | Required Outcome | Evidence / Notes |
|---|---|---|---|---|---|
| WI-10 | Native Chrome proof | Gate 1 / Gate 5 | Red | Real Chrome + real mic artifact with coherent transcript, no repetition, no unrecovered `onerror`. Native is launch-critical because it is the first low-friction onboarding STT path for trial users. | Latest synthetic `say` artifact `/private/tmp/speaksharp-native-say-trace-1779704244115.json` is technically clean but functionally red: first visible `8.655s`, transcript only `on the`. The proof harness now waits for Native audio readiness before playing synthetic speech so the next artifact tests the app/provider path instead of racing Chrome startup. Fake-device Native probe `/private/tmp/speaksharp-native-fake-audio-probe.json` showed Web Speech events fired but transcribed `this one still has a bit next` instead of expected `stale smell old beer lingers`; fake-device Native is diagnostic only until the selected fixture audio is proven to reach Web Speech. |
...
| WI-16 | STT corpus gates | Gate 1 / Gate 5 | In progress | Harvard WAV/truth fixtures become the common STT-A WER corpus, representative fixtures become STT-B browser journey evidence, and `conv_01`/`conv_02` become STT-C filler-value evidence. Fake-device runs prove deterministic code correctness; real-mic runs prove product readiness. | `RC_GATES.md` and `RC_TEST_INVENTORY.md` now define the two-layer gate. Native fake-device probe is not countable yet because it did not transcribe the intended `h1_1.wav` content. Added `pnpm rc:stt:corpus` real-mic harness to produce JSON artifacts with transcript, WER, first-text timing, console/page/network errors, history/analytics visibility, and mode-specific traces. Next step is running calibration against the current app on `4173`. |
