...
- **Paid Early Access Capability**: SpeakSharp supports **paid Early Access** to Pro (Private transcription, deeper history, Cloud STT, expanded coaching capacity). Paid enrollment is gated by the two payment switches (`VITE_PAYMENTS_ENABLED` AND `PAYMENTS_ENABLED`) plus aligned live Stripe configuration and entitlement verification — the Stripe key class validates configuration but does not by itself open checkout. The current tester cohort's enrollment toggle state lives in `RELEASE_STATUS.md`, not here. Enabling a future paid tester cohort requires the full payment activation contract **and** explicit Prod Owner authorization. **Controlled paid Early Access is a separately-authorized step, not the same as broad public launch.** Existing accounts with a valid paid-Pro entitlement retain access regardless of whether new enrollment is currently open.
...
- **Private-First Hierarchy**: Private is the recommended transcription mode and the main experience. Browser is a free "Quick preview" convenience path, NOT an equivalent to Private. Cloud is a Pro feature requiring paid-Pro entitlement: while new paid enrollment is disabled for the current tester cohort (state in `RELEASE_STATUS.md`), Cloud is unavailable to Free testers, while existing accounts with a valid paid-Pro entitlement retain access.
- **STT Mode Consent**: Private STT MUST NOT silently switch to Cloud STT. Cloud is a first-class Pro choice, but it requires explicit user selection and Pro entitlement; while new paid enrollment is disabled for the current tester cohort, the Cloud option is not available to Free testers (existing paid-Pro accounts retain access).
