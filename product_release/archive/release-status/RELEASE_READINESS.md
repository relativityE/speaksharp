...
| **Native STT** | Real Chrome/Edge mic run uses the browser Web Speech path; `onspeechstart -> first onresult` is < 3s; first result contains words from the spoken phrase; no repeated phrase loop, defined as any 4-word sequence appearing more than once in the final transcript; no unrecovered `onerror` (`no-speech`, `network`, or similar); restart after `onend` fires within 300-400ms when still recording. | `/private/tmp/speaksharp-native-[timestamp].json`. Artifact must include Web Speech event timestamps, raw/final transcript events, restart events, browser console events, and the 4-word repetition check result. |
...
| **Analytics** | Filler count matches the predeclared spoken transcript. For the Cloud baseline sentence, expected ground truth is `like = 1` and `basically = 1`; any other count is red. WPM is calculated as transcript word count divided by recording duration in minutes and must be within ±15% of the hand-counted reference. Clarity label/copy can be explained by a non-technical user as a concrete behavior to improve; session page, Analytics, persistence, and history/detail use the shared calculation source. | Analytics screenshot plus persisted session JSON/DB row from the same Cloud or Private session. Artifact must show transcript, duration, hand-counted word count, calculated WPM, displayed WPM, WPM delta percentage, clarity, filler/custom-word counts, and guidance copy. |
