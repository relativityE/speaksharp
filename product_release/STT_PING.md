...
| 2026-06-06T19:45Z | V4 | **STOP RETRACTED — v1 bakeoff was contaminated; rerun corrected probe** | Dev retracts the prior STOP: the v1 numbers did NOT measure pure v4. Confounds: (1) **omitted the HF realtime-demo dtype `fp32-enc/q4-dec`** (the proven-good config + my earlier BOTH-OK probe) — v1 only ran q8/q8 (hang), fp16/fp16 (adapter lacks `shader-f16`), and **fp32/fp32** (the 209MB worst case); (2) measured a **single COLD decode** incl one-time WebGPU shader compilation; (3) ran on an **fp16-incapable adapter**; (4) v2 comparator broke on missing `buffer`/`long` polyfills. Vendor envelope: HF `realtime-whisper-webgpu` runs whisper-base on WebGPU **faster than real-time** ("up to 100× faster than WASM"), so a ~3s clip should be **sub-second warm** — the 48s (RTF≈16) is ~30-50× off = harness artifact, not v4. CORRECTED probe `frontend/v4-bakeoff-probe.html` (`dev/v4-recovery@ada3de1c`): adds fp32e/q4d + q4/q4, measures **warm steady-state** decode (median of N after a cold warm-up) + reports cold/warm/**RTF**, reports adapter `shader-f16`, bundles both libs (`?bundle`) so v2 base/tiny run. ASK (TEST, no auth): rerun "Run all" on a **shader-f16-capable** WebGPU GPU; paste `window.__V4_BAKEOFF__` (adapter + per-dtype loadMs/coldMs/warmMs/RTF/WER). THEN apply the locked decision rules to the WARM numbers. Release spine unchanged: v2 tiny default + v2 base opt-in. | `=> test` |
