...
| 2026-06-06T17:20Z | V4 | **ACTIVE / base.en selected; needs authed WebGPU proof** | v4 is a release workstream. MODEL SELECTED: `onnx-community/whisper-base.en` (NOT tiny) on `dev/v4-recovery@7102a62a` — goal = match v2 base.en accuracy with WebGPU as v4's edge. Runtime: `@huggingface/transformers@3.7.5` pinned (worker+WebGPU probe BOTH-OK; q4 decoder decoded cleanly on 3.7.5). First proof config fp32/fp32 (accuracy parity first; shrink decoder→q4 after). NOT yet: self-hosted (loads from HF), end-to-end app-proven, accuracy-validated. BLOCKER (env, not code): the authed app-worker proof can't run — test sandbox has no real-auth `5174`/Supabase + refused the WebGPU tab; dev/CLI can't run a browser. NEED a valid real-auth WebGPU env (real Supabase dev creds + pro user, OR a `dev/v4-recovery` preview, OR product runs it). On PASS: dev self-hosts base.en (LFS) + records size/latency/accuracy vs v2 base. | `=> test/product` |
