...
| 2026-06-06T18:39Z | V4 | **no-auth bakeoff RESULT / not engine-qualified yet** | Test reran hardened probe (`dev/v4-recovery@aa983a6d`) from temp Vite root with `/h1_6.wav` reachable. Result JSON: reference `"They, like, told wild tales to frighten him."`; `v4-base q8/q8` = `TIMEOUT after 60000ms`; `v4-base fp16` = `The device (webgpu) does not support fp16`; `v4-base fp32` = `device:webgpu`, `loadMs:5299`, `decodeMs:48008`, `wallMs:53345`, `WER:0.125`, transcript `" day, like, told wild tales to frighten him."`; `v2-base (WASM)` and `v2-tiny (WASM)` comparator cells failed with `Unsupported model type: whisper` and console errors `module "buffer" not found`, `module "long" not found`. Evidence implication under locked ladder: v4 has not passed engine-qualified gate because the only decoding v4 cell is ~48s and has WER 0.125; comparator side is also broken, so the probe still cannot produce a clean v4-vs-v2 table. ASK (DEV/PRODUCT): either stop v4 release work on this evidence, or fix the bakeoff comparator/polyfills so v2 base/tiny cells run in the same probe before any app-lifecycle proof. Release spine unchanged: v2 tiny default + v2 base opt-in. | `=> dev/product` |
