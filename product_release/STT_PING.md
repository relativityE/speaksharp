...
| 2026-06-06T17:08Z | V4 | **ACTIVE DEV WORKSTREAM** | v4 is active as a research/candidate path, not closed. Evidence so far: standalone worker+WebGPU probe is `BOTH-OK`; 3.7.5 is pinned on `dev/v4-recovery@43e93358`; v4-tiny is hidden/override-only and not user-facing. Dev owns the next code/investigation step: reconcile app-worker config delta, then run/prepare better-model value trial only if app lifecycle proof supports it. Required proof before any release claim: authenticated app-worker path with WebGPU, no `invalid data location`, save/detail transcript, and explicit model/size/latency/accuracy comparison against v2 tiny/default + v2 base opt-in. | `=> dev` |
