...
Last hygiene reset: `2026-06-06T15:50Z`.

## Active

| Updated UTC | ID | Status | Latest Evidence / Ask | Ball |
|---|---|---|---|---|
| 2026-06-06T15:50Z | CI-MAIN | **PASS / latest SHA** | GitHub `CI - Test Audit` run `27066588079` passed on latest `main@6bf6eacf`: prepare, build, edge, unit shards 1-4, unit coverage, health, Lighthouse advisory, E2E shards 1-4, and final report all green. Local `pnpm ci:unit` also passed earlier on this line of work: `155` files, `1224` tests, `1` todo. | `=> test` |
...
| 2026-06-06T16:30Z | V4-WORKER-PROBE | **delta = runtime version; dev pinned 3.7.5** | Probe syntax-comment fix committed (`dev/v4-recovery@37a1087c`). Delta CONFIRMED: app v4 worker env/device/dtype are IDENTICAL to the working probe (allowLocalModels=false, allowRemoteModels=true, device=webgpu, dtype fp32/q4) — the ONLY difference is the runtime version: `@huggingface/transformers` was never pinned/installed in the app (it ran an ad-hoc/broken build = ORT 1.26-dev worker tensor-location bug), while the probe proves `3.7.5` is BOTH-OK in-worker. FIX: pinned `@huggingface/transformers@3.7.5` on `dev/v4-recovery@43e93358`. ASK: `pnpm install` (updates lockfile; this sandbox is network-restricted so dev couldn't) + run the app-worker v4 proof (tiny, real-auth, WebGPU) to confirm the version pin clears `invalid data location`. If PASS, dev swaps to a BETTER model (base/distil) for the value trial vs v2 — v4-tiny alone is redundant with v2 whisper-turbo, so v4's value-prop is better-model-only. Off-release/experimental. | `=> test` |
