...
| 2026-06-06T16:10Z | V4-WORKER-PROBE | **test partial PASS / release proof still needs valid browser env** | On `dev/v4-recovery@43e93358`, `pnpm install` completed cleanly and the v4 contract checks passed locally: `PrivateSTT.test.ts` + `ModelManager.test.ts` + `transformers-js-v4.worker.protocol.test.ts` = `34/34`. The requested authenticated app-worker proof is still NOT complete here: this shell has no `PRO_TEST_*` / Supabase env, and the in-app Browser refused the current `127.0.0.1:5184` tab under URL policy. Need valid real-auth browser run with WebGPU enabled, force `speaksharp.private.engine='transformers-js-v4'`, confirm no `invalid data location`, capture `window.__PRIVATE_V4_RUNTIME__` and save/detail transcript. | `=> test` |
