...
| 2026-06-06T19:00Z | V4 | **bakeoff probe hardened; 3-gate ladder locked; rerun no-auth bakeoff** | Probe hardened (`dev/v4-recovery@aa983a6d`): per-cell isolated worker + per-cell timeout (default 60000ms) + progress + incremental render + per-cell Run buttons, so one hung dtype (q8/q8) records a `TIMEOUT` and the rest still run. PROMOTION LADDER (product-locked — do NOT skip levels): (1) **engine-qualified** = passes no-auth bakeoff; (2) **app-qualified** = passes authed record→save→detail; (3) **release-contained** = self-hosted (no-HF). v4 is NOT promotable on bakeoff alone. FAIL v4 for release if: worse WER than v2 base · not materially faster · can't prove `resolvedDevice=webgpu` when labeled accelerated · silent fallback · requires HF at release · fails record/save/detail. CONTINUE only if: matches/beats v2 base WER · materially faster decode/finalize · WebGPU confirmed · truthful fallback · hidden until app proof. ASK (TEST, no auth): rerun `/v4-bakeoff-probe.html` (WebGPU Chrome, "Run all"), paste `window.__V4_BAKEOFF__` — capture per dtype: WER, transcript, loadMs, decodeMs, resolvedDevice, fallback, which dtypes decode (expect q8/q8 timeout → fp16 likely the smallest viable, ~146MB > v2 base 80MB). If v4 base is NOT materially better than v2 base → STOP v4 for release. If it matches/beats + faster → authed app-lifecycle proof (`v4-base-run-handoff.md`); only after THAT does dev self-host the winning artifact. Release spine unchanged: v2 tiny default + v2 base opt-in. | `=> test` |
