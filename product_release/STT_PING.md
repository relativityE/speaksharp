...
| 2026-06-06T18:25Z | V4 | **base.en + full risk buy-down landed; bakeoff probe runnable NOW (no auth)** | v4 = `onnx-community/whisper-base.en` on `dev/v4-recovery@9d4c95bc`, runtime `@huggingface/transformers@3.7.5`. ALL dev-controllable buy-down DONE: (a) label-truth invariant — WebGPU-only worker, no silent v4→WASM, `modelSource` telemetry (`@84f5ca05`); (b) artifact manifest `product_release/v4-base-artifact-manifest.md` — **q8/q8 = 76.9MB = v2 base byte-identical; q4 dec (124MB) > q8 (53.7MB)** so size-optimal path = q8/q8 (dtype order q8→fp16→fp32); (c) hidden-guard test (v4 never default, fails closed) + run-handoff doc `product_release/v4-base-run-handoff.md`; (d) **NEW non-auth bakeoff probe `frontend/v4-bakeoff-probe.html`** — loads `/h1_6.wav` and decodes through v4 base (q8/q8, fp16, fp32) + v2 base + v2 tiny in a worker, reports device/load+decode ms/transcript/WER → `window.__V4_BAKEOFF__`. ASK (TEST, no env needed): serve `dev/v4-recovery` (`pnpm install` first) in a WebGPU Chrome, open `/v4-bakeoff-probe.html`, Run, paste `__V4_BAKEOFF__` — gives accuracy/latency/size + whether q8/q8 decodes on 3.7.5, WITHOUT auth. Only the authed app-lifecycle stage (record→save→detail) still needs the real-auth env. SHIP rule: match/beat v2 base accuracy AND materially faster, else hidden; no self-host until winning dtype known. | `=> test` |
