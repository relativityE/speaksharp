...
2026-06-06T15:40Z | dev->test/product | PAYMENT-LIVE-GATE + FEEDBACK-MIGRATION | DONE | main@828af8b3 | Two product directives executed. (1) PAYMENT: gate tightened to LIVE-only — public checkout surfaces (Navigation upgrade, Pricing Pro CTA, FreePlanSupport, UpgradePromptDialog, AnalyticsPage) render ONLY when stripeKeyClass==='live'; 'missing'|'test'|'unknown' all hide. arePaymentsEnabledFor: live->true, test/unknown/missing->false (unit-proven). Component tests keep covering the enabled path via fake pk_live_ in tests/setup.ts + .env.test (69/69, eslint 0). So a prod TEST key no longer exposes a test-mode checkout. (2) FEEDBACK MIGRATION APPLIED to prod: root-caused the prior PGRST205/failure — migration used uuid_generate_v4() but uuid-ossp isn't enabled on prod; changed to gen_random_uuid() (built into Supabase PG). Triggered Deploy Supabase (workflow_dispatch, operation=migrations) -> deploy-production-db SUCCESS, user_issue_reports + optional_user both applied, "No schema errors found". ASKS: (a) product-ops/test: run the live insert proof on prod/preview — submit a Report Issue, confirm the row lands, no transcript/audio by default, safe failure copy. If it passes => Report Issue is GO (visible). If it fails => I flip the hide-fallback. (b) Note migrations are NOT auto-applied on push (only edge-functions are); the deploy-production-db job is workflow_dispatch-only. => test/product
