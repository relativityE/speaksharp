...
- **Deploy-version signal (there is NO `version.json` endpoint):** the deployed commit SHA is baked in at build time as the `__BUILD_ID__` Vite `define` = `BUILD_ID ?? VERCEL_GIT_COMMIT_SHA ?? GITHUB_SHA ?? timestamp` ([frontend/vite.config.mjs](../frontend/vite.config.mjs)). On Vercel prod it resolves to `VERCEL_GIT_COMMIT_SHA`. It surfaces at runtime as `appRuntimeConfig.release` ([frontend/src/config/appRuntimeConfig.ts](../frontend/src/config/appRuntimeConfig.ts)) and as PostHog `release_sha`. Verify deploy/SHA-equality by reading `__BUILD_ID__` from the deployed bundle or `appRuntimeConfig.release`. **Caveat:** `vercel.json` rewrites every unmatched path to `index.html`, so an HTTP `200` from an arbitrary URL (e.g. `/version.json`) is **NOT** endpoint proof — it may be the SPA fallback returning HTML. Any future version endpoint must be **build-generated** (not a committed static file) and its health check must assert `Content-Type: application/json` + expected schema + exact SHA equality, never `200` alone.
