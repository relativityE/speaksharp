...
> **Sanitation note (#1148):** references to a third-party, unaffiliated domain (PO determination, #1123) were
> redacted from this and other repository documents/evidence — historical findings are preserved; account
> identifiers were redacted to the reserved `example.com` test domain and production URLs to the approved
> release-proof host `https://speaksharp-public.vercel.app`. Pre-existing synthetic accounts under the old
> domain are recorded for the authorized #1146 cleanup (not deleted here). A CI scanner enforces zero
> reintroduction.
...
| PL-005 | Billing failure/cancel/downgrade lifecycle | P0 | Stale Pro access or wrong downgrade is trust/billing risk. | Canceled, failed, duplicate, and replayed payment states keep entitlement correct. | PASS IN TEST MODE / LIVE SIGNATURE VERIFIED AT ACTIVATION | Local webhook tests prove cancellation, unpaid, past_due, 3+ payment failures, skipped duplicate events, and RPC failure handling — the accepted functional proof. Live signed cancel/failure events are verified as activation-configuration at authorized activation (real Stripe webhook signing secret or Stripe test API access); this is not a required real-money proof. |
...
### PL-004 Activation-Configuration Verification (post-authorization; not a money-proof)

The Stripe test-mode checkout proves the entitlement path executes correctly in the current environment — the accepted functional proof (`RELEASE_CLOSEOUT_LEDGER.md` §D). Broad public launch does **not** require re-running this proof with live Stripe keys or a real payment; it requires the full activation contract (both payment switches ON + aligned live keys/webhook/prices + webhook/price/entitlement verification + written Prod Owner authorization). The items below are **activation-configuration verification** (or optional post-activation smoke), **not** a required real-money proof:

| Activation-config item | Disposition |
|---|---:|
| Live Stripe Checkout session `cs_live_...` | Optional post-activation smoke — not a required launch proof (test-mode journey is the accepted proof) |
| Live webhook signature verification from a production Stripe event | Activation-configuration verification at authorized activation — not a required real-money proof |
| Live paid user remains Pro after refresh/logout/login | Covered by the test-mode entitlement proof (PL-004 above); a live re-check is optional post-activation diligence, not a required proof |
...
|---|---|---|
| PL-003 / PL-004 / PL-005 Live Stripe configuration (Ops cutover) | The checkout→webhook→billing-portal **journey is PROVEN with Stripe TEST-mode keys = the accepted proof** (`RELEASE_CLOSEOUT_LEDGER.md` §D); live keys are not money-tested and not required as a proof. Paid launch still requires the full activation contract, **not merely a key swap**. | **Ops launch-day step (not a Dev/QA proof):** enable **both** payment switches (`VITE_PAYMENTS_ENABLED=true` **and** `PAYMENTS_ENABLED=true`), set aligned `sk_live`/`pk_live`/live `whsec`/live price IDs in prod env, register the live webhook endpoint in the Stripe LIVE dashboard, then verify `window.__APP_RUNTIME_CONFIG__.stripeKeyClass==="live"` plus webhook/price/entitlement, under written owner authorization. Either payment switch OFF keeps checkout closed. The "Pending live keys/events" rows below are part of this activation, not an outstanding proof. |

## PL-007 Cloud Transcript Attempt

| Step | Result | Evidence |
|---|---:|---|
| Login as Pro account | PASS | Pro account `manual-pro-cloud-20260515@example.com` created through `create-user.yml` run `25946599064`; Cloud-ready screenshot `/private/tmp/speaksharp-real-mic-cloud-ready.png`. |
