...
| P0 | Private browser transcript + artifact path | ✅ Complete | Run `25634578516` passed `live-private-artifact` on `310ded8d`: fresh trial Pro signup, Private CPU transcript, stop/save, analytics history, real Gemini AI feedback, and real PDF export artifact. PDF proof: `session_20260510_170412_cc4d4704_9417_485f_8a30_fd0dcac007a1.pdf`, parsed `textLength: 733`, `textIncludesTranscript: true`. |
...
| P0 | Full Live Release Matrix | 🔴 Failed, narrowed | Run `25634760761` failed only `live-cloud-artifact`. Green in the same run: automatic trial entitlement, `live-native-preflight`, `live-stripe-webhook-readiness`, `live-custom-words`, `live-stripe-checkout-readiness`, `live-cloud-token-gates`, and `live-private-artifact`. |
...
| P0 | Trial abuse/throttle | ✅ Complete | Run `25635969309` passed `live-trial-abuse` on `82a5c521`: 9 invalid attempts returned statuses `[400,400,400,400,400,400,400,400,429]` with final error `Too many trial attempts. Please try again later.` |
...
| Basic user | Start/stop/save allowed within quota | Pro-gated with clear upgrade/trial path | Pro-gated with clear upgrade/trial path | Pro-gated with clear upgrade/trial path | Validate after deploying `20260521000000_basic_tier_cutover.sql`; no runtime path should emit the old unpaid-tier value. |
| Future paid Basic user | Start/stop/save allowed within paid baseline quota | Pro-gated with clear upgrade/trial path | Pro-gated with clear upgrade/trial path | Pro-gated with clear upgrade/trial path | Paid Basic pricing remains parked until current tester release is stable. |
| Trial Pro tester | Start/stop/save allowed | Required: CPU-first setup, transcript, save, history | Manual accelerated-path evidence only | Explicitly selectable; token must be usage-gated | Primary human tester route. |
...
| Expired trial user | Continue as baseline tier or enter new trial/upgrade | Blocked with clear expired-trial copy | Blocked with clear expired-trial copy | Blocked with clear expired-trial copy | Verify no stale Pro access. |
...
| Trial path | ✅ Live artifact path complete | Run `25634578516` passed `live-private-artifact`: one-time trial Pro signup, Private transcript, stop/save/history, real Gemini AI feedback, and timestamped PDF artifact export. |
...
| Full trial canary treats benign Private model/ONNX warnings as fatal diagnostics. | Full trial canary failed after Pro activation/return succeeded. | ✅ Focused deployed trial canary passed 3/3 with fresh account `7543246` after allowlisting known benign Private startup/ONNX diagnostics. Keep under regular CI/canary monitoring. |
| Usage Edge CORS must remain request-aware. | Hardening consistency with `check-usage-limit`; not a quota/privacy/session-save P0. | 🟡 Local fix applied: shared request-aware CORS; deploy/header validation pending. |
| Local focused E2E cannot bind preview server in Codex sandbox. | `CI=true pnpm exec playwright ... trial-onboarding-journey` failed before browser execution with `listen EPERM: operation not permitted 127.0.0.1:4173`. This is not product evidence because the app server never started. | ⚪ Track as tooling friction. Revisit only if the same bind failure reproduces from normal macOS Terminal or GitHub Actions. |
