...
| P0 | Private browser transcript + artifact path | ✅ Complete | Run `25634388146` passed `live-promo-private-artifact` on `0f185663`: fresh promo Pro signup, Private CPU transcript, stop/save, analytics history, AI feedback request, and export control assertion. Trace shows model inference succeeded with `textLength: 78` and transcript callback/store apply. Artifact inspection found video/trace/screenshot, not a saved `.pdf` file. |
| P0 | Full Live Release Matrix | 🔴 Failed, narrowed | Run `25634388146` failed only `live-cloud-artifact`. Green in the same run: `live-promo-private-artifact`, `live-stripe-webhook-readiness`, `live-stripe-checkout-readiness`, `live-custom-words`, `live-cloud-token-gates`, `live-native-preflight`, and `generate-promo`. |
...
| P1 | PDF/export and AI feedback artifact path | ✅ AI feedback complete / 🟡 PDF file artifact pending | Run `25634388146` passed `live-promo-private-artifact`. `GET_AI_SUGGESTIONS_LIVE_RESPONSE` returned HTTP 200 with content-specific AI suggestions and no `degraded: true` marker in the logged response slice, proving the Supabase `GEMINI_API_KEY` path rather than the missing-key fallback. Export control assertion passed, but downloaded artifacts contained no saved `.pdf`; add explicit PDF artifact capture before claiming a PDF file export. |
