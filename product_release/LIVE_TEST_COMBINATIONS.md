...
| Area | Status | Notes |
|---|---|---|
| Automated CI baseline | 🔴 Latest GitHub CI failing / logs pending | Latest pushed commit is `e177bc7c`. `CI - Test Audit` run `25623359127` is still in progress, but shard 4 has already failed and shard 1 is still running at last check. GitHub will not expose logs until the run completes. Focused local validation for the shard-4 user-features fix passed 7/7 before push, so this may be another CI-only harness issue or a different shard-4 assertion. |
| Production canary | ✅ Passed on latest pushed commit | `Production Canary Smoke Test` run `25623359125` passed on `e177bc7c` in 1m52s. Edge Function deploy run `25623359118` also passed on the same commit. |
...
| Custom words persistence | 🟡 Mocked + canary Cloud keyterms pass / logout-login persistence pending | Mocked E2E add/remove/detection passed 7/7. Production canary covers live Cloud keyterms with `CANARY_PASSWORD`. Local `.env*` files do not contain `E2E_FREE_EMAIL`/`E2E_FREE_PASSWORD`, so `tests/live/user-filler-words-persistence.live.spec.ts` cannot run locally without temporary env. | Mark for next session: add a GitHub workflow/manual dispatch or secret-backed job that runs `VITE_USE_LIVE_DB=true tests/live/user-filler-words-persistence.live.spec.ts` with `E2E_FREE_EMAIL` and `E2E_FREE_PASSWORD`. |
...
| Live promo Pro Private transcript stayed at placeholder. | Deployed live promo artifact test with fresh one-time code `9192136` reached Pro session and recording active, but transcript remained `Listening...` for 90s; save/history/PDF assertions were not reached. | 🔴 LIVE BLOCKER: Private browser transcript path remains unproven despite DB/RPC private save working. Needs trace/log inspection and a fresh deployed retest; do not mark Private CPU browser path green yet. |
