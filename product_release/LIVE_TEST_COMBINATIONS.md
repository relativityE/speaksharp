...
| P1 | Cloud live analytics/session path | 🔴 Product artifact path pending | Latest Cloud evidence moved through several buckets: run `25643057818` proved token 200/WebSocket open but failed on AssemblyAI 3007 tiny-chunk input duration; commit `da94963d` fixed Cloud PCM chunking to 100 ms / 1600 samples. Run `25643435705` then produced real Harvard transcript text but stopped before the 5 second save policy. Run `25643916971` failed before transcription because the Cloud selector raced Pro policy sync. The selector helper now waits up to 45 seconds for Pro STT options. Full artifact matrix run `25644204999` did not evaluate Cloud because Private failed first; focused Cloud-only run `25644479281` is now running via `mode=cloud`. |
...
| Cloud path | 🔴 Token gates + WER green / product artifact path pending | Run `25634760761` passed `live-cloud-token-gates`. GitHub `STT Ceiling Benchmarks` run `25643746457`, job `75268546634`, proved AssemblyAI WER at `0.00%` across all 10 Harvard WAV fixtures. Product live path remains blocked until the app records Cloud, shows fixture transcript, stops after the 5 second save policy, saves, opens analytics, gets AI feedback, and exports PDF. The latest full matrix did not run Cloud because Private failed first; focused Cloud-only run `25644479281` is in progress on commit `bd17ed50`. |
