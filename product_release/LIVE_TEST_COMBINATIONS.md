...
| Area | Status | Notes |
|---|---|---|
| Automated CI baseline | 🟢 Passing on latest pushed commit | `CI - Test Audit` passed on `744fea8` (run `25607518542`) after the session-nav expectation fix. Current pause/Cloud audio pump and expired-promo dialog fixes are local and need the next push/rerun. |
| Production canary | 🟢 Passing on `744fea8` | GitHub production canary passed on `744fea8` (run `25607518535`); Edge Function deploy passed on the same push (run `25607518527`) and Supabase migration deploy passed on `7da01c8` (run `25606499810`). Keep required for deployed smoke. |
...
| Analytics return path | 🟡 Local patch / live validation pending | CI is green on `9dff649`. A newer local analytics correctness patch preserves persisted WPM/clarity values in comparisons. Local mic-frame pump fix now lets Native/Cloud pause metrics receive real audio frames instead of staying flat. Live returning-user comparison and input-change proof are still pending. |
