...
| P0 | Stripe live webhook | 🟡 Test-mode readiness proved / signed proof rerun pending | Run `25634760761` passed `live-stripe-webhook-readiness`: deployed Supabase webhook returned HTTP 400 `STRIPE_WEBHOOK_INVALID` for an unsigned event, proving runtime config/import readiness instead of missing config. Run `25634938426` failed the first signed no-op proof before raw-body sending was fixed; follow-up patch sends the signed payload as raw bytes. Rerun pending. |
...
| P1 | Cloud live analytics/session path | 🔴 Provider new-session rate-limit pending cooldown rerun | Run `25634760761` proved the Cloud harness is serialized (`--workers=1`, job concurrency lock) and still failed with AssemblyAI `Unauthorized Connection: Too many concurrent sessions`. Logs show app teardown fired (`Nuclear termination requested`, `Stopping connection`, `Destroying engine`, `destroy() complete`). Classify as provider/account new-session rate limiting unless it reproduces after a 5-10 minute cooldown with only one Cloud smoke running. This is not transcript correctness evidence. |
...
| Cloud path | 🔴 Token gates green / streaming artifact blocked by provider rate limit | Run `25634760761` passed `live-cloud-token-gates`, but `live-cloud-artifact` failed after the v3 endpoint fix because AssemblyAI returned `Too many concurrent sessions`. The serialized harness logged explicit teardown before failing the classified provider new-session rate-limit error. Wait 5-10 minutes and rerun one Cloud smoke only before escalating to app leak. |
