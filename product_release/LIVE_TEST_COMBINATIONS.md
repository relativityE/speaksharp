...
| Area | Status | Notes |
|---|---|---|
| Automated CI baseline | 🟢 GitHub CI green on `9dff649` | `CI - Test Audit` passed on `9dff649` (`exclude forensic probes from release e2e`, run `25602605844`). This proves mocked orchestration only; it does not make any live STT path green. |
| Production canary | 🟢 Passing on `9dff649` | GitHub production canary passed on `9dff649` (run `25602605839`) after the route/runtime fixes; Edge Function deploy also passed on the same push (run `25602605834`). Keep required for deployed smoke. |
