...
| P0 | Full Live Release Matrix | 🔴 Failed, narrowed | Run `25634578516` failed only `live-cloud-artifact`. Green in the same run: `generate-promo`, `live-native-preflight`, `live-stripe-webhook-readiness`, `live-custom-words`, `live-stripe-checkout-readiness`, `live-cloud-token-gates`, and `live-promo-private-artifact` with PDF artifact proof. |
| P0 | Cloud quota/token abuse gates | ✅ Complete | Run `25634388146` passed `live-cloud-token-gates`, covering the live Cloud token gate matrix including denial cases and active Pro token issuance. Keep in release reruns. |
...
| P1 | Cloud live analytics/session path | 🔴 Provider concurrency/leak classification pending rerun | Run `25634388146` selected Cloud, reached `RECORDING`, and sent audio chunks over the updated v3 WebSocket path, so the old `/v3/realtime/ws` 404 is cleared. New failure: AssemblyAI returned `Unauthorized Connection: Too many concurrent sessions`; transcript remained `Listening...` and no session persisted. Harness patch in progress serializes the Cloud streaming job, forces `--workers=1`, explicitly stops after each test, waits for closed/disconnected state, and classifies this error as provider concurrency/leaked-session evidence rather than transcript correctness failure. |
...
| P1 | Observability | 🟡 Partial | Frontend Sentry transport returned HTTP 200; Stripe test-mode webhook/checkout readiness passed in run `25634150689`; Sentry dashboard visibility, Edge Function Sentry/backend ingest, PostHog launch events, and signed Stripe delivery remain pending. Sentry is not fully tested yet. |
...
| Area | Status | Notes |
|---|---|---|
| Automated CI baseline | 🟡 Latest running / prior green | `CI - Test Audit` run `25634361733` passed on `0f185663`. Latest run `25634570777` on `310ded8d` is still in progress. |
| Production canary | ✅ Latest green | `Production Canary Smoke Test` run `25634570757` passed on `310ded8d`. Deploy Edge Functions run `25634570774` also passed on the same commit. |
...
| Private CPU path | ✅ Browser artifact path complete | Run `25634578516` produced Private CPU transcript, analytics/history, real Gemini AI feedback, and parsed PDF artifact text. Separate cache first/second-start proof remains pending under the Private cache matrix. |
...
| Cloud path | 🔴 Token gates green / streaming artifact blocked by provider concurrency | Run `25634578516` passed `live-cloud-token-gates`, but `live-cloud-artifact` failed after the v3 endpoint fix because AssemblyAI returned `Too many concurrent sessions`. The current harness patch serializes the Cloud streaming job, runs one worker, stops explicitly, waits for disconnected/closed state, and classifies future concurrency hits as provider limit or leaked-session evidence. |
