...
| Area | Status | Notes |
|---|---|---|
| Automated CI baseline | 🟡 Latest rerun required | `CI - Test Audit` passed on `9dff649` (`25602605844`), but the newer `7da01c8` run exposed an Edge Function Deno test import issue around `npm:stripe@16`. Local fix lazy-loads Stripe/Supabase in the webhook runtime and `pnpm test:edge` passes 8 files / 36 steps; GitHub rerun is required before this returns green. |
| Production canary | 🟢 Passing on `7da01c8` | GitHub production canary passed on `7da01c8` (run `25606491521`); Edge Function deploy passed on the same push (run `25606491505`) and Supabase migration deploy passed on `7da01c8` (run `25606499810`). Keep required for deployed smoke. |
...
| Analytics return path | 🟡 Local patch / live validation pending | CI is green on `9dff649`. A newer local analytics correctness patch preserves persisted WPM/clarity values in comparisons, but live returning-user comparison is still pending. |
| Expired promo path | 🟡 Local fixes / deploy + live smoke pending | 2026-05-09 visible-browser test found the expired Pro dialog could trap testers and block sign-out/signup navigation. Local fix adds `Switch account` plus targeted component coverage; deploy/live verification pending. Local backend fixes deny expired promo-only Cloud token issuance and treat expired promo-only users as Free for DB session/heartbeat paths. |
