...
**Last Verified:** 2026-07-30 — artifact locations enumerated from the live `product_release/evidence/` tree and the retained-evidence destinations recorded in `DOC_MIGRATION_LEDGER.md` §2 / §3.I. This file indexes historical evidence; it carries no current release posture.
...
**Supersedes:** the ad-hoc "current truth source" framing of `evidence/README.md`, the dated-proof rows of `RELEASE_CLOSEOUT_LEDGER.md`, and the dated evidence summaries of `PUBLIC_LAUNCH_LEDGER.md` / `ENTITLEMENT_PRO_LIMIT_EVIDENCE.md` (indexed here as retained evidence per `DOC_MIGRATION_LEDGER.md` §3.I).
**Evidence Sources:** the `product_release/evidence/` tree itself; `DOC_MIGRATION_LEDGER.md` §2 / §3.I retained-evidence mapping; `attribution-sanitation-crosswalk.md` for pre-2026-07-15 SHA provenance.
...
## 4. STT & UX reproof runs (dated)

Point-in-time STT accuracy/latency and UX-resilience reproofs. STT interpretation is owned by `STT.md`; these are the raw dated captures.

| Artifact | Date | What it captured |
|---|---|---|
| `product_release/evidence/stt_product_metrics_release_matrix_2026-06-02.json` / `.md` | 2026-06-02 | STT product-metrics release matrix (per-engine speed/accuracy snapshot). |
| `product_release/evidence/private_v2_human_bakeoff_2026-06-05.json` | 2026-06-05 | Private v2 human-speech bakeoff. |
| `product_release/evidence/private_v2_mtwasm_human_2026-07-29.json` | 2026-07-29 | Private v2 multithreaded-WASM human-speech reproof. |
...
| `product_release/evidence/stt_p5_vad_b6159_reproof_2026-06-05.json` / `stt_p5_vad_c5f3_reproof_2026-06-05.json` / `stt_p5_vad_current_main_reproof_2026-06-05.json` | 2026-06-05 | Phase-5 VAD reproof set (two pinned variants + current-main). |
| `product_release/evidence/stt_p6_base_default_70746410_reproof_2026-06-05.json` | 2026-06-05 | Phase-6 base-default reproof. |
| `product_release/evidence/stt_p6_conv02_edge_bakeoff_2026-06-05.json` | 2026-06-05 | Phase-6 conv-02 edge bakeoff. |
| `product_release/evidence/stt_p6_enriched_base_rc_2026-06-05.json` | 2026-06-05 | Phase-6 enriched-base RC reproof. |
| `product_release/evidence/stt_p6_human_bd94_base_selector_2026-06-05.json` / `stt_p6_human_rc_base_selector_2026-06-05.json` | 2026-06-05 | Phase-6 human base-selector reproofs. |
| `product_release/evidence/stt_v4r_exact_contract_probe_2026-06-05.json` | 2026-06-05 | v4 exact-contract probe (v4 hard-off exploration). |
| `product_release/evidence/stt_v4r_fp32_lifecycle_2026-06-05.json` | 2026-06-05 | v4 fp32 lifecycle probe. |
| `product_release/evidence/stt_v4r_realbrowser_devprobe_2026-06-05.json` | 2026-06-05 | v4 real-browser dev probe. |
| `product_release/evidence/stt_v4r_stable_wasm_reproof_2026-06-05.json` | 2026-06-05 | v4 stable-WASM reproof. |
...
| `.github/workflows/stt-runtime-evidence.yml` + `scripts/private-v2-worker-evidence.mts` (PR #1127) | Private-v2 production-worker runtime | Emitted production worker/WASM, self-hosted immutable model/config equality, coherent PCM boundary, single-thread fallback, and zero external/application writes for the fixed fixture | Not a microphone journey, p95, minimum-device study, or broad accuracy result; requested, configured, and worker-reported thread counts remain distinct; the sanitized diagnostic artifact is retained for one day. |
...
## 6. Historical STT release reports (`test_reports/`)

Root-cause archaeology only; not authoritative after newer proof exists (per the archive rules absorbed into §2).

| Artifact | Date | What it captured |
|---|---|---|
| `product_release/evidence/test_reports/PRIVATE_STT_RELEASE_EVIDENCE_2026-06-02.md` | 2026-06-02 (updated 2026-06-04) | Private v2 local/browser STT: setup consent, accuracy, trust UI, save/history/detail. |
| `product_release/evidence/test_reports/CLOUD_STT_RELEASE_EVIDENCE_2026-06-02.md` | 2026-06-02 | Cloud STT release evidence. |
| `product_release/evidence/test_reports/NATIVE_STT_RELEASE_EVIDENCE_2026-06-02.md` | 2026-06-02 | Native (Browser) STT release evidence. |
| `product_release/evidence/test_reports/STT_SPEED_ACCURACY_MARKET_SURVIVAL_REVIEW_2026-06-02.md` | 2026-06-02 | STT speed/accuracy market-survival review. |

---

## 7. Retained evidence indexed from other sources

These are dated proofs that live in interim files today and move under `evidence/` at documentation closeout (per `DOC_MIGRATION_LEDGER.md` §2 / §3.I). Indexed here as historical evidence; the current interpretation is owned by the canonical doc named.

| Retained-evidence destination | Captured content | Interpretation owned by |
|---|---|---|
| `evidence/PUBLIC_LAUNCH_LEDGER.md` (moves from root at closeout) | Dated broad-launch gate proofs PL-001…PL-011: public signup, first Free session, test-mode Stripe checkout→entitlement journey (accepted as functional proof), webhook/billing-lifecycle local proofs, trial lifecycle, real-mic Cloud proof (2026-05-15), AI feedback, PDF export, mobile-viewport baseline, observability/support smoke. | `ROADMAP.md` (open gates) + `RELEASE_STATUS.md` (current) |
