...
| `evidence/retained/2026-09-01-codex-code-review-intake.md` (SHA-256 `3299feb1c04ebe8d2c2bd790f88e0c4ad4c9be1a8fe9904463c8fece668d5b35`; added by `463453dc7`) | Point-in-time raw register of Codex inline review findings, generated 2026-09-01 UTC from the latest 100 pull requests by update time: 77 PRs, 206 findings, sourced from inline review threads authored by `chatgpt-codex-connector`. **Not authoritative:** a listed finding is not evidence that it is reachable on current `main`, and the register is not an implementation queue — each finding needs a current triage state (`confirmed-current`, `fixed-verified`, `superseded`, …) before any work is promoted from it. | PR #1399 triage (per-finding disposition and the durable issues it creates) + exact current-`main` evidence for reachability |
