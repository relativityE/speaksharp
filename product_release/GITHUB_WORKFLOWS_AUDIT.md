...
|---|---|---|---|---|---|---|
| `.github/workflows/ci.yml` | Primary quality gate: prepare, mocked E2E shards, Lighthouse, SQM/report aggregation. | Push, PR, manual | Yes | 🟢 PASSING ON `9dff649` | **Keep required** | `CI - Test Audit` passed on `9dff649` (`exclude forensic probes from release e2e`, run `25602605844`). This proves the mocked automated gate, not the live feature matrix. |
| `.github/workflows/canary.yml` | Production deployed smoke: provision canary user, login, Native session, save/read. | Main push, daily schedule, manual | Yes | 🟢 PASSING ON `9dff649` | **Keep required** | Production canary passed against `https://speaksharp-public.vercel.app` on `9dff649` (run `25602605839`); keep as deployed smoke because it caught real route/runtime drift. |
...
| `.github/workflows/deploy-edge-functions.yml` | Deploy Edge Functions on main push/manual. | Main push, manual | Yes if used instead of migration workflow deploy step | 🟡 PATCHED / LATEST RUN PENDING | **Consolidate decision needed** | Edge Function deploy passed on `9dff649` (run `25602605834`). Local workflow patch adds previously omitted `get-ai-suggestions` and `stripe-checkout`; latest run evidence pending. Explicit owner still needed: either this deploys functions, or migration workflow does, not both ambiguously. |
...
| Edge/Deno tests absent from required CI | Edge Function Deno tests were runnable locally but not part of `.github/workflows/ci.yml`. | Runtime/security regressions in quota, token, promo, webhook, or AI functions could miss the main gate. | Local patch adds `pnpm test:edge` and an `edge-tests` CI job after `prepare`; latest GitHub evidence pending. |
...
Workflow evidence status on 2026-05-09: GitHub `CI - Test Audit` passed on `9dff649`, production canary passed on the same push, Edge Function deploy passed on the same push, and Supabase migration deploy has successful manual runs from 2026-05-08. This updates the workflow gate, but it does not mark live feature paths green.
