...
when `durationSeconds >= 30` **and** `compositeQuality(s) != null` (`utils/aggregateProgress.ts:24,111`). A
...
- **Pro-interest capture.** No `pro_interest` object exists in `backend/`, there is no reachable frontend
  action and no submission journey. The only non-test frontend reference is a **comment** in
  `ObjectiveSetupDialog.tsx:10`.
