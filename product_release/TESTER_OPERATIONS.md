...
- **PostHog flags (owner-configured) — inspect the control plane for every current value.** `private_stt_v4_enabled` is the control-plane mechanism: per-user targeting via a `distinct_id` (= Supabase `user.id`) condition, plus a broad-rollout percentage. **Read the current rollout percentage, the targeted `distinct_id`s, and which v4 flags actually exist directly from PostHog before acting — none of those values are recorded here, because all of them change independently of this procedure.** First-wave exposure is performed by adding test/owner `distinct_id`s to the flag condition. Kill switch / rollback = clear the targeted `distinct_id`s and set the broad rollout to zero → new users get v2 immediately; existing saved sessions keep their recorded arm.
...
- **Phase 0 — internal proof:** force v2 + v4 on owner/test accounts; confirm setup, transcript, save/history/detail, Report Issue, telemetry + saved metadata. *(Whether this phase has been completed is a status fact — check `RELEASE_STATUS.md` and the dated artifacts indexed by `EVIDENCE_INDEX.md`, not this procedure.)*
