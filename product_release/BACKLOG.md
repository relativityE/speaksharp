...
| P0 | FEEDBACK-1 | In-app issue-report flow is implemented; production migration is recorded as applied, but live submit proof is still missing. | Earlier proof failed with `PGRST205` because `public.user_issue_reports` was absent. Current coordination record says the migration was applied after switching default UUID to `gen_random_uuid()` (`main@828af8b3`, deploy-production-db success per dev note). Public read-only probe found no unauthenticated Report Issue entrypoint on `/`, `/pricing`, `/auth/signup`, or redirected `/session`. | test-release-agent/product: run authenticated app-path proof: submit issue, confirm row lands, required metadata present, transcript/audio fields null unless opted in, and safe failure copy if the backend rejects. Product-ops only reopens if this proof shows the migration still absent. |
