...
| P1 | FEEDBACK-1 | In-app issue-report flow implemented; needs backend migration/deploy proof. | Test-release-agent added `user_issue_reports` Supabase migration, authenticated nav "Report issue" dialog, automatic metadata capture, and hard transcript/audio opt-in boundaries. Focused tests passed `17/17`; typecheck passed. | Deploy/apply migration, verify an authenticated user can submit a report, confirm backend row includes metadata, and confirm transcript/audio fields are null unless opted in. |
