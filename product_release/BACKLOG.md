...
| P1 | BLOAT-TURBO-ASSETS | ✅ DEV DONE (test verify+merge) — full retirement on `dev/release-closure@2b928fd9`. | Per product decision "delete + accept risk", retired (not quarantined) as one coherent change: 34 files, -1654/+41; ~57 MB assets removed (`tiny-q8g16.bin` 49 MB + turbo wasm/js ~6 MB) + 93 pkgs (`whisper-turbo`/`whisper-webgpu` + transitive). Removed: `WhisperTurboEngine.ts`, `WhisperEngineRegistry.ts`, PrivateSTT selection/promotion, sttProviderConfig entry, sw.js mapping, vite/vitest turbo config, predev/prebuild hooks, dup mocks, patch. Behavior-preserving (turbo only promoted when model pre-cached → never in prod). Proof: tsc clean; affected vitest 254/254 (coverage-off); `pnpm build` exit 0 with NO turbo chunk; no dangling imports. | Test verify+merge per ACTIVE_COORDINATION acceptance (v2 works, v4 still off-by-default + opt-in, no turbo refs in `dist`/deps, no UX regress). Inert `'whisper-turbo'` literal kept in EngineType/PrivateSttProvider unions + privateRuntimePath WebGPU tier + ModelManager IndexedDB probe → follow-up `BLOAT-TURBO-LITERAL`. |
| P3 | BLOAT-TURBO-LITERAL | Post-beta cleanup: remove the now-inert `'whisper-turbo'` string residue left after BLOAT-TURBO-ASSETS. | Kept deliberately to avoid churning ~10 test files that use `'whisper-turbo'` as a generic mock-engine label: (a) `'whisper-turbo'` literal in `EngineType` (`IPrivateSTTEngine.ts`) + `PrivateSttProvider` (`providers/types.ts`); (b) `privateRuntimePath.ts` WebGPU tier (pure decision fn; promotion never enabled — `webgpuPromotionAllowed:false` always); (c) `ModelManager` `isWhisperTurboDownloaded` + `hasIndexedDbStoreEntries` + `WHISPER_TURBO_*` constants (no caller; CacheStorage path unaffected); (d) inert dead branches in PrivateSTT `getPrivateProviderIds`/`checkAvailability`. All compile + tests green; none reachable at runtime. | Post-beta: remove the literal from the unions, then rewire the ~10 mock-label test usages to `'transformers-js'`, drop the ModelManager IndexedDB turbo probe + its `ModelManager.test.ts` block, and simplify the privateRuntimePath tier. Not a beta blocker. |
| P1 | BLOAT-PRIVATE-FALLBACK | Main-thread Private fallback conflicted with no-HF/base-default posture. | Original `frontend/src/services/transcription/engines/TransformersJSEngine.ts` main-thread path hardcoded local `whisper-tiny.en`; on local load failure it enabled remote models and loaded `Xenova/whisper-tiny.en`. Dev fixed strict no-HF fallback on `dev/release-closure@b579c3a4`; targeted test `TransformersJSEngine.test.ts` passes 23/23 with no-HF assertions. Browser/network proof `/private/tmp/TEST_PRIVATE_NO_HF_BROWSER_PROOF_2026-06-08.md` PASS on `localhost:5174`: Private base identity local/default, seven `/models/whisper-base.en` requests from origin, zero Hugging Face/HF requests, no fallback. New nuance: ORT WASM runtime fetched from `cdn.jsdelivr.net`, which is not HF and does not upload audio, but means the path is not fully same-origin runtime self-hosted. | Close after merge/deploy reproof if product claim is "no Hugging Face model weights." If product/dev require "all Private runtime/model assets are self-hosted," dev follow-up: serve ORT WASM from origin via `env.backends.onnx.wasm.wasmPaths` or equivalent bundling, then rerun browser/network proof. |
| P1 | BLOAT-PAYMENT-CTA | Some upgrade CTAs remained visible when payments were disabled/non-live. | Core payment enablement is correctly `pk_live_` gated. Dev committed `dev/release-closure@d7cd305c`: `AnalyticsPage`, `AnalyticsDashboard`, and `SunsetModals` now render-gate upgrade CTAs on `arePaymentsEnabled()`. Focused unit/component tests passed in the handoff. Browser proof `/private/tmp/TEST_PAYMENT_DISABLED_BROWSER_PROOF_2026-06-08.md` PASS on `localhost:5174` with `stripeKeyClass="test"`: public `/pricing`, public home, and authenticated `/analytics` expose no checkout/upgrade controls and make zero `stripe-checkout` requests. | Code-side and local browser proof done; close only after merge to main and deployed-release reverify. |
| P1 | BLOAT-CONFIG-REQUIRED-COPY | Configuration fallback page copy was stale after config-gate-relax, and build-time validation still had the old required-env policy. | Boot now only requires `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY`, but `ConfigurationNeededPage` originally told users/testers `VITE_STRIPE_PUBLISHABLE_KEY` and `VITE_SENTRY_DSN` were required. Test-agent fixed the page/test on `dev/release-closure`: only Supabase URL/anon are listed as required; Stripe/Sentry are described as optional for startup, with payment surfaces hidden unless a live Stripe key is configured. Focused proof passed: `ConfigurationNeededPage`, runtime config, payment/analytics/upgrade surfaces `73/73`. Original clean-worktree mismatch proof `/private/tmp/TEST_VALIDATE_ENV_MISMATCH_2026-06-08.md`; dev fix verified by `/private/tmp/TEST_VALIDATE_ENV_FIX_PROOF_2026-06-08.md`: Supabase-only `pnpm validate-env` passes with optional Stripe/Sentry warnings, missing Supabase URL fails, and missing Supabase anon fails. | Code + branch verification done; close after merge to main and deploy/build recheck. |
| P1 | BLOAT-NATIVE-FROZEN-COPY | Native watchdog exposed scary “Engine Frozen” user-facing text. | Prior human proof observed `Speech recognition is taking a moment (Engine Frozen)` during successful Native recognition. Dev committed `dev/release-closure@d7cd305c`: Native heartbeat drift is logged but no longer surfaces the scary user-facing frozen warning; non-Native warning copy is calmer. Fresh unit proof `/private/tmp/TEST_NATIVE_ENGINE_FROZEN_UNIT_PROOF_2026-06-08.md` passes 3/3, including the Native-specific assertion that heartbeat drift does not emit `{ isFrozen: true }`. | Code-side and unit proof done; test must browser/manual proof healthy Native recognition shows no scary frozen toast after merge. |
...
| P2 | BLOAT-FORMAT-TRANSCRIPT | Complex Native formatter/Gemini path conflicts with Native-as-quick-start beta framing. | Test-agent disposition saved at `/private/tmp/TEST_FORMAT_TRANSCRIPT_DISPOSITION_2026-06-08.md`. This path is not dead: `EngineFactory`/`NativeBrowser` register the production Native formatter, `SpeechRuntimeController` calls `nativeAsyncFormatter`, `nativeGeminiFormatter` calls Supabase `format-transcript`, the Edge Function/tests exist, and `.github/workflows/deploy-supabase-migrations.yml` deploys it. Product framing now says Native beta should be instant/browser-dependent/minimal-cleanup, with Private as the quality upgrade. | Dev/product decision: either hard-disable the Native Gemini formatter client call for beta and prove no Native save calls `format-transcript`, or explicitly approve server/LLM Native formatting and run real-mic quality/cost/quota proof. Test-agent recommendation for beta: hard-disable Native Gemini formatting and upsell Private for quality. |
...
| P2 | BLOAT-BUILD-WARNINGS | Production build passes with bundle-health warnings after env is supplied, but clean no-env build is blocked by stale `validate-env` optionality. | Test-agent build disposition saved at `/private/tmp/TEST_BUILD_WARNINGS_DISPOSITION_2026-06-08.md`. Clean `pnpm build` without env failed because `env.required` still hard-requires Stripe/Sentry; with dummy public env values build passed in 22.47s. Current warnings: stale Browserslist data, `onnxruntime-web` eval warning, empty `vendor-pdf` chunk, mixed static/dynamic imports preventing chunking, duplicate timestamped/non-timestamped WASM assets, >500 KB chunks (`main`, transformer bundles, `AnalyticsPage`), and turbo chunk still emitted before dev's active turbo retirement lands. | Dev second pass after `BLOAT-TURBO-ASSETS`: rerun build, decide accepted warnings vs fixes, confirm whether duplicate WASM and empty `vendor-pdf` are intentional. Fix `validate-env` optionality before treating build hygiene as closed. |
...
| ID | Finding | Disposition | Found by |
|---|---|---|---|
| PARKED-ENGINE-QUARANTINE | whisper-turbo (WebGPU) cluster ✅ RETIRED in `dev/release-closure@2b928fd9` (BLOAT-TURBO-ASSETS): `models/tiny-q8g16.bin`, `whisper-turbo/*`, `WhisperTurboEngine.ts`, `WhisperEngineRegistry.ts`, `patches/whisper-turbo@0.11.0.patch`, deps `whisper-turbo`/`whisper-webgpu`, duplicate mocks, and the `whisper-turbo` provider entry all removed. **Still parked (KEEP for post-release):** `transformers-js-v4` provider (off by default; opt-in via explicit `forceEngine`) + `webgpuSupport.ts` (used by v4 + runtime-path resolver). | Turbo: done (test verify+merge). v4: keep quarantined for beta — not user-selectable / not in default bundle path — and enable post-beta per product ("need v4 working post initial release"). | both |
...
| PLAYWRIGHT-CONFIGS | 9 root playwright configs — confirm each is invoked; prune unused. | Test inventory artifact `/private/tmp/TEST_PLAYWRIGHT_CONFIG_INVENTORY_2026-06-08.md`: keep `playwright.base.config.ts`, `playwright.config.ts`, `playwright.live.config.ts`, `playwright.deployed-live.config.ts`, `playwright.canary.config.ts`, and `playwright.soak.config.ts`; archive/document candidates are `playwright.stripe.config.ts`, `playwright.demo.config.ts`, and `playwright.config.d.ts`; stale proof-path candidate is `tests/live/live-transcript.live.spec.ts` because it does not navigate before asserting. | dev/test second pass |
