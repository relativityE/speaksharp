...
| P1 | BLOAT-PAYMENT-CTA | Some upgrade CTAs remained visible when payments were disabled/non-live. | Core payment enablement is correctly `pk_live_` gated. Dev progress 2026-06-08: uncommitted `dev/release-closure` changes gate `AnalyticsPage` and `SunsetModals` on `arePaymentsEnabled()`; focused `AnalyticsPage` test passes. `AnalyticsDashboard` secondary action still needs final scan/proof after commit. | Dev/test: finish/commit render-gate changes, add/update regression coverage, and browser-proof payment-disabled surfaces do not render dead/no-op upgrade buttons. |
| P1 | BLOAT-CONFIG-REQUIRED-COPY | Configuration fallback page copy is stale after config-gate-relax. | Boot now only requires `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY`, but `ConfigurationNeededPage` still tells users/testers `VITE_STRIPE_PUBLISHABLE_KEY` and `VITE_SENTRY_DSN` are required. Focused component test still asserts the stale copy, so current tests are not sufficient. | Dev second pass: update copy/tests so the page lists only actual boot blockers and treats Stripe/Sentry as optional release/ops checks. |
| P1 | BLOAT-NATIVE-FROZEN-COPY | Native watchdog exposed scary “Engine Frozen” user-facing text. | Prior human proof observed `Speech recognition is taking a moment (Engine Frozen)` during successful Native recognition. Dev progress 2026-06-08: uncommitted `TranscriptionService.ts` change suppresses user-facing frozen warning for Native heartbeat drift and uses calmer copy for other modes; focused heartbeat test passes. | Dev/test: commit with tests, then browser/manual proof healthy Native recognition shows no scary frozen toast. |
