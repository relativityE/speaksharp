...
> ## P0/P1 disposition for Beta-50 (authoritative, 2026-07-10)
> Every P0/P1/P1-BLOCKER row in this file falls into exactly one of four dispositions. Read each dated row below through this lens; the individual dated rows are **preserved as historical evidence** and are **not** rewritten, so their proof dates stay intact.
> - **(d) STILL BLOCKING Beta-50 — the only open GO blockers:** the **Private v2 saved-transcript opening-clause fidelity drop** (#891/#892, reproduced on deployed via rc-gates Gate 3 DAST run `29125536866`, issue #960) and the **P0-validate immediate-start re-gate** (the same Gate-3 failure is its proof). These live in the "Active Private STT release work" section directly below. **No other row re-opens the GO decision.**
> - **(a) RESOLVED with evidence:** every row marked **✅ CLOSED …@`<sha>`** (Release Bloat inventory; Open Beta Closeout Findings; the paid-spine/webhook rows; PROD-CONFIG-1/PRIVACY-OBS-1/FEEDBACK-1; the May-30 and 2026-06-08 manual-STT ledger rows carrying a committed fix SHA).
> - **(b) SUPERSEDED by current Beta-50 scope:** every row marked **❌ OBSOLETE**, the WebGPU/turbo rows (v4 is flag-gated, not in the release path), and the tiny.en-era accuracy rows (Private default is now v2 `whisper-base.en`). Also the historical lane snapshots explicitly headed "Superseded By Current Rows Above."
> - **(c) WAIVED for Beta-50 by owner decision (tracked, not GO-blocking):** **paid/Stripe soft-launch** rows — Stripe is LIVE (`stripeKeyClass="live"`, Pro working, Basic reserved); opening user-visible paid checkout is a separate product decision, not a beta blocker. **Browser (Web Speech) filler undercount + no punctuation** = P2 non-blocking product-quality caveat. **Deferred** sections (no-concurrent-recording lease #794, auth/account-recovery security work, refund/cancel admin tooling) are explicitly not-in-this-release. The **full-5-min post-stop finalize <30s** latency item is tracked in-progress (tasks #19/#20), secondary to the opening-fidelity blocker.
> If a historical row's wording looks like an open blocker but it is not in disposition (d) above, it is historical — the current blocker set is exactly the two (d) items.
...
> **RECONCILIATION UPDATE 2026-07-10 (current state; ship posture lives in `RELEASE_STATUS.md`):**
> - **#891/#892 opening-clause fidelity — STILL BLOCKING (reproduced on deployed).** #898 (capture-from-start) merged + deployed, but rc-gates Gate 3 DAST on prod `46682800` (run `29125536866`) reproduced a **Private v2 saved-transcript opening-clause DROP**: `tests/live/stt-switching-contract.live.spec.ts:244` saved Private (`private_v2:whisper-base.en`, session `4ea44584`) dropped the opening ~2 sentences ("The stale smell of old beer, like, lingers, basically, a dash of pepper spoils beef stew…") and started "Well, the swan dive…". The **#892 fidelity gate correctly caught it** (opening anchor absent from first 12 words). → **NOT resolved; blocks Beta-50 GO.**
> - **P0-validate immediate-start re-gate — CONFIRMED still-open** (the Gate-3 DAST failure above is the concrete proof; do not mark resolved).
> - **Browser (Web Speech) no-punctuation + filler undercount = P2 non-blocking caveat** (owner disposition). Monitoring chip `task_c9525b8c`.
> - **#957 Private first-run mic-download + durable readiness gate = DONE** (merged `899161b2`, deployed, validated): click-mic-to-download (removed the separate "Set up" button), gated on the durable `privateModelStatus`/`data-model-status` (not transient sttStatus) — fixes post-session dead-mic lockout + no model-less start (desktop + mobile). 231 unit tests + independent review CLEAN. Beta-50 Option D functional QA = PASS (does not override the Gate-3 blocker).
> - **Latency / 5-min scope (owner ruling 2026-07-10): a 5-minute Private recording IS in Beta-50 scope.** 90s cap lifted to 300s (task#17 done). Tester invite = "Record one practice session, up to 5 minutes…". Full-5-min post-stop finalize **<30s is NOT yet proven** (tasks#19/#20 in-progress) = a tracked in-progress quality item, **NOT** the current GO blocker (the Gate 3 DAST opening-fidelity drop is). Do **NOT** frame this as "deferred because the invite is 1–2 minutes." Owner has not (in writing) classified the finalize-latency as a hard blocker or accepted caveat — carry it as **still-tracked (in-progress), secondary to the opening-fidelity blocker**.
> - **Stripe is LIVE** (`stripeKeyClass="live"` verified on the deployed runtime; live **Pro** price reachable/working, **Basic 404 = reserved/not-launched**, non-blocking). This is **not** the current blocker. The `stripeKeyClass="test"` / "payments hidden" references in the closed rows further below (e.g. BLOAT-PAYMENT-CTA, PROD-CONFIG-1, both dated `c5a2a460` 2026-06-08) are **historical pre-cutover evidence** — read them as the state at their proof date, superseded by this live-key note; they are not rewritten so the dated proof stays intact.
> - Post-beta follow-up chips: `task_02ea5353` (forced download-required live harness), `task_26a2b5d7` (deployed PDF download proof), `task_99533748` (Report Issue failure-state proof), `task_e3e21592` (funnel first_transcript_visible/analytics_opened/pdf_download_clicked), `task_c9525b8c` (Browser Web-Speech caveat monitoring).
...
| P1-BLOCKER | latency | Private finalization re-decodes the WHOLE recording at Stop (measured ~0.27× realtime single-thread WASM in prod → ~24s for 90s, ~80s for 5min). **Product requirement (owner, 2026-06-30): a trial user records ONE full 5-min Private session, sees progress while speaking, and waits <30s after Stop.** The deployed 90s per-recording cap is a **temporary engineering SAFETY SWITCH, NOT settled beta product behavior** — a 5-min trial that forces a stop every 90s contradicts its own promise. So full-5-min-under-30s is **REQUIRED before beta** — **owner ruling (2026-06-30): the 90s cap is NOT acceptable wave-1 beta behavior**; it stays ONLY as an internal safety switch, never the trial promise. | **Cap rejected for beta.** Primary path: **prototype Moonshine v2 (streaming) on a branch** = the full-5-min solution under evaluation. NOTE: Moonshine v2 is a Feb-2026 paper, NOT in the installed `@huggingface/transformers@4.2.0` (only v1/batch is) — the prototype must verify the LATEST lib + v2 ONNX weights + browser viability, then bakeoff (WER/5-min stop-to-final/loop) vs Whisper base.en. **Fallback if v2 doesn't pan out: segmented finalization** (Whisper is BATCH; design ready at `/private/tmp/SEGMENTATION_DESIGN.md`, ~1wk build + validation). COOP/COEP multithread is a *secondary* accelerator (~2.2×→~36s/5min, insufficient alone). |
| P1 | moonshine-v2-prototype | **PRIMARY full-5-min path (ACTIVE branch prototype).** Moonshine v2 = streaming-native ergodic encoder (Feb-2026 paper) — if browser-viable, Stop finalizes only the tail → full 5-min under 30s WITHOUT a Whisper segment ledger. Steps: (a) **viability** — Moonshine v2 is NOT in installed `@huggingface/transformers@4.2.0` (only v1/`MoonshineForConditionalGeneration`, batch, is registered); confirm LATEST lib + v2 ONNX weights exist + load + run in browser (WASM/WebGPU); model size + license. (b) **bakeoff vs whisper-base.en** — WER on Harvard corpus, time-to-first-text, **5-min stop-to-final**, memory/load, loop behavior, save/history compat. Stop if not browser-viable. | In progress (branch). **NOTE (Dev env can't reach HF CDN — `us.aws.cdn.hf.co` ENOTFOUND — so model load/bakeoff must run where HF is reachable: the branch CI / a networked machine.)** If v2 fails browser viability → fall back to the segmentation MVP (`/private/tmp/SEGMENTATION_DESIGN.md`). |
| P0-validate | immediate-start re-gate | The mic-ready gate (#902/#904) that fixes immediate-start opening loss is **deployed but proven only on a DELAYED-start take** (6.76s lead-in). The bug it targets (lose "My main" when you hit Record + speak immediately) is **unvalidated on a real immediate-start recording**. | **Run before inviting testers:** Record → wait for green "Ready — speak now" → speak immediately → Stop → saved-History preserves "My main point…". "Deployed + locally validated" ≠ "proven on the real case." |
...
| P1 (Pro/enterprise differentiator) | **Presentation Rehearsal Coach** — a.k.a. **Executive Briefing Rehearsal**. Preparation layer, sequenced **AHEAD of** Live Meeting Companion (row below). | **User feedback captured 2026-07 (real user, high-stakes board/comp-committee presenter).** **The reframe (the whole point):** this is NOT a generic meeting assistant and NOT filler-word coaching — it is a **high-stakes presentation rehearsal coach**. The valuable question is not "did I say *um* too much?" but "**did I deliver the intended message clearly, strategically, and confidently to a high-stakes audience?**" **Use case:** "I have an important board/client/investor presentation; I know the topics I need to cover — help me rehearse, sound polished, hit the right points, and improve before the real meeting." **Three natural layers (before / during / after):** BEFORE = Sandbox/Rehearsal (this feature); DURING = later ambient live cueing (Live Meeting Companion); AFTER = post-mortem scorecard (coverage + structure + executive polish). **Sandbox MVP inputs:** audience (board/client/investor/committee/interview panel), meeting type (board presentation/comp committee/client pitch/internal update), objective (inform/persuade/defend/recommend), key talking points, optional slide outline, desired tone, privacy preference → rehearse aloud → SpeakSharp compares the transcript against the intended brief. | Fits the core promise (practice privately, trustworthy feedback, improve one thing at a time) better than jumping to a live Zoom/Meet overlay. **Difference vs Live Meeting Companion (Zoom/Google Meet/Teams):** **Rehearsal Coach** = BEFORE the meeting · prepare/polish/check coverage · LOWER risk · gets richer LIVE cues DURING the practice run + a post-rehearsal scorecard after (NOT post-run only) · user voluntarily rehearses selected content (simple privacy) · builds on current Session+transcript+AI stack · strong Pro monetization · very feasible MVP. **Companion** = DURING the meeting · assist live while speaking · HIGHER risk · user must digest cues mid-presentation · may capture live meeting/other-speaker content (privacy-heavy) · needs overlay/call integration · later enterprise feature. **Ship Rehearsal FIRST** — easier to ship, trust, and monetize; the Companion later reuses the same brief + coverage/coaching model. | **Build sequence (do NOT start with the live HUD — risk control):** (1) **Sandbox Mode FIRST** — enter brief, rehearse aloud, get a post-rehearsal SCORECARD; directly monetizable. (2) **Rehearsal HUD SECOND** — used during a *practice* run (not live Zoom yet). Because it is rehearsal, it can show **richer live cues than a real meeting would allow**: short AI text prompts, section-level guidance, "try this next" suggestions, plus a live per-talking-point coverage checklist (e.g. Business strategy = Covered · Compensation alignment = Partially covered · Retention risk = Missing · Recommendation = Covered) rendered as gray→green dots. The text-prose restraint is a **Live Companion** constraint, not a rehearsal one. (3) **Live Meeting Companion THIRD** — Zoom/Meet/Teams, reuse the same brief + coverage model, minimal ambient cues only. **AI feedback answers:** (a) did I cover the points I intended? (b) did I connect details to the larger business strategy? (c) did I sound polished / executive-ready? (d) what did I miss or underdevelop? (e) what to improve next rehearsal. **Outputs:** talking-point coverage; missed/underdeveloped points; over-explained points; clarity + executive polish; audience-fit; 2–3 next-rehearsal actions; optional saved rehearsal report — far beyond generic WPM/filler. **The split-attention trap (a LIVE-MEETING constraint, not a rehearsal one):** during the *real* meeting a verbose HUD can make the presenter WORSE — someone presenting to a comp committee cannot read AI prose mid-sentence, and a bad cue ("Mention long-term incentive alignment with strategy") could derail them, so the **Live Meeting Companion** must be **ambient, sparse, and non-verbal** (status dots + short labels), never text-heavy real-time coaching. The **Rehearsal HUD is different:** because it is practice, Feature 1 explicitly INCLUDES real-time AI presentation-improvement suggestions (short text prompts, live talking-point coverage, executive-polish prompts, "try this next") in addition to the post-rehearsal scorecard. Feature 1 is **not** post-run only — the Rehearsal↔Live distinction is context / risk / distraction budget, not live-vs-non-live. **Privacy — do NOT overclaim (honest framing):** the product must NOT imply "your audio, transcripts, and slide content never leave your machine" — SpeakSharp does not have full local semantic analysis today. HONEST claim: **"Private transcription can keep AUDIO local; AI content feedback (coverage/strategy/polish over transcript, talking points, slides) may require sending that content to a CLOUD AI provider unless local LLM analysis is separately built."** This local-vs-cloud distinction is critical for board/comp/client-confidential work and must be explicitly disclosed. **Relationship to Live Meeting Companion:** this IS the preparation layer; the future live companion reuses this brief/coaching model but must NOT be built before rehearsal quality is proven. |
...
| P1 | Private STT CPU speed via cross-origin isolation | **ACCELERATION HIERARCHY — verified vs code 2026-06-30 (`privateRuntimePath.ts` · `wasmThreads.ts` · `webgpuSupport.ts` · `transformers-js.worker.ts` · `…v4.worker.ts`):** **v2 (DEFAULT, `@xenova/transformers`) = CPU/WASM ONLY, NO WebGPU path** → WASM **multi-thread** (≤4 threads, `computeWasmThreadCount`) IF `crossOriginIsolated===true`, ELSE WASM **single-thread** floor (current prod: single-thread, ~0.27× realtime → ~24s/90s, ~80s/5min). **v4 (FLAG-GATED, `@huggingface/transformers`) = WebGPU** IF v4 flag ON + real adapter (`detectWebGPUSupport()`→`requestAdapter()` non-null; base_q4 floor / distil_q4 tier), ELSE WASM fallback → v2 CPU floor. **Resolver priority: v4-WebGPU(flag+adapter) → v2-multithread(if isolated) → v2-single-thread; Cloud NEVER automatic.** Wave-1 external (no v4 flag / no WebGPU / not isolated) lands on the **v2 single-thread floor**. COOP/COEP only unlocks the v2 multi-thread rung; WebGPU is v4-only; v2 has no GPU route without promoting v4 or adding a v2-on-v4-runtime path. — Multi-threaded WASM (the only CPU speedup for no-WebGPU users, since cloud fallback is prohibited by the privacy promise) requires `crossOriginIsolated === true`, which needs COOP/COEP headers. `frontend/vite.config.mjs` removed those headers with the note "they block Stripe.js and other third-party resources", but the app no longer **embeds** Stripe.js — checkout is a server-created redirect (`window.location.href = data.checkoutUrl` in `PricingPage.tsx`/`AnalyticsPage.tsx`/`Navigation.tsx`), no `loadStripe`/Elements in the shipped app. **CORRECTION (the prior "no `@stripe/stripe-js` npm dependency" claim was FALSE — see line ~529 / BLOAT-STRIPE-JS-DEPS):** `@stripe/stripe-js`+`@stripe/react-stripe-js` DO exist in root `package.json` but are TEST-ONLY (0 prod imports). So the Stripe-*embed* premise is genuinely stale; the npm-dep sub-claim was wrong; re-anchor the audit on actual runtime loads, not the package list. **Static Layer-1 audit (2026-06-30): `frontend/index.html` has ZERO cross-origin embeds; fonts are system/self-hosted ('Inter' + system-ui, no Google Fonts link); Stripe is redirect-only (navigation, unaffected by COEP). The REAL `credentialless` targets are RUNTIME cross-origin loads: (#1) ORT WASM from jsDelivr — `transformers-js.worker.ts` sets numThreads/SIMD but NEVER overrides `env.backends.onnx.wasm.wasmPaths`, so it pulls from the CDN (self-host via ORT-WASM-SAME-ORIGIN to remove this risk entirely); (#2) PostHog remote assets/recorder; (#3) Supabase + Sentry fetches (Bearer/API-key, not cookies → credentialless likely OK). Live preview measurement (`crossOriginIsolated===true` + each resource loads + decode multiplier) still required — but the blocker surface is now small and known.** The P3 WASM-threads change (`transformers-js.worker.ts`) already guards on `crossOriginIsolated` and is a safe no-op until isolation is enabled. | For no-WebGPU users (WebGPU is v4-flag-only — does nothing for the default/wave-1 path), single-threaded CPU Whisper is the slow path (~0.27× realtime measured → ~24s for 90s, ~80s for 5min). Re-enabling isolation activates v2 multi-threading (~2–2.5× UNMEASURED → ~32–36s/5min, still over 30s alone) — a secondary accelerator, NOT the full-5-min fix (that's Moonshine-v2-prototype / segmentation, see the P1-BLOCKER latency row). | Investigate (do NOT flip blind): (1) confirm via git history why COOP/COEP were removed and whether it predates the redirect-checkout migration; (2) re-enable `COOP: same-origin` + `COEP: credentialless` (the lenient mode `serve-e2e.mjs` already uses) in dev/preview/prod vite config; (3) smoke-test every cross-origin third-party under isolation — Stripe redirect, Supabase, PostHog, Sentry, Gemini, fonts/images; (4) confirm `crossOriginIsolated === true` in the worker so P3 activates, then benchmark CPU decode before/after. Decision owner: confirm the Stripe constraint is truly obsolete before shipping. **Acceptance criteria (all must pass in a production-like build before enabling):** `crossOriginIsolated === true`; Supabase auth/data works; Sentry reports; PostHog captures; Stripe hosted-checkout redirect completes; model assets (`/models/...`) load; fonts/images load; the transcription worker logs `device: 'wasm-multithread'` with `threads > 1` (now emitted in the `loaded` telemetry); and CPU decode time improves materially vs single-thread baseline. If any third-party breaks under `COEP: credentialless`, do not ship isolation. |
...
| P2 (paid-launch Ops cutover) | Stripe live-key cutover | Billing evidence is intentionally test-mode/sandbox; the checkout→webhook→portal **journey is PROVEN with TEST keys = the accepted proof** (`27173676657`; `RELEASE_CLOSEOUT_LEDGER.md` §D). **Per the release-owner ruling, live keys are not money-tested and there is no pending "live proof"** — going live is a config cutover, not a charging test. | **Not a controlled-beta blocker** (beta hides payments / `stripeKeyClass="test"`). Required only for paid launch. | **Ops launch-day cutover (not a Dev/QA proof):** set `sk_live`/`pk_live`/live `whsec`/live price IDs in prod env, register the live webhook endpoint, verify `stripeKeyClass==="live"`. (A single controlled live smoke is optional ops diligence, not a required release gate.) |
