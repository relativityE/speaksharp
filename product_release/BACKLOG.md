...
| P1 | BLOAT-PAYMENT-CTA | Some upgrade CTAs remain visible when payments are disabled/non-live. | Core payment enablement is correctly `pk_live_` gated, but `AnalyticsPage` renders `analytics-page-upgrade-button` and silently returns on click; `AnalyticsDashboard` renders a secondary upgrade action; `SunsetModals` always links to `/pricing`. | Dev second pass: gate the render/copy under missing/test Stripe key, not only the handler, and add regression tests. |
| P1 | BLOAT-CONFIG-REQUIRED-COPY | Configuration fallback page copy is stale after config-gate-relax. | Boot now only requires `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY`, but `ConfigurationNeededPage` still tells users/testers `VITE_STRIPE_PUBLISHABLE_KEY` and `VITE_SENTRY_DSN` are required. | Dev second pass: update copy/tests so the page lists only actual boot blockers and treats Stripe/Sentry as optional release/ops checks. |
| P1 | BLOAT-NATIVE-FROZEN-COPY | Native watchdog still exposes scary “Engine Frozen” user-facing text. | `TranscriptionService.ts` emits `Speech recognition is taking a moment (Engine Frozen)` when heartbeat drifts; prior human proof observed this during successful Native recognition. | Dev second pass: remove “Engine Frozen” from user copy and make warning mode-aware/evidence-driven. |
| P1 | BLOAT-STALE-COORD | Repo still contains stale coordination source-of-truth file. | `product_release/ACTIVE_COORDINATION.md` claims to be the single source of truth, but active coordination moved to `/private/tmp/ACTIVE_COORDINATION.md`; repo file contains stale `origin/main@7cc9fed2`. | Dev/test coordination second pass: tombstone or delete repo copy so agents/reviewers do not follow stale rows. |
| P2 | BLOAT-VAULT-DOCS | Release docs still use stale “Vault Mode” terminology. | `product_release/PRODUCT_FEATURES.operational.md` and related docs still say “Private / Vault Mode” while app release guards block raw/stale terminology in UI. | Product/dev docs second pass: update or archive stale docs before reviewer/customer-facing handoff. |
| P2 | BLOAT-5173-DOCS | Local CORS/checklist docs still point at `localhost:5173`. | `_shared/cors.ts` fallback/comment and `LAUNCH_ENV_CHECKLIST.md` mention 5173; manual proof mode is 5174. Dynamic localhost echo reduces runtime severity, but docs/config are stale. | Dev/docs second pass: align fallback comments and release checklist with the canonical 5174 manual-proof contract. |
| P2 | BLOAT-FAILURE-DUMP | Tracked test failure dump remains in source tree. | `frontend/test_failures_v7.txt` is tracked (~108 KB) and appears to be a failure log, not source/test/canonical evidence. | Dev second pass: remove or move to `product_release/archive` only if intentionally retained. |
...
| P2 | BLOAT-EVIDENCE-REPORTS | Tracked release evidence is small but potentially stale/contradictory. | `product_release/evidence` tracked files total roughly hundreds of KB, including STT test reports and dated proof JSONs from 2026-06-02/05. | Dev/test second pass: keep canonical evidence, move superseded reports to archive, and avoid active docs with contradictory recommendations. |
...
| P2 | BLOAT-SCRIPT-REACHABILITY | Several scripts look one-off or research-only. | Rough reference scan flagged `scripts/native-human-cdp-monitor.mjs` (0 refs), `private-local-punctuation-feasibility.mts`, `benchmark-filler-ceiling.mts`, `generate-filler-audio.sh`, `private-status-reducer.mjs`, `private-timing-reducer.mjs`, `vm-recovery.sh`, `dev-init.sh`, and `preinstall.sh` as review candidates. | Dev second pass: classify keep/delete/archive; do not remove RC gates, CI, canary, STT corpus, or Supabase/test-user scripts without deeper review. |
| P2 | BLOAT-DEPS | Dependency list has likely-unused or release-parked packages. | Rough import scan found several manifest-only or parked-path candidates. Dev confirmed `whisper-turbo` still has a real TS import through the parked registry path, so dependency cleanup must follow feature disposition. `compromise`, `@vitest/ui`, `pdf-lib`, `pdf-parse`, `node-fetch`, `jose`, and `baseline-browser-mapping` still need confirmation. | Dev second pass: run dependency-cruiser/depcheck or manual package audit; remove only after confirming no script/config/runtime use. |
...
| STALE-DOCS | "Vault Mode" + `:5173` in `product_release` docs/evidence (31 hits) — **not** in user-facing `frontend/src` (a guard test forbids the term). | Update docs; non-blocking. | test only |
