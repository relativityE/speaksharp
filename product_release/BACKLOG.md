...
| P0 | FEEDBACK-1 | In-app issue-report flow is implemented but production cannot accept reports because the table is missing. | Live proof on deployed `main@349b0551` created a real authenticated session, then anonymous insert returned Supabase `PGRST205`: `Could not find the table 'public.user_issue_reports' in the schema cache`. The `Deploy Supabase` workflow succeeded for `349b0551`, but the `deploy-production-db` job was skipped, so schema did not change. | @dev-agent/product-ops: apply `20260605080000_user_issue_reports.sql` and `20260605120000_user_issue_reports_optional_user.sql` to production, then rerun the authenticated anonymous insert proof. Pass requires a row with `user_id=null`, required metadata present, and transcript/audio fields null unless opted in. |
