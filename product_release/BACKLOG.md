...
| P2 | BLOAT-FORMAT-TRANSCRIPT | ✅ DEV DONE (test runtime-verified locally; merge/deploy reproof pending) — Gemini LLM path REMOVED, deterministic wired. Committed `dev/release-closure@8125a899`. | Product call: "if in use, remove it." Removed (not flag-gated): `nativeGeminiFormatter` (+test), the `format-transcript` Supabase edge fn (index/test/deno.json), and its deploy lines (GEMINI_API_KEY sync kept — get-ai-suggestions still uses it). Wired `nativeDeterministicCleanup` (`normalizeNativeTranscript`, $0/instant/word-safe) through the SAME seam (word-preservation guard + telemetry + Private guard unchanged; same `registerNativeProductionFormatter(mode)` API). Proof: tsc clean; native cluster 74/74; `pnpm build` exit 0; grep-clean. Real Chrome/CDP freeform proof `/private/tmp/TEST_NATIVE_REAL_MIC_FREEFORM_RUNTIME_PROOF_2026-06-08.md` PASS: zero `/functions/v1/format-transcript` network calls, deterministic formatter telemetry `attempted=true`, `wordPreserving=true`, `fallbackToRaw=false`, `errorCode=null`. NeMo-ONNX = UNGUARANTEED post-beta candidate, not a plan. | Close after merge/deploy reproof. If product wants Native formatter-quality claims, run a scripted ground-truth Chrome mic proof; the freeform run only proves runtime path. |
...
| ENV-PROD-TRACKED | ✅ RESOLVED — `frontend/.env.production` **removed** on `dev/release-closure` (outside Vite `envDir`, never build-loaded; all `VITE_*` client-public, never a secret leak). | Real prod client config = Vercel Project Env (Home B) + `ENV_INVENTORY.md`. Do not re-add. | dev only |
