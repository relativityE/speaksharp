...
| P1 | latency | Private finalization re-decodes the WHOLE recording at Stop (measured ~0.27× realtime single-thread WASM in prod → ~24s for 90s, ~80s for 5min). **Beta ships with a 90s per-recording individual cap** (deployed; keeps stop-to-final <30s; 5-min TOTAL trial allowance unchanged). The full-5-min-under-30s requirement is now a **pre-GA** target, NOT pre-beta — a conscious sequencing choice: cap → collect data → lift via Moonshine-or-segmentation before GA. Drain-cap #900 bounds a pathological wait only. | **Interim 90s cap LIVE.** Real fix = **segmented finalization** (decode only the unfinalized tail at Stop), **gated on the Moonshine eval below**. Threading (COOP+COEP) is a *measured* ~2–2.5× multiplier (likely insufficient alone), not the primary fix. |
