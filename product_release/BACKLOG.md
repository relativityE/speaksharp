...
| P1-BLOCKER | latency | Private finalization re-decodes the WHOLE recording at Stop (measured ~0.27× realtime single-thread WASM in prod → ~24s for 90s, ~80s for 5min). **Product requirement (owner, 2026-06-30): a trial user records ONE full 5-min Private session, sees progress while speaking, and waits <30s after Stop.** The deployed 90s per-recording cap is a **temporary engineering SAFETY SWITCH, NOT settled beta product behavior** — a 5-min trial that forces a stop every 90s contradicts its own promise. So full-5-min-under-30s is back **in beta scope** unless the owner explicitly repositions the trial as "≤90s per practice" (not chosen). | **Cap is interim, not the answer.** Path, in order: (1) ✅ backlog audit row corrected + Layer-1 static COOP/COEP audit done; (2) **bounded COOP/COEP measured spike** — enable isolation in preview, verify `crossOriginIsolated`+each resource loads, MEASURE the real multithread multiplier (do NOT presume it solves latency; ~2.2× → ~36s/5min still over 30s, so likely insufficient alone); (3) **time-boxed (1–2d) ASR bakeoff** — Whisper base.en (+ segmentation design) vs **Moonshine v2** (streaming-native ergodic encoder) IF browser/WASM+transformers.js support is real; (4) **segmented finalization = the real full-5-min architecture** (Whisper is BATCH; chunking ≠ streaming), design-first. Decide on data. |
| P2 | moonshine-eval | **Decision gate for segmentation.** Tightly-scoped, time-boxed BRANCH comparison: Moonshine v2 vs Whisper base.en on the Harvard corpus, measuring (1) **WER** — matches base.en on our audio? (2) **latency-at-length** — does its streaming/variable-length decode get a real 5-min recording <30s? (3) **loop behavior** — does variable-length decode kill the v4 duplication? **FIRST** confirm `transformers.js` + browser-WASM support for Moonshine v2 specifically (verify it runs in our stack, not just that it exists) before investing. | Not started (post-beta). Outcome decides: Moonshine swap (may shrink/remove segmentation) OR build segmentation on Whisper. **Segmentation is engine-agnostic** — if we keep Whisper it remains the latency fix, so this analysis is not wasted either way. |
