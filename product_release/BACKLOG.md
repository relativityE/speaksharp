...
| P2 (post-launch) | Apples-to-apples LibriSpeech re-measure + harness calibration | Private (v2 + v4) | **DEFERRED post-launch, NOT a release blocker (release-owner decision 2026-06-17, "option 2").** Today's vendor-vs-SpeakSharp numbers are NOT comparable: vendor = LibriSpeech, ours = Harvard Sentences (easier corpus) measured best-case Node clean-decode; our v4 `base_q4` IS `openai/whisper-base.en`, so it cannot exceed its own published 95.8% — any higher reading is a measurement artifact. **No customer-facing vendor-vs-SpeakSharp comparison until this lands.** Scope: run our `whisper-base.en` / `distil-small.en` through the Node harness on LibriSpeech test-clean (or a fixed subset); `base.en` should reproduce ≈95.8% — if it instead reports ~98.85% the harness scoring is too lenient (real calibration bug to fix). Context: `tests/STT_BENCHMARKS.json` → `_measurement_framing`, and `/private/tmp/STT_VENDOR_VS_MEASURED_OPTIONS.md`. | @dev-agent (post-launch) |
