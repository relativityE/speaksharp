...
| ✅ ACCEPTED (was P0 RC-5MIN-PROOF) | 5-min Private finalization | A full 5-min Private v2 recording finalizes in **≈90s** post-stop, shown as honest "Finalizing…" progress | Owner accepted ~90s as the RC limitation (2026-07-14); tester copy states "~90s finalization" (#977). No loop/corruption at length (long-form ~98.95% fixture). | **Closed as accepted limitation.** Faster finalization (streaming/segmentation/multithread) is a post-limitation improvement lane, not a blocker. The earlier `<30s` GREEN threshold is withdrawn. |
...
| P2 (improvement) | latency | Private finalization re-decodes the WHOLE recording at Stop (measured ~0.27× realtime single-thread WASM in prod → ~24s for 90s, ~80s–90s for 5min). **Accepted RC limitation (owner, 2026-07-14): a trial user records ONE full 5-min Private session, sees "Finalizing…" progress after Stop, and waits ≈90s.** The earlier `<30s` product requirement is **withdrawn**. The internal 90s per-recording safety switch stays a flag-only guard (zero saved-text mutation), not user-facing beta behavior. | **Post-limitation improvement lane (not a blocker).** Candidate faster paths: **Moonshine v2 (streaming) prototype** — NOTE Moonshine v2 is a Feb-2026 paper, NOT in installed `@huggingface/transformers@4.2.0` (only v1/batch), so it needs LATEST-lib + v2-ONNX + browser-viability verification then a bakeoff (WER / 5-min stop-to-final / loop) vs Whisper base.en; **fallback segmented finalization** (decode only the unfinalized tail at Stop); COOP/COEP multithread is a secondary accelerator (~2.2×→~36s/5min). None gates controlled beta. |
| P2 (improvement) | moonshine-v2-prototype | **Faster-finalization candidate (branch prototype; not a blocker — ≈90s is the accepted limitation).** Moonshine v2 = streaming-native ergodic encoder (Feb-2026 paper) — if browser-viable, Stop finalizes only the tail → materially faster than the ≈90s whole-recording re-decode WITHOUT a Whisper segment ledger. Steps: (a) **viability** — Moonshine v2 is NOT in installed `@huggingface/transformers@4.2.0` (only v1/`MoonshineForConditionalGeneration`, batch, is registered); confirm LATEST lib + v2 ONNX weights exist + load + run in browser (WASM/WebGPU); model size + license. (b) **bakeoff vs whisper-base.en** — WER on Harvard corpus, time-to-first-text, **5-min stop-to-final**, memory/load, loop behavior, save/history compat. Stop if not browser-viable. | In progress (branch). **NOTE (Dev env can't reach HF CDN — `us.aws.cdn.hf.co` ENOTFOUND — so model load/bakeoff must run where HF is reachable: the branch CI / a networked machine.)** If v2 fails browser viability → fall back to the segmentation MVP (`/private/tmp/SEGMENTATION_DESIGN.md`). |
...
## DEV OWNERSHIP — Accuracy / Efficiency / Performance workstream (owner: dev-agent)

A/E/P STT optimization workstream. **Owner: dev-agent.** Items in scope:
...
TEST has overall jurisdiction to confirm app behavior; DEV owns the code fixes above and will
