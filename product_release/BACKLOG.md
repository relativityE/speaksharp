...
| P2 | BLOAT-VAULT-DOCS | Release/docs/tester surfaces still use stale “Vault Mode” terminology. | `product_release/PRODUCT_FEATURES.operational.md` still says `Private/Vault`; GitHub tester feedback template still offers `Private / Vault` and warns about `Private/Vault mode`. App release guards block raw/stale terminology in user-facing `frontend/src`, so this is docs/tester-facing drift, not current UI copy. Dev commit `320959bc` reduced some occurrences but did not close all hits. | Product/dev docs second pass: update or archive stale docs/templates before reviewer/customer-facing handoff. Close only when `rg "Vault Mode|Private / Vault|Private/Vault" product_release .github/ISSUE_TEMPLATE` has no active-facing hits except archived evidence intentionally marked historical. |
