...
> - **Operational release posture — ✅ controlled-beta GATES GREEN on final signoff SHA `0c485add`.** The previously-pending final gate re-run is DONE: `gate=all` all 5 gates green (run `28306514090`) on the exact post-merge SHA, with deploy-health canary, telemetry e2e, stt-switching 3/3, and full CI all green on the **same** SHA, and `auth.users` Δ=0 (35→35). Per **Final-SHA freshness** this is current signoff evidence, not historical. **No gate is pending — the remaining step is the release-owner's tester-invite go/no-go decision.** Payments stay hidden/`stripeKeyClass="test"` for beta; paid launch is a separate Ops cutover (RELEASE_CLOSEOUT_LEDGER §D).

| Release Track | Status | Why |
|---|---|---|
| Controlled private beta / early-access (non-payment) | ✅ GATES GREEN on signoff SHA `0c485add` — invite go/no-go = owner | Full post-merge signoff PASSED on `0c485add`: `gate=all` all 5 green (`28306514090`), deploy-health canary (`28306448320`, Free canary verified), telemetry e2e (`28306516507`), stt-switching 3/3 (`28306519120`/`28306525596`/`28306652236`), full CI (`28306448318`), `auth.users` Δ=0 (35→35). No gate pending; invites are the owner's call. Payments hidden/controlled (`stripeKeyClass="test"`). |
