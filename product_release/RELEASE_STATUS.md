...
**Last updated:** 2026-07-10 · Last updated by: dev-agent (claude).

> **UPDATE 2026-07-10 — CURRENT STATE (supersedes all earlier updates below).**
> **Ship posture: ⛔ INVITES HELD / NO-GO.** Current production SHA **`46682800`** (main head == deployed).
> - **Blocker (Dev-owned, real, reproduced on deployed):** **rc-gates on `46682800` = FAILURE at Gate 3 (DAST / Running App)** — Gates 1/2/4/5 pass; local DAST 18/18 pass; **live DAST 7/8** (run `29125536866`). The failing spec `tests/live/stt-switching-contract.live.spec.ts:244` caught a **Private v2 saved-transcript OPENING-CLAUSE DROP**: fixture audio "The stale smell of old beer, like, lingers, basically, a dash of pepper spoils beef stew…" but the saved Private (`private_v2:whisper-base.en`, session `4ea44584`) transcript dropped the opening ~2 sentences and started "Well, the swan dive…" — the #892 fidelity gate flagged the missing opening anchor. This is the **#891 opening-clause drop reproduced live**; the BACKLOG **#891/#892 + P0-validate immediate-start re-gate** rows are CONFIRMED still-open. **HOLD invites until fixed + Gate 3 DAST green on the fixed SHA.**
> - **Stripe is now LIVE** (`stripeKeyClass="live"` verified on the deployed runtime): live **Pro** price reachable/working (`pro=200`); **Basic 404 = intentionally reserved/not-launched** (non-blocking). Any earlier "test-mode/payments-hidden" statement below is STALE.
> - **#957 Private first-run fix MERGED + DEPLOYED + validated** (`899161b2`; PRs #957/#958/#959): click-mic-to-download (separate "Set up" button removed), gated on the **durable `privateModelStatus`** — no post-session lockout, cached-return works, mobile no model-less start; 231 unit tests + independent review CLEAN. **Beta-50 Option D functional QA = PASS** (Cloud/Private/Browser record→transcript→filler→save; Report Issue; filler SSOT §6; PDF affordance; console/network/Sentry no blockers) — evidence `product_release/evidence/beta50_private_2026-07-10/`. Functional QA passing does **not** override the Gate-3 DAST blocker.
> - **Engine posture:** v2/`whisper-base.en` = Private default; **v4 NOT in the release path**. 90s cap lifted to 300s (5min); **a 5-minute Private recording IS in Beta-50 scope** (owner 2026-07-10; tester invite = "Record one practice session, up to 5 minutes…"). Full-5-min post-stop finalize **<30s is NOT yet proven** (tasks #19/#20) = a tracked in-progress quality item, **not** the current GO blocker (the Gate 3 opening-fidelity drop is); do NOT frame it as deferred-because-short-invite.
> - **Monitoring (verified 2026-07-10):** ops-health run `29125370928` = 7 OK / 1 review (Stripe Basic) / 1 fail (GitHub = the real Gate-3 red); real services (App/Vercel/Supabase/AssemblyAI/Gemini/Sentry/PostHog) 🟢. Observability API smoke `29125372163` SUCCESS (Sentry frontend/edge + PostHog ingest confirmed). Sentry diagnose `29125406521` = 0 P0 / 0 P1 / 1 P2 (self-healing stale-tab chunk-load; 2 synthetic smoke probes). Issue-triage `29125373515` digest works (rows redacted). PostHog funnel arriving (session_started 49, session_saved 46, conversion_cta_viewed 187).
>
> _History (superseded) below._
...
| Stripe (billing journey) | ✅ Checkout → webhook → billing-portal **journey PROVEN in Stripe TEST mode = the accepted proof** (release-owner ruling, `RELEASE_CLOSEOUT_LEDGER.md` §D; runs `27441691671` / `27173676657`). Live keys are **not** money-tested and not required as a proof. Going to **paid** launch = an **Ops config cutover** (set `sk_live`/`pk_live`/live `whsec`/live price IDs, register the live webhook, verify `stripeKeyClass==="live"`) — a launch-day step. **UPDATE 2026-07-10: prod now runs `stripeKeyClass="live"`** (verified on the deployed runtime) — live **Pro** price reachable/working (`pro=200`); **Basic `basic=404` = reserved/not-launched** (non-blocking). Whether paid checkout is user-visible remains a product decision, but the live key IS in prod. | LIVE key in prod (2026-07-10); paid-checkout visibility = product decision |
