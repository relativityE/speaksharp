...
> **`baseline` and `deployed-release` CONVERGE, and that is a read, not an inference.** Both are `86f7b8e9d654523457da8b2640aa419152adceb0`. The deployed release was read directly from `window.__APP_RELEASE__` on the canonical Production app `https://speaksharp-public.vercel.app` at **2026-09-18T11:57:15Z**, by a read-only page load with no interaction and no sign-in. It equals `main`'s tip because the #1492 deploy had propagated by then — **not** because a merge was taken as evidence of a deployment. The criterion below is unchanged and still governs: the deployed release is a read, never inferred from a merge or a moving `main` pointer, so the next time `main` advances these two values diverge again until a new read is taken. The previous entry (`28b66bc422ad90b2f785ae4f03f7858196fbcbe3`, read 2026-09-16T17:48:10Z after #1489) is superseded by this read.
...
|---|---|---|
| **Repository `main` (moving branch pointer)** | `86f7b8e9d654523457da8b2640aa419152adceb0` (after #1492), read 2026-09-18 | Re-read GitHub before every exact-pair decision. |
| **Deployed product release (last READ)** | `window.__APP_RELEASE__ = 86f7b8e9d654523457da8b2640aa419152adceb0`, read from the canonical Production app `https://speaksharp-public.vercel.app` at 2026-09-18T11:57:15Z | **A literal read, by read-only page load with no interaction or sign-in.** It currently equals the baseline because the #1492 deploy had propagated when the read was taken; that equality is the observed result, not the method. This value must be re-read before any RWT, deployment or go/no-go claim, and it goes stale the moment `main` advances — a merge is never evidence that it deployed. |
...
| **Open merge candidates** | PR #1494 (Home, H-1…H-5), PR #1498 (Session, S-8…S-14), PR #1488 (canary exact deployed-take evidence) | Each needs exact-head CI including browser shards, cleared automatic review, PM acceptance and an exact PO head/base authorization. Merges stay serial on one token. No acceptance or authorization is implied here. |
| **Recommended for closure** | PR #1497 (allowlist the newest-one retention migration for exact apply) | The read-only migration ledger shows `20260908120000_transcript_retention_newest_one.sql` already **applied** in Production (see *Database migration state* below), so an allowlist entry to apply it has no remaining purpose. Closure is a PM/PO decision. |
...
| **Documentation currency** | This currentization (issue #1318) | Restores the baseline to `main@86f7b8e9`. The currency guard reads committed files and ancestry only. |
...
1. **Close current PRs serially.** The 2026-09-16/17 queue merged: #1477, #1486, #1467, #1487, #1490, #1493 and #1492. Open now: PR #1494 (Home), PR #1498 (Session S-8…S-14) and PR #1488 (canary). Each restacks and requalifies on the exact head after every `main` advance.
...
- The three September migrations above were applied together on **2026-09-12** by the whole-queue `supabase db push` in "Deploy Supabase" run `34691493637`, not by the exact-allowlist workflow. The newest-one retention version is therefore live in Production, while #1452 (owner of the fixes required before newest-one activation) is still open. **PM and the Product Owner must disposition that ordering gap**; this document records the read and authorizes nothing.
