...
|---|---|---|
| P0 (first-impression RC) | Free users default to Browser/Native — the weakest path (no punctuation + filler undercount) | In-UI + tester caveat copy before tag; do not claim Browser polish; nudge Private after a Browser session. Finding `RC-BROWSER-CAVEAT` (BACKLOG.md 2026-07-12). |
| P0 (first-impression RC) | Private 5-min v2 finalize ≈80s reads as a hang without honest state | Honest visible Finalizing progress/estimate + "longer recordings take longer to finalize" copy. `RC-PRIVATE-FINALIZE-COPY`. |
| P0 (first-impression RC) | No current 5-min Private production proof — "up to 5 min" unproven | Owner-driven 5-min v2 proof; GO <30s stop-to-final / CONDITIONAL 30–90s + honest state / NO-GO >90s or loops. `RC-5MIN-PROOF`. |
| P0 (first-impression RC) | Default Private engine/version must be guaranteed v2, not v4 | Guard/prove a default Private user saves `private_v2:whisper-base.en`; v4 off/flag-gated. `RC-V2-DEFAULT-GUARD`. |
