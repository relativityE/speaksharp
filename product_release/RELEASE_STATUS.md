...
**Evidence Sources:** GitHub `main`; Production `window.__APP_RELEASE__`; read-only Migrations Preflight run `35341531149`; merge receipts for PRs #1467, #1486, #1487, #1490, #1492, #1493, #1494, #1498, #1502–#1510; read-only identity inventory run `35467307725` (#1500); issues #1258, #1263, #1304, #1390, #1471–#1476 and #1495.

> **`baseline` and `deployed-release` CONVERGE, and that is a read, not an inference.** Both are `289da05a362e271314a6b77fdd9f7b78bb78960d`. The deployed release was read directly from `window.__APP_RELEASE__` on the canonical Production app `https://speaksharp-public.vercel.app` at **2026-09-19T21:23:15Z**, by a read-only page load with no interaction and no sign-in. It equals `main`'s tip because the #1509 deploy had propagated by then — **not** because a merge was taken as evidence of a deployment. The criterion below is unchanged and still governs: the deployed release is a read, never inferred from a merge or a moving `main` pointer, so the next time `main` advances these two values diverge again until a new read is taken. The previous entry (`5b89dc8636ce69740eda66ade7987c280c74180a`, read 2026-09-19T11:01:27Z after #1502) is superseded by this read. Intermediate deploys were each read and receipted on their PRs: `ced7af7f` (#1504), `d74fb9bb` (#1508), `22f8c023` (#1507), `72ab0158` (#1506) and `464b5a50` (#1510, read 2026-09-19T21:12:05Z).

<!-- CURRENCY-BLOCK
# Machine-readable state parsed by tests/config/documentationContract.test.ts.
baseline: 289da05a362e271314a6b77fdd9f7b78bb78960d
deployed-release: 289da05a362e271314a6b77fdd9f7b78bb78960d
...
|---|---|---|
| **Repository `main` (moving branch pointer)** | `289da05a362e271314a6b77fdd9f7b78bb78960d` (after #1509), read 2026-09-19 | Re-read GitHub before every exact-pair decision. |
| **Deployed product release (last READ)** | `window.__APP_RELEASE__ = 289da05a362e271314a6b77fdd9f7b78bb78960d`, read from the canonical Production app `https://speaksharp-public.vercel.app` at 2026-09-19T21:23:15Z | **A literal read, by read-only page load with no interaction or sign-in.** It currently equals the baseline because the latest deploy had propagated when the read was taken; that equality is the observed result, not the method. This value must be re-read before any RWT, deployment or go/no-go claim, and it goes stale the moment `main` advances — a merge is never evidence that it deployed. |
...
| **Documentation currency** | This currentization (issue #1318) | Restores the baseline to `main@289da05a`. The currency guard reads committed files and ancestry only. |
...
1. **Close current PRs serially.** Merged 2026-09-16…19: #1477, #1486, #1467, #1487, #1490, #1493, #1492, #1494, #1498, #1502, #1505, #1503, #1504, #1508, #1507, #1506, #1510 and #1509. Open now: PR #1499 (this document) and PR #1488 (frozen; merge-queue pilot). Each restacks and requalifies on the exact head after every `main` advance.
...
7. **Product presentation:** #1474 owns the complete G10 during/after Practice Loop hierarchy and theme; the Design Correction Brief continues it in PR #1493, PR #1494 (Home) and PR #1498 (Session), all merged; the G8 Share feedback retheme, the G7 set-up retheme (#1503), the FAQ (#1504, #1507), Sign Out (#1508) and the Session `before` state (G16, #1506) are merged. #1475 (PR #1487 merged, pending PO verification) owns the approved G12 landing page; its commercial terms were cut to two statements by G17 (PR #1509, merged): the hero line “Free for 30 days, $10/month after.” and the pricing block, with no “no card” and no terms in the closing band (PO + PM accepted this as superseding #1470's closing-band copy, recorded on #1470). Open follow-up: the G16 numeric body (clarity change vs the previous comparable session), awaiting the Designer's Option A §6.
