...
> **`baseline` and `deployed-release` CONVERGE, and that is a read, not an inference.** Both are `5b89dc8636ce69740eda66ade7987c280c74180a`. The deployed release was read directly from `window.__APP_RELEASE__` on the canonical Production app `https://speaksharp-public.vercel.app` at **2026-09-19T11:01:27Z**, by a read-only page load with no interaction and no sign-in. It equals `main`'s tip because the #1502 deploy had propagated by then — **not** because a merge was taken as evidence of a deployment. The criterion below is unchanged and still governs: the deployed release is a read, never inferred from a merge or a moving `main` pointer, so the next time `main` advances these two values diverge again until a new read is taken. The previous entry (`61597bdcd7a6bd587b3579c4deb6462ef0a98adf`, read 2026-09-18T23:03:00Z after #1498) is superseded by this read.
...
|---|---|---|
| **Repository `main` (moving branch pointer)** | `5b89dc8636ce69740eda66ade7987c280c74180a` (after #1502), read 2026-09-19 | Re-read GitHub before every exact-pair decision. |
| **Deployed product release (last READ)** | `window.__APP_RELEASE__ = 5b89dc8636ce69740eda66ade7987c280c74180a`, read from the canonical Production app `https://speaksharp-public.vercel.app` at 2026-09-19T11:01:27Z | **A literal read, by read-only page load with no interaction or sign-in.** It currently equals the baseline because the #1502 deploy had propagated when the read was taken; that equality is the observed result, not the method. This value must be re-read before any RWT, deployment or go/no-go claim, and it goes stale the moment `main` advances — a merge is never evidence that it deployed. |
...
| **Open merge candidates** | PR #1503 (Focus Points set-up retheme, G7), PR #1505 (#1498 P2 follow-ups), PR #1504 (FAQ currentization), PR #1488 (canary; merge-queue pilot) | Each needs exact-head CI including browser shards, cleared review, PM acceptance and an exact PO head/base authorization. **Codex Code Review capacity is exhausted; per the PO (2026-09-19), PM + Consultant code reviews substitute for it until it returns** (Codex Security still runs). |
...
| **Documentation currency** | This currentization (issue #1318) | Restores the baseline to `main@5b89dc86`. The currency guard reads committed files and ancestry only. |
...
1. **Close current PRs serially.** Merged 2026-09-16…19: #1477, #1486, #1467, #1487, #1490, #1493, #1492, #1494, #1498 and #1502. Open now: PR #1503, #1505 (the #1498 P2 follow-up), #1504, #1499 (this document) and PR #1488. Each restacks and requalifies on the exact head after every `main` advance.
