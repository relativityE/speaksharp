...
| Post-Session | **Post-save reconciliation (#982)** | Current (shipped) | After save, one consolidated `StatusNotificationBar` (finalized transcript + retained secondary Private CTA + Analytics link), a once-per-session completion toast "Next: Analytics" (in-flow, ≥5s, `aria-live="polite"`), and a filler disclosure that reads the finalized snapshot. Mobile uses a 2-row bar. | Finalized signal published only at the terminal join (persist → reconcile → formatter terminal), session-guarded; covered by unit + E2E (`tests/e2e/paid-invite-trust-smoke.e2e.spec.ts`) + 4-state screenshots. |
