-- Store Stripe customer ids when subscription webhooks confirm paid status.
--
-- This is intentionally an overload of process_stripe_webhook_event rather than
-- a destructive replacement. During deploy, the old edge function can still call
-- the five-argument RPC while the newly deployed function calls this six-argument
-- version with p_stripe_customer_id.
