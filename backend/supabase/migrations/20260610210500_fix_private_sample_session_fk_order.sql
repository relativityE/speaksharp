-- Fix Private sample session persistence order.
--
-- The original Private sample migration wrote user_profiles.private_sample_session_id
-- inside update_user_usage before create_session_and_update_usage inserted the
-- corresponding sessions row. Production correctly rejected that with FK 23503.
-- This replacement creates the session row first, then marks sample usage against
-- the existing session id. If usage enforcement rejects the session, the inserted
-- active row is removed before returning the usage error.
