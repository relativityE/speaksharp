-- Acknowledge Stripe downgrade webhooks that reference an unknown subscription.
--
-- Stripe may send deleted/failed-payment events for test, already-removed, or
-- otherwise unmatched subscription ids. Retrying those forever adds noise and
-- does not protect paid access. Multiple matched rows remain an invariant
-- violation and still fail hard.
