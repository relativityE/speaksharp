# Executive Summary: Sharding Unit Coverage for CI Speed
...
* **Failing on CI failures vs. Merging:** Initially, the logic aborted merging tests failed. The code review indicated that merging should always continue even when unit tests fail, ensuring developers receive partial telemetry to debug failures. I updated `ci.yml` `Require Upstream Success` check in the `unit-coverage-merge` job to explicitly allow `failure` so developers have access to coverage stats for code that *did* compile.
