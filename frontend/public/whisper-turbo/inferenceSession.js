...
export class InferenceSession {
    ...
    transcribe(audio, raw_audio, options, callback) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (callback) {
                if (this.session instanceof Session) {
                    return yield this.session.stream(audio, raw_audio, options, callback);
                }
                else {
                    return yield this.session.stream(audio, raw_audio, options, Comlink.proxy(callback));
                }
            }
            else {
                return yield this.session.run(audio, options);
            }
        });
    }
    destroy() {
