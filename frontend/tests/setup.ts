...
if (typeof window !== 'undefined') {
    ...
    // Fake LIVE-class key (format-only) + explicit payments-enabled flag so component tests exercise
    // the payments-ENABLED UI path under the two-part gate (arePaymentsEnabled = explicit flag AND live
    // key — see P0.1). The flag-off / test-key / missing -> hidden behavior is asserted deterministically
    // in appRuntimeConfig.test.ts (pure paymentsEnabled matrix) and via explicit per-test overrides.
