 ...
 */
/**
 * #1314 C3 — the per-key map filtered to the COUNTING tier: true fillers + the user's own words
 * (+ discourse markers only when opted in). Discourse markers are EXCLUDED by default, so the
 * on-session review chips and the total both come from this one map and can never contradict, while
 * the true-filler headline is preserved. Returns `{}` for a measured zero / all-discourse snapshot,
 * and `null` only when the input carries no usable per-key map.
 */
export const countedFillerMap = (
    fillerWords?: Record<string, number> | FillerCounts | null,
    { includeDiscourseMarkers = false, userWords = [] }: FillerTierOptions = {},
): FillerCounts | null => {
    if (!fillerWords || typeof fillerWords !== 'object' || Array.isArray(fillerWords)) return null;
