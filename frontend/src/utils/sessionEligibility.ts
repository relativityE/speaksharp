 ...
 *   | Word count       | ≥ 75                                               |
 *   | Transcript       | present                                            |
 *   | Attribution      | `verified` (`isVerifiedAttribution`)               |
 *
...
import { isVerifiedAttribution } from '@/constants/attributionStatus';
...
export function coachingIneligibilityReason(input: SessionEligibilityInput): IneligibilityReason | null {
    ...
    if (!isVerifiedAttribution(input.attributionStatus)) return 'unverified_attribution';
    return null;
}
