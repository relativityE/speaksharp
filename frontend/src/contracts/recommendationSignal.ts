// #1306 — the STRICT structured recommendation contract that REPLACES the free-form `ai_suggestions` prose.
// A session persists EXACTLY ONE structured next action built only from fixed enums + numeric values — no
// transcript, quoted speech, or free-form coaching text ever crosses the persistence interface. The
// user-facing copy is generated at RENDER time from these codes (see `renderRecommendationCopy`), never stored.
//
...
export const RECOMMENDATION_TEMPLATE_VERSION = 'rec_v1' as const;
...
export const TEMPLATE_VERSIONS = [RECOMMENDATION_TEMPLATE_VERSION] as const;
 ...
 */
export function validateRecommendationSignal(input: unknown): RecommendationValidation {
  const errors: string[] = [];
  if (typeof input !== 'object' || input === null || Array.isArray(input)) {
    return { ok: false, errors: ['recommendation must be a non-array object'] };
  }
...
export function renderRecommendationCopy(signal: RecommendationSignal): { title: string; body: string } {
