...
export const SIGNAL_CONTRACT = [
  ...
  },
  {
    ...
    intent: 'Boot/render-path barrier. Indicates React app boot reached render, not that visible route content has committed.',
    waitGuidance: 'Use only as a first boot wait. User-visible browser tests must wait for data-app-visible-ready via waitForAppVisibleReady().',
    replacement: 'data-app-visible-ready',
  },
  {
    name: 'data-app-visible-ready',
    kind: 'dom-signal',
    status: 'active',
    audience: ['e2e', 'manual-proof', 'diagnostic'],
    owner: 'frontend/src/stores/useReadinessStore.ts',
    writers: ['router readiness signal after requestAnimationFrame route commit'],
    readers: ['tests/e2e/helpers.ts waitForAppVisibleReady()'],
    intent: 'Visible route-shell barrier. Indicates first meaningful visible shell/content has committed for the current route.',
    waitGuidance: 'Use this with data-app-ready for browser tests that need user-visible readiness.',
    replacement: null,
  },
  {
