...
export const generateSessionPdf = async (session: Session, username: string = 'User', isPro: boolean = false) => {
  ...
  try {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      // PRD §283: Free Tier Watermark
      if (!isPro) {
        doc.setFontSize(10);
        doc.setTextColor(180, 180, 180);
        const watermarkText = 'Generated by SpeakSharp (Free Tier) • Upgrade to Pro for Clean Exports';
    ...
    if (typeof window !== 'undefined') {
      document.body.setAttribute('data-pdf-token', isPro ? 'clean' : 'watermarked');
