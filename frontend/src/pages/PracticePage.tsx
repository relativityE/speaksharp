...
function ModeCard({ vars, art, title, promise, description, marker, ctaLabel, onClick, selected, testid }: {
  vars: React.CSSProperties; art: React.ReactNode; title: string; promise: string; description: string;
  marker: string; ctaLabel: string; onClick: () => void; selected?: boolean; testid: string;
}) {
  return (
    <button type="button" onClick={onClick} data-testid={testid} aria-expanded={selected} style={vars}
      className={`group ss-ring flex flex-col overflow-hidden rounded-3xl bg-[color:var(--ss-surface)] text-left transition-all duration-200 ${selected ? 'shadow-xl shadow-slate-900/[0.12] ring-2 ring-[color:var(--ss-card)] -translate-y-1' : 'shadow-lg shadow-slate-900/[0.06] ring-1 ring-[color:var(--ss-border)] hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-900/[0.10] hover:ring-2 hover:ring-[color:var(--ss-card)]'}`}>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <span className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-[color:var(--ss-card-btn)]">{ctaLabel} <ArrowRight size={15} aria-hidden className="transition-transform group-hover:translate-x-0.5" /></span>
      </div>
    </button>
  );
}
...
export default function PracticePage() {
  ...
  return (
    <div className="practice-root ss-landing-canvas min-h-screen font-sans antialiased">
            ...
            <div className="mx-auto -mt-6 max-w-5xl px-5 pb-12 sm:px-8">
              <div className="grid grid-cols-1 items-stretch gap-5 md:grid-cols-2">
                <ModeCard vars={QUICK_VARS} art={<QuickPracticeArt />} title="Quick Practice" promise="Speak freely. See how you’re progressing."
                  ...
                  marker="No agenda required." ctaLabel="Explore Quick Practice" onClick={openQuick} testid="practice-card-quick" />
                <ModeCard vars={GUIDED_VARS} art={<GuidedRehearsalArt />} title="Guided Rehearsal" promise="Prepare what matters. Rehearse until it lands."
                  ...
                  marker="Agenda and outcome guided." ctaLabel={guidedExpanded ? 'Hide preview' : 'See how it works'} onClick={toggleGuided} selected={guidedExpanded} testid="practice-card-guided" />
              </div>
