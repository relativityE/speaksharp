...
describe('CloudAssemblyAI', () => {
  ...
  it('should get a token and open a websocket on startTranscription', async () => {
    ...
    expect(MockWebSocket).toHaveBeenCalledExactlyOnceWith(expect.stringContaining('token=fake-token'));
    ...
    expect(micStream.onFrame).toHaveBeenCalledExactlyOnceWith(expect.any(Function));
  });
  ...
  it('should handle incoming partial transcript messages', async () => {
    ...
    const messageEvent = { data: JSON.stringify({ type: 'Turn', transcript: 'hello', turn_is_formatted: false }) };
  ...
  it('should handle incoming final transcript messages', async () => {
    ...
    const messageEvent = { data: JSON.stringify({ type: 'Turn', transcript: 'hello world', turn_is_formatted: true, words: [] }) };
    mockSocketInstance.onmessage(messageEvent as MessageEvent);
  ...
  it('should close the websocket and unregister frame handler on stopTranscription', async () => {
    ...
    expect(mockSocketInstance.close).toHaveBeenCalledExactlyOnceWith(1000);
  });
});