...
class MockHeartbeatEngine implements ITranscriptionEngine {
    constructor(private type: 'private' | 'native' | 'cloud' = 'private') { }
    private liveness = Date.now();
    public setLiveness(val: number) { this.liveness = val; }
    ...
    getEngineType = () => this.type;
    getLastHeartbeatTimestamp = () => this.liveness;
}
...
describe('TranscriptionService - 8s Heartbeat (Task 7)', () => {
    ...
    it('should correctly handle multi-segment handoff (Task 8)', async () => {
        ...
        engine.getTranscript.mockResolvedValue('Chapter 1 text from Private');
        ...
        expect(onHistoryUpdate).toHaveBeenCalledWith(expect.arrayContaining([
            { mode: 'private', text: 'Chapter 1 text from Private' }
        ]));
        expect(service.getTranscriptHistory()).toHaveLength(1);
        expect(service.getTranscriptHistory()[0]).toEqual({ mode: 'private', text: 'Chapter 1 text from Private' });
