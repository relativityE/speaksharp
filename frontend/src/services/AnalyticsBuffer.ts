...
import { sanitizePrivateSampleProps } from './transcription/privateSampleTelemetry';
...
class AnalyticsBuffer {
  ...
  private send(event: AnalyticsEvent): void {
    try {
      ...
      // allowlist (`sanitizePrivateSampleProps` in privateSampleTelemetry). Here, at the send boundary,
      ...
      const sanitized = isPrivateEvent
        ? sanitizePrivateSampleProps(event.properties)
