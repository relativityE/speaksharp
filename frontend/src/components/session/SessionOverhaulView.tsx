...
export const SessionOverhaulView: React.FC<SessionOverhaulViewProps> = ({
    ...
    if (canDeriveCoverage) {
        /**
         * `reviewSettled`, NOT `inAfter` (#1421 P1).
         *
         * `resolveSessionState` returns `after` the moment Stop flips the view to
         * `isFinalizing && !isListening` — before the transcript, the evaluation and the save finish. The
         * nearby review telemetry was already moved to `reviewSettled`; this call was left on the broader
         * condition, so it still emitted the coverage verdict and marked `evaluation_complete` early. The
         * mark deduplicates permanently, so the real evaluation could never correct it, and the emitted
         * rows could describe a pre-final transcript.
         *
         * `canDeriveCoverage` (from #1423) decides whether we may compute a verdict at all; `reviewSettled`
         * decides whether it is the FINAL one worth reporting. Both are required.
         */
        coverage = deriveFocusCoverage(effObjectivePoints ?? [], transcriptSource, effElapsed, coveredLatch.current, reviewSettled);
