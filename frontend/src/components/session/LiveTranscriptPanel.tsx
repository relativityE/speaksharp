...
export const LiveTranscriptPanel: React.FC<LiveTranscriptPanelProps> = ({
    ...
    return (
        ...
        >
            ...
            <div
                ref={containerRef}
                className="flex-1 overflow-y-auto min-h-32 glass rounded-xl p-4 leading-relaxed transition-all"
            ...
            >
                {isListening && (!transcript || transcript.trim() === '') ? (
                    <p className="text-muted-foreground italic animate-pulse">Listening and transcribing...</p>
                        ...
                        {tokens.map((token, i) => {
                            ...
                            if (token.type === 'filler') {
                                return (
                                    <span
                                        key={i}
                                        className="bg-secondary/20 text-secondary px-1.5 py-0.5 rounded-md font-medium mx-0.5 transition-all"
                                    >
                                        {token.text}
                                    </span>
                                );
                    ...
                    <p className="text-muted-foreground italic">Your words will appear here in real-time...</p>
                )}
            </div>
        </div>
    );
