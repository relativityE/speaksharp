                    ...
                    <ResponsiveContainer width="100%" height={200}>
                        <LineChart data={accuracyData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                            ...
                            {engines.map((engine, i) => (
                                <Line key={engine} type="monotone" dataKey="accuracy" data={accuracyData.filter(d => d.engine === engine)} name={engine === 'Private' ? 'Private' : engine} stroke={colors[i % colors.length]} strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 6 }} />
                            ))}
                        </LineChart>
                    </ResponsiveContainer>
                ) : (
