...
| 27 | **CI/Local Command Parity Guard** | `package.json`, `.github/workflows/*`, `scripts/run-ci.mjs` | P1 | 🔴 Not Started | Ensure `pnpm ci:full` demonstrably matches GitHub CI required gates or document the exact intentional differences. Release blocker for production readiness. |
| 28 | **Goal-Setting Readiness Hang** | `tests/e2e/goal-setting.e2e.spec.ts`, analytics readiness flow | P1 | 🔴 Active | `pnpm test:full` passes lint, typecheck, unit, and build, then reports 39/40 E2E. Remaining blocker: custom-goals E2E hangs on `Readying your experience / Synchronizing your preferences...`. |
...
| **L2** | **Test Category Consolidation** | **HIGH** | ✅ Complete | Superseded legacy per-phase test trackers. Current execution ladder: `pnpm test:infra`, `pnpm test:unit`, `pnpm test:e2e`, `pnpm test:full`, and `pnpm ci:full`. Latest validation: 621 unit passing + 1 todo; isolated `full-suite` E2E reached 40/40, while `pnpm test:full` currently reports 39/40 due to the goal-setting readiness hang tracked above. |
