...
| 17 | **Unified STT Engine Contract (Cloud/Native)** | `CloudAssemblyAI.ts`, `NativeBrowser.ts` | P2 | 🟡 In Progress | Migrating Cloud/Native engines to the `STTEngine` base class to enable unified heartbeat monitoring and telemetry. In progress, separate sprint. Does not affect CI gate today. |
