...
Our Playwright E2E tests rely on a sophisticated set of mocks and stubs to create a stable and deterministic test environment.
- **Network Stubbing (`tests/sdkStubs.ts`):** This is the core of our E2E strategy. It uses `page.route()` to intercept all outgoing network requests. Third-party services (Sentry, PostHog) are blocked, and all Supabase API calls (`/auth/v1/**`, `/rest/v1/**`) are intercepted to return predictable mock data. This allows us to simulate various user states (Pro, Free, usage exceeded) without needing a live backend.
- **Session Injection (`__E2E_MOCK_SESSION__`):** Tests for authenticated users use `page.addInitScript()` to inject a `__E2E_MOCK_SESSION__` global variable into the browser context *before* the application loads. The `AuthContext` is designed to detect this variable and use the mock session data instead of making real authentication calls.
- **Media Device Mocking (`tests/mockMedia.ts`):** To handle browser-level microphone permission prompts, which are problematic in a headless environment, we use an init script to replace `navigator.mediaDevices.getUserMedia` with a custom function that returns a fake, silent audio stream. This completely bypasses the browser's permission UI and prevents tests from hanging.
- **State Synchronization (`waitForAppReady`):** Our tests use a custom `waitForAppReady` helper function that waits for multiple conditions to be met before proceeding with interactions. This includes waiting for React to mount, for our network stubs to be ready (`window.__STUBS_READY__`), and for our mock session to be initialized (`window.__SESSION_READY__`). This prevents race conditions between Playwright's actions and the application's asynchronous state initialization.
