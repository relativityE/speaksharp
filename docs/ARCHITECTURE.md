...
The SpeakSharp application operates in three distinct environments, each tailored for a specific purpose. The Vite build tool uses a `--mode` flag to apply the correct configuration for each context.

*   **Production (`production`):** This is the live environment for end-users. It is launched from a static build generated by `pnpm build` (`vite build --mode production`). The code is fully optimized, minified, and connects to the live Supabase database and third-party services.

*   **Local Development (`development`):** This is the primary workspace for developers. It is launched using `pnpm dev` (`vite`), which defaults to `development` mode. This mode enables features like Hot Module Replacement (HMR) for rapid feedback and connects to a development database.

*   **Test (`test`):** This is a specialized, automated environment for running the E2E test suite. It is **launched automatically by Playwright**, not manually by a developer. The test runner uses the command `vite --mode test` to start the server. This mode is critically different:
    *   It uses Mock Service Worker (MSW) to provide a completely mocked backend, ensuring test isolation and determinism.
    *   It sets a build-time flag (`import.meta.env.VITE_TEST_MODE`) that allows the application source code to conditionally exclude libraries that are incompatible with the test environment, preventing silent crashes.
