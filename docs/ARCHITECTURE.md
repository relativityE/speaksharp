...
The project's testing strategy emphasizes E2E and integration tests to ensure reliable user flows. This is supported by a robust, deterministic E2E testing environment orchestrated by Playwright, with a focus on comprehensive, inline logging for CI/CD environments.

### Failure and Logging Strategy

To ensure a robust and debuggable CI process, especially within environments without persistent filesystems (like an AI agent's sandbox), the testing strategy has been re-architected around ephemeral, in-memory logging.

**Core Principles:**
1.  **Fail Loud:** All CI shell scripts (`ci-run-all.sh`, etc.) still begin with `set -e` to ensure immediate exit on failure.
2.  **Automated, Inline Logging:** Instead of writing to log files, a custom Playwright fixture (`/tests/setup/verifyOnlyStepTracker.ts`) automatically wraps every test action (`goto`, `click`, `expect`, etc.). This fixture prints structured log messages directly to `stdout`.

**CI Log Analysis:**
When a test fails, the CI logs provide a rich set of diagnostic information without needing to access any files:

| Log Marker | Description |
|---|---|
| `---STEP_START---` | Indicates the beginning of an action (e.g., "Click .my-button"). |
| `---LAST_SUCCESSFUL_STEP---` | **(On Failure Only)** Explicitly states the last action that completed successfully. The error occurred on the *next* action in the test code. |
| `[pageerror]`, `[requestfailed]` | Captures runtime browser errors and failed network requests, providing context for the failure. |
| `---DEBUG_SCREENSHOT_BASE64_START---` | **(On Failure Only)** A base64-encoded PNG of the UI at the moment of failure, which can be parsed for visual inspection. |

This approach provides a complete, self-contained diagnostic record within the CI log itself, making it ideal for automated analysis and debugging. For a detailed guide on using these markers, see `AGENTS.md`.

### Hardened E2E Test Configuration

To address persistent environment instability and non-deterministic test outcomes, the E2E testing configuration has been significantly hardened.

**Key Changes:**
1.  **`playwright.config.ts`:**
    *   The `webServer` configuration now pipes all `stdout` and `stderr` from the Vite server process directly into the test runner's output. This makes server-side errors immediately visible.
    *   Default artifact generation (`screenshot`, `video`, `trace`) is turned **off**. All diagnostic information is now generated by the custom `verifyOnlyStepTracker` fixture and printed to the console.
    *   The JSON reporter is configured to output a structured report to `test-results/playwright-report.json`.
    *   A `deviceScaleFactor: 1` has been added to ensure consistent rendering between local and CI environments, mitigating visual regression failures.

2.  **`postcss.config.cjs`:**
    *   The configuration has been updated to use `@tailwindcss/postcss` instead of the deprecated `tailwindcss` plugin, resolving a critical build failure in the CI environment.
...
| `playwright.config.ts` | Configures Playwright, the `webServer`, and the test reporters. |
| `postcss.config.cjs` | Configures PostCSS for Tailwind CSS processing. |
| `tests/setup/verifyOnlyStepTracker.ts` | The custom Playwright fixture that provides automated, inline logging for all E2E tests. |
| `test-results/playwright-report.json` | A structured JSON report of the test run results. |
