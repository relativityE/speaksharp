...
*   **[ACTIVE] E2E Test Flakiness due to MSW Race Condition:** The E2E test suite (`smoke.e2e.spec.ts`) is currently unstable due to a network race condition between the application and the Mock Service Worker (MSW) initialization.
    *   **Context & Handoff for Next Agent:** This is the final unresolved issue after a major effort to stabilize the test environment. The root cause is that the Playwright test script often executes faster than MSW can initialize in the browser. When the test's `programmaticLogin` helper attempts to set a Supabase session, it triggers a real network request from the application's Supabase client *before* MSW is ready to mock it. This results in a `net::ERR_NAME_NOT_RESOLVED` error in the browser console and a test timeout.
    *   **Attempted Solution & Why It Failed:** A synchronization flag (`window.mswReady`) was implemented. The application's entry point (`src/main.tsx`) was modified to set this flag to `true` after MSW initialized, and the test helper (`programmaticLogin`) was modified to wait for this flag. This approach failed with a timeout, indicating that the `msw.start()` promise was not resolving in the test environment, preventing the flag from ever being set. This suggests a fundamental issue with how MSW's lifecycle is managed within the Vite dev server when run under Playwright.
    *   **Recommendation:** The next agent should investigate a different synchronization strategy. Instead of relying on a flag set by the application, a more robust approach would be to have the test environment itself poll for a network-level signal. For example, the test could repeatedly try to fetch the mock `user` endpoint (`/auth/v1/user`) and only proceed with the login flow once it receives a successful mocked response. This would make the test resilient to the MSW startup time and decouple it from the application's internal initialization sequence.
