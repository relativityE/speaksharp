...
export function comparisonRows(evidence) {
  ...
  for (const row of evidence.candidateEvidence) {
    ...
    for (const [name, value] of [['controlNonce', row.controlNonce], ['persistedSessionId', row.persistedSessionId]]) {
      if (typeof value !== 'string' || !TOKEN.test(value)) throw new Error(`${cell} ${name} is invalid`);
    }
  ...
  return { releaseSha, rows: evidence.candidateEvidence, nonces: [...nonces], sessions: [...sessions] };
}
...
export function decodePostHogRows(rows) {
  ...
  return rows.map((row, index) => {
    if (!Array.isArray(row) || row.length !== 11) throw new Error(`PostHog row ${index} has an unexpected shape`);
    return {
