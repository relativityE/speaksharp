...
const CANARY_TRIAL_EMAIL = process.env.CANARY_TRIAL_EMAIL;
const CANARY_PAID_EMAIL = process.env.CANARY_PAID_EMAIL;

if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY || !CANARY_TRIAL_EMAIL || !CANARY_PAID_EMAIL) {
  const msg = 'Missing Supabase credentials or protected canary identity configuration for the ceiling check';
...
const res = await enforceCeiling(admin, {
  max: CANARY_MAX,
  enforce: ENFORCE,
  allowedEmails: [CANARY_TRIAL_EMAIL, CANARY_PAID_EMAIL],
});

if (res.status === 'ok') { console.log(`  [OK] ${res.count}/${res.max} configured canary identities exist.`); process.exit(0); }
