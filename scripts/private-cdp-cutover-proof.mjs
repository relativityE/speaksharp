...
if (release !== EXPECTED_RELEASE) {
  console.error(`STALE_BUNDLE page=${release ?? 'null'} expected=${EXPECTED_RELEASE}`);
...
console.log(`PROOF_DONE release=${release} terminalObserved=${terminalObserved} cloudRequests=${cloudHits.length} persistenceFailures=${persistenceCalls.filter((c) => !c.ok).length} recognizedLen=${peak.len} artifact=${ART}`);
await b.close();
