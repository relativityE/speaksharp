...
function resolvedPackageVersion(specifier: string, packageName: string): string {
  let cursor = dirname(require_.resolve(specifier));
  for (;;) {
...
const ONNXRUNTIME_WEB_VERSION = resolvedPackageVersion('onnxruntime-web', 'onnxruntime-web');
...
async function main(): Promise<void> {
  ...
  const requestedModel = arg('model', MODEL_NAME);
  ...
  try {
    ...
    await page.goto(`${baseUrl}/private-dropin.html?privateModel=${encodeURIComponent(requestedModel)}`, { waitUntil: 'domcontentloaded' });
    ...
    const row = finalizeRow({
      ...
      runtime_capability: {
        ...
        runtimePath: proof.runtime.workerReportedThreads === 1 ? 'wasm' : 'wasm-multithread',
        crossOriginIsolated: proof.runtime.crossOriginIsolated,
        sharedArrayBufferAvailable: proof.sharedArrayBufferAvailable,
        fallbackReason: proof.runtime.workerReportedThreads === 1
          ? 'crossOriginIsolated=false; production worker configured and reported the single-thread WASM floor'
          : null,
      },
